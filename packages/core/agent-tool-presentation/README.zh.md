...
preset 能拥有的是这份注册表的**呈现方式**。`ctx.tools.presentAs()` 只为正在挂载的那个 agent 声明，于是一个 PTC 会话可以和多个 native 会话同进程并存，各自看到各自的清单。[`dsh-tools`](../tools/README.md) 那一行上的 `mode` 仍然是默认值，供未作声明的 agent 使用。

## 它做什么

`native` 立即生效。PTC 则等待 `ctx.ptcRuntime`——这是一个宿主平面服务（[`dsh-ptc-runtime-node`](../../ptc-runtime/ptc-runtime-node/README.md)）：若某个 preset 在未组装运行时的部署上选择 PTC，本行就停在 pending，`dsh-agent-presets` 会指名此 id 拒绝挂载。另一种做法——先乐观应用——会把失败推迟到该会话的第一次请求，那时操作者对 preset 和组装都已无从下手。
...
- **运行时仍在宿主平面** —— preset 可以选择 PTC，却无法自带它所需的运行时；未组装运行时的部署也就无法组装任何 PTC preset。
