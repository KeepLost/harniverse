...
- `PromptSection` — `{ name, order, text, complete?, interpolate? }`. Static sections are concatenated in ascending `order`. Order bands: `-100` is the harness identity and tool guidance uses `100–199`; the order-0 deployment persona is a dynamic context. One effective `complete` section suppresses all other sections after cooperative assembly. `interpolate: false` keeps the resolved text verbatim — for external content (e.g. MCP server instructions) whose braces are literal, not variable references.
...
- `renderPrompt(assembly)` — interpolates `{{variable}}` references in each section, drops empty sections, joins with blank lines. STRICT: an unknown reference (`Object.hasOwn` lookup — prototype names like `{{constructor}}` are unknown), a registered-but-valueless reference, a malformed complete `{{…}}` group, or a `{{` that opens no complete group while a `}}` still follows (`{{{model}}}`) throws — fail loud beats shipping a malformed prompt. A lone `{{` with no `}}` anywhere after it passes through verbatim; substituted values are never re-scanned. Sections marked `interpolate: false` skip interpolation entirely and keep their resolved text verbatim.

Merge-extensible: plugins can declare extra fields on `PromptAssembly` and `AssembleContext` via declaration merging.

### Extension points

- Section providers: tool packages own their cross-call guidance (`tool:bash`, `tool:read`, …); this plugin owns the static `harness:identity` section.
- Context providers: this plugin owns the dynamic `deployment:persona` context; application and policy plugins contribute their own runtime facts.
...
By default every assembly starts with the harness identity and ordered static plugin sections after strict variable interpolation. `includeHarnessIdentity: false` omits only that fixed opener. Empty sections disappear; scoped sections and variables can shadow globals for one agent. The `system-prompt/assemble` waterfall determines the delivered prompt and tool schemas unless one effective section declares itself complete; that exact section then becomes the whole system prompt while the waterfall's contexts, tools, and variables remain. Ordered dynamic contexts are assembled separately and become sourced user-role runtime snapshots only when present. `includeRuntimeContext: false` or a scoped suppressor removes all such contexts, including listener additions, without disabling the services that own the underlying policy or state.
...
- **No escape syntax for literal `{{…}}` braces** — every complete group in an interpolated section is matched against registered variables; external content that must keep braces verbatim registers with `interpolate: false` instead of an escape.
