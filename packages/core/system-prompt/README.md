...
- `PromptSection` — `{ name, order, text, complete? }`. Static sections are concatenated in ascending `order`. Order bands: `-100` is the harness identity and tool guidance uses `100–199`; the order-0 deployment persona is a dynamic context. One effective `complete` section suppresses all other sections after cooperative assembly.
