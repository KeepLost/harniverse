...
- `ctx.agents.registerAdmission(admit: (session: Session) => void): () => void` — install a plugin-owned synchronous policy before any factory adopts a prepared Session; a thrown error aborts unpublished setup and publication. The registration is effect-scoped.
...
- `ctx.agents.close(id: SessionId): Promise<boolean>` — invoke the exact memoized disposer that the factory supplied when this live Agent entered the registry. Concurrent callers join that disposer; an absent id returns `false`, while a directly registered Agent without a factory-owned close capability rejects rather than simulating teardown from a bare registry entry.

`AgentHandle = { agent: Agent; dispose(): Promise<void> }`. The disposer is a **consumer capability** — an observer holding a bare Agent cannot synthesize teardown. The caller fiber and registered factory provider are structural co-owners, and the registry retains only the factory's exact close capability for by-id control. Every owner reaches one memoized quiescence boundary: stop new admission, cancel and await the loop, unwind its scoped world, flush the exact Session while it is still attached, then detach the Agent and Session. Scope or flush failures are collected without skipping detach. `ctx.agents.get(id)` still returns a bare `Agent`; the ACP bridge and in-process subagent backends hold consumer handles, while config-created agents are owned by the loop fiber.
