...
- `ctx.agents.registerAdmission(admit: (session: Session) => void): () => void`：在工厂接管已准备的 Session 之前安装插件自有的同步策略；抛出的错误会在未发布 setup 和发布前中止流程。注册随 effect 作用域释放。
...
- `ctx.agents.close(id: SessionId): Promise<boolean>`：调用该实时 Agent 进入注册表时由工厂提供的确切记忆化 disposer。并发调用方加入同一个 disposer；id 不存在时返回 `false`，而没有工厂自有 close 能力的直接注册 Agent 会被拒绝，不会从裸注册表条目模拟 teardown。

`AgentHandle = { agent: Agent; dispose(): Promise<void> }`。Disposer 是一项 **消费方能力**；持有裸 Agent 的观察方不能自行合成 teardown。调用方 fiber 与已注册工厂提供方是结构化共同拥有者，注册表只为按 id 控制保留工厂提供的确切 close 能力。每个拥有者都会到达同一个记忆化完全停稳边界：停止新准入、取消并等待循环、撤销其作用域世界、在确切 Session 仍附加时 flush，随后 detach Agent 和 Session。Scope 或 flush 失败会被汇总，但不会跳过 detach。`ctx.agents.get(id)` 仍返回裸 `Agent`；ACP 桥接层与进程内 subagent 后端持有消费方 handle，而配置创建的 agent 由循环 fiber 拥有。
