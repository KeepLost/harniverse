...
`maxResultTextChars` is an integer from 120 through 50,000. The minimum preserves the complete retention-failure safety warning. After the definition's `finalizeContent` runs, `ToolRuntime` counts Unicode code points across all recursively nested text blocks and always bounds their aggregate length; reasoning and non-text blocks do not consume this budget.
...
- `tools/post-execute` may replace presentation content, replace the canonical value, block with feedback, or attach ordered contexts. A definition's optional `finalizeContent` then owns its last content-only invariant across normal results and outer pipeline failures. `ToolRuntime` next concatenates the complete finalized text and saves it before shortening an oversized inline result to a bounded head/tail preview with an `artifact_read` locator; `tools/result` observes the immutable final outcome. Content replacement is not a confidentiality boundary: block or replace the value when programmatic consumers must not receive it.

If complete retention is unavailable or fails, the runtime emits a bounded `TOOL_RESULT_RETENTION_FAILED` error instead of publishing an unrecoverable partial success. Its warning says the operation may have completed and not to retry blindly because it may have side effects; callers must verify external state or otherwise apply the tool's retry semantics before repeating it.
...
The loop retains model-emitted arguments and the registry's final bounded content. Oversized finalized text appears as a head/tail preview containing the opaque `artifact_read` locator for the complete saved text. A retention failure instead appears as the bounded `TOOL_RESULT_RETENTION_FAILED` warning that cautions against blindly repeating a possibly side-effecting operation. Any other thrown or denied call becomes exactly `Error: <message>`. Code Mode returns only the outer program's printed lines and rendered return value, `(run_code completed with no output)` when both are empty, or `Error: code run failed (<kind>): <message>` followed conditionally by `Captured output:` and the captured lines. Inner dispatch events stay log-only; post-execute listeners may append source-attributed context after the result.

#### Token effect

Arguments, bounded result text, and additional context are data-dependent and resent until compaction. The complete artifact text enters context only in bounded `artifact_read` pages requested by the model. Restrictions that hide tools also remove their schemas before the model can call them.
