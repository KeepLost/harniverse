...
describe('ToolRuntime', () => {
  ...
  it('runs asynchronous final-result policy after tool finalization and before notification', async () => {
    const ctx = await setup()
    const order: string[] = []
    ctx.tools.register(defineContentToolFixture({
      name: 'finalized', description: 'finalized result', parameters: {},
      async execute() { return [{ type: 'text', text: 'body' }] },
      finalizeContent: (_exec, result) => {
        order.push('definition')
        const text = result.content[0]?.type === 'text' ? result.content[0].text : ''
        return [{ type: 'text', text: `${text}-definition` }]
      },
    }))
    ctx.on('tools/finalize-result', async (_exec, result, next) => {
      order.push('policy-before')
      await Promise.resolve()
      const delegated = await next()
      order.push('policy-after')
      const text = result.content[0]?.type === 'text' ? result.content[0].text : ''
      return { ...delegated, content: [{ type: 'text', text: `${text}-policy` }] }
    })
    ctx.on('tools/result', () => { order.push('result') })

    const result = await ctx.tools.execute({
      signal: testToolSignal,
      callId: CallId('finalized'),
      name: 'finalized',
      arguments: {},
    })

    expect(result.content).toEqual([{ type: 'text', text: 'body-definition-policy' }])
    expect(order).toEqual(['definition', 'policy-before', 'policy-after', 'result'])
  })

  it('normalizes a non-lossless final-result policy outcome before notification', async () => {
    const ctx = await setup()
    ctx.tools.register(echoTool)
    ctx.on('tools/finalize-result', async (_exec, result, next) => {
      await next()
      return { ...result, content: [{ type: 'text', text: () => undefined }] } as never
    })

    const result = await ctx.tools.execute({
      signal: testToolSignal,
      callId: CallId('invalid-final-result'),
      name: 'echo',
      arguments: { text: 'body' },
    })
    ...
    expect(result).toMatchObject({
      isError: true,
      content: [{ type: 'text', text: 'Error: tool result must be losslessly JSON-serializable' }],
    })
  })
