...
一个压缩（compaction）能力家族（参见[能力 seam](../../.agents/notes/implemented/architecture/2026-06-13-capability-seams.md)）：Service Definition、两个互斥摘要 provider、主动与回溯模型 Consumer、无模型工具结果修剪配套工具，以及用户命令 Consumer。这些全是**产品**包。
...
组合会选择一个后端，并通过该 seam 加载可选修剪器、主动、回溯与用户 Consumer；token 测量仍是独立的 LLM（大语言模型）家族服务。[压缩能力 seam Agent Note](../../.agents/notes/implemented/feature/2026-06-18-compaction-capability-seam.md)负责说明依赖关系，[模型请求压缩 Agent Note](../../.agents/notes/implemented/feature/2026-08-21-model-requested-context-compaction.md)负责说明主动触发，[summary DAG Agent Note](../../.agents/notes/implemented/feature/2026-08-18-summary-dag-compaction-history.md)负责说明回溯设计。
