...
- **路由策略**：主动压力与 agent-request 压缩从拥有最新持久提供方／模型路由的适配器解析容量，再将默认策略与可选的精确目标覆盖缩放为具体 token 预算。模型发现仍仅供参考，不参与此处的策略解析。
...
- **路由适配预检** —— 请求监听器还会在注定失败的请求发出*之前*恢复:当测得会话超过路由窗口减去固定 1,024 token 预留时,分层循环按最老优先压缩,保留预算逐层折半(无法收缩的层——孤立检查点或看不到新跨度的预算——是反馈而非失败,继续折半),因此后续层合法地总结先前检查点并保持可传递溯源。小于等于预留的窗口只在已溢出的信封上触发。循环耗尽预算仍装不下的信封警告一次,把失败留给 provider 路径。这是模型切换与静默截断的本地 provider 的恢复路径。
...
| `maxTokens` | 否（默认 `8192`） | 摘要调用的上限；匹配的请求策略、模型输出能力与由 Provider 锚定的安全上下文空间可以进一步降低它。可包含推理 token。 |
...
适配器可能无法为有效动态路由返回容量，已解析容量也可能暴露无效的绝对保留预算。此时压力与 agent-request 检查会抛出目标特定配置错误；自动 listener 会对该精确目标警告一次，并携带完整历史继续。不相关的操作性失败仍会独立可见。规范提供方溢出仍会尝试恢复，因为提供方已确立压缩的必要性。
...
加载插件会注册 `ctx.compaction`。随附默认配置不会启用 pruner，因此符合条件的压缩会直接进入摘要。请在该插件之前显式启用同级 [`dsh-compaction-tool-result-pruner`](../compaction-tool-result-pruner/README.md)，以添加不依赖模型的压力与溢出处理阶段。该选择启用可以减少或避免摘要器输入成本并降低溢出风险，但会改写较早的模型可见表层内容，并使从首个变更 token 起的 KV Cache 复用失效。当 `auto: true`（默认）时，compaction-basic 会在 token 压力下自动压缩。同级 [`dsh-tool-compaction`](../tool-compaction/README.md) 与 [`dsh-command-compact`](../command-compact/README.md) 分别请求活动 agent 与空闲 agent 压缩；编程调用方也可以直接使用任一 seam 操作。
...
成功步骤越过阈值，或 agent request 选择出有效的保留尾部前缀后，随附的仅摘要组合会直接进入摘要。当部署显式启用可选 pruner 时，压力与溢出可先改写超大工具结果；agent request 不会这样做。如果仍需摘要，下一个请求会收到下方检查点前导、一个空行、`<compacted-summary>`、根据数据生成的摘要以及 `</compacted-summary>`。溢出恢复会根据使表层前进的任何替换重建立即重试。检查点会替换已选较早范围，后面跟随已保留的近期单元。之后禁用 pruner 时，已剪枝的替换仍是持久表层的一部分。
...
- **没有匹配 usage 锚点时，计量准确度取决于固定启发式规则**：换用另一摘要模型或日志缺少可复用 Provider usage 时，会回退到字符数加结构开销及有界安全余量，而不是精确 token 化。
