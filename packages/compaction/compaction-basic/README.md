...
- **Convergence** — pressure retries head-checkpoint compaction up to `compactionRetries`; an agent request performs at most one retained-tail replacement. Every trigger rejects a summary that does not shrink its source, and pressure throws if retries cannot return below threshold.
- **Summarization** — a direct `llm/stream` call uses the configured provider/model pair and cap, falling back to the latest logged request target and then the agent target, without running the loop-only `agent/request` extension point. The effective output cap is the minimum of the compaction policy, the matching conversation cap, the selected model's adapter-resolved output capacity and configured request default when available, and the context space left after the replayed input plus a safety reserve of up to 4,096 tokens. A summary that uses the conversation model projects the latest matching Provider usage onto the selected prefix by subtracting the unselected tail's heuristic price, then takes the larger of that anchored result and direct estimation; another model cannot reuse that tokenizer-specific anchor. The call replays the conversation's own system prompt, tools, and shadowed-region messages verbatim, including image references, and appends the compaction instruction as the final user message, so it reuses the provider's warm prefix cache instead of invalidating it. The selected adapter must resolve or explicitly reject those images. It sets `GenerateOptions.purpose` to `compaction`, which adapters may forward as request attribution (the DeepSeek adapter sends `x-deepseek-harness-compact: 1`) without touching the model-visible body. Only returned text enters the checkpoint; reasoning and summary text stream to the authorized Web UI through transient progress events while the request runs, and the final checkpoint keeps only the safe text summary.
...
- **Lifecycle** — all entry points share one bracket-first region transaction. It validates the range and live lock, appends `compaction/start` synchronously, prepares and awaits the summary, revalidates, appends `compaction/summary` plus the replacement, and makes exactly one closing attempt. Pressure is checked both before step derivation and after the current input has been appended; the latter can compact before dispatch and rebuilds the request history from the committed replacement. The request listener also clamps `maxTokens` to the remaining model capacity; canonical provider overflow enters through `agent/request-error`, first halves an explicit output cap and retries, then uses forced compaction as the fallback. `compactNow()` waits for a running agent to settle, reserves idle admission, uses `turn: null`, accepts append-only context outside its selected span, flushes every closed attempt, and releases admission in `finally`.
- **Route-fit pre-flight** — the request listener also recovers *before* a doomed request is sent: when the measured session exceeds the routed window minus a fixed 1,024-token reserve, a layered loop compacts oldest-first with a retained budget that halves each layer (a layer that cannot shrink — a lone checkpoint or a budget that sees no fresh range — is feedback, not failure, and halves again), so later layers legitimately summarize earlier checkpoints with transitive provenance. Windows at or below the reserve only trigger on an already-overflowing envelope. An envelope that cannot fit after the loop exhausts its budgets warns once and leaves failure to the provider path. This is the recovery for model switches and for local providers that silently truncate instead of reporting overflow.
...
| `maxTokens` | no (default `8192`) | Upper bound for the summarization call; matching request policy, model output capacity, and Provider-anchored safe context space can reduce it. May include reasoning tokens. |
...
Loading the plugin registers `ctx.compaction`. Shipped defaults load it without an active pruner, so qualifying compaction proceeds directly to summarization. Explicitly enable [`dsh-compaction-tool-result-pruner`](../compaction-tool-result-pruner/README.md) as a sibling before this plugin to add the model-free pressure and overflow pass. That opt-in can reduce or avoid summarizer input cost and lower overflow risk, but it rewrites earlier model-visible surface content and invalidates KV-cache reuse from the first changed token. With `auto: true` (the default) compaction-basic compacts automatically under token pressure. The siblings [`dsh-tool-compaction`](../tool-compaction/README.md) and [`dsh-command-compact`](../command-compact/README.md) request active-agent and idle-agent compaction respectively; programmatic callers may also use any seam operation directly.
...
- **Meter accuracy follows the fixed heuristic without a matching usage anchor** — another summarization model or a log without reusable Provider usage falls back to character count plus structural overhead and the bounded safety reserve rather than exact tokenization.
