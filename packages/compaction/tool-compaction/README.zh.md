...
工具接受一个必填且非空的 `reason`,以及可选的 `from`/`to` 位置用于指名要压缩的跨度:1 起算、从最老保留消息数起,两者须成对提供。都省略时维持策略选段。Provider 从受保护的执行身份取得调用方 Agent,应用已路由模型的保留策略,保持工具调用／结果配对,并将取消转发给摘要流程。显式跨度必须止于当前轮之前;边界向外向内吸附以保持工具配对(结果会报告吸附的位置数),解析后的跨度经编程式 `compactRegion()` 事务以 `agent-request` 归属执行。结果文本报告压缩条目数、遮蔽 token 数与保留上下文的估算规模,供模型判断后续调用。Provider 从受保护的执行身份取得调用方 Agent，应用已路由模型的保留策略，保持工具调用／结果配对，并将取消转发给摘要流程。
