...
剪枝读取当前投影后的消息。图片卸载占位文本保持完整，包括先前替换中已经保存的占位文本。它们计入 `thresholdChars`，头尾文本按剩余预算缩减。若占位文本与标记无法一起容纳，则保留该结果供模型压缩处理。每个发出的替换均不超过阈值且严格小于输入；同一消息的替换会保留仍可见图片的出现龄。
...
所有值都必须是整数；阈值必须为正数，头部／尾部必须为非负数。`headChars + marker + tailChars` 之和不得超过 `thresholdChars`。完整占位文本耗尽预算时，该结果无法剪枝。
