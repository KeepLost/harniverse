...
注册由根层持有的 `compaction` 设置命名空间。可选的 `thresholdRatio` 会在下一次压缩决策时覆盖所有 Agent Profile 的自动压力阈值。该命名空间还承载 context-nudge 策略——`nudgeEnabled`、`nudgeThresholdTokens`(首次提醒的绝对 token 占用)与 `nudgeRefireDeltaTokens`(再次提醒前所需的增长;必须小于阈值,否则覆盖被忽略)——由 [`@deepseek-ai/dsh-context-nudge`](../../context/context-nudge/README.md) 消费,并在 Web 设置压缩卡上暴露。该值缺省时，每个 Profile 的压缩 Provider 继续使用其组合配置阈值。
