...
- **`compaction/end`**（无 `error`）—— 由一个受控的异步恢复持久追加待发布消息。代理已空闲时立即执行（手动 `/compact`）；仍有轮次在运行时，恢复等待空闲落定，因为落在该轮最后一个请求里的压缩在轮内已无后续边界。追加是幂等的：已通过上述路径恢复过快照的轮次在此不再写入。
...
- **空闲后的压缩恢复是尽力而为** —— 落在空闲检查之间的 `compaction/end` 会在空闲落定或下一个步骤/请求边界恢复；没有持久标记跟踪遗漏的恢复。
