...
The authenticated `sessionReferenceResolver/candidates` Remote adds a canonical mention without reconstructing identity from the title. The service wraps `agent/pre-step` after downstream admission, parses canonical user mentions, and enters the untrusted snapshot context followed by the readable direct message.
