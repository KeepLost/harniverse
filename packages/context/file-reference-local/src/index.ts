...
import { DEFAULT_FILE_SEARCH_EXCLUDED_DIRECTORIES, DEFAULT_FILE_SEARCH_MAX_ENTRIES, DEFAULT_FILE_SEARCH_MAX_RESULTS, WorkspaceFileSearch, type FileSearchConfig } from './search.ts'
...
/** Local provider with one bounded cache per live Agent. */
  ...
  constructor(ctx: Context, config: Config = {}) {
    ...
    const installPrompt = (agent: Agent): void => {
      ...
      const fiber = agent.ctx.inject(['systemPrompt', 'tools'], (scope) => {
        scope.systemPrompt.section({ name: 'context:file-reference', order: 99, text: () => agent.ctx.tools.get('read', agent) === undefined ? '' : FILE_REFERENCE_PROMPT })
      })
    ...
    ctx.on('agent/disposed', ({ agent }) => { this.searches.get(agent)?.dispose(); this.searches.delete(agent); disposePrompt(agent) })
    ...
    ctx.effect(() => async () => {
      ...
      await Promise.all([...fibers.map(fiber => fiber.dispose()), ...this.promptDisposals])
    }, 'file-reference-local: search cache')
  }

  override list(agent: Agent, query: string, signal: AbortSignal): Promise<FileReferenceCandidate[]> {
