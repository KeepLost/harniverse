...
`dsh-subprocess-ssh` implements `ctx.subprocess` using the shared SSH helper. Executable lookup, ordinary processes and terminal sessions run beside the SSH filesystem. Remote native process owners govern termination and quiescence; consumers continue to own command semantics, output limits and execution deadlines.
...
Ordinary spawn returns a handle while remote allocation proceeds; piped stdin accepts writes during allocation. Terminal allocation, writes, foreground inspection, signals and termination retain their asynchronous interfaces. The handle's `pid` stays `-1` until the remote start acknowledgement publishes it.
...
Executable verification comes from the remote helper, and lookup misses remain distinct from SSH failures. Terminal dimensions are fixed at allocation on this seam generation.
...
- Callers must consume or close raw output streams. Destroying public stdout or stderr closes its matching transport, including when destruction precedes remote allocation. Independent channels do not remove per-stream backpressure or shared-network congestion.
...
The `./invariant` companion registers package ownership with an explained empty installer: wire validation and the owning filesystem, subprocess and sandbox providers enforce the observable obligations, so this adapter adds no independently observed state relation.
