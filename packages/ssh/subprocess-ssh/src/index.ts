...
/** One remote ordinary process; stdin remains usable during asynchronous SSH allocation. */
class RemoteProcess implements SubprocessHandle {
  ...
  get pid(): number { return this.observedPid }
  ...
  private async start(): Promise<void> {
    ...
    try {
      ...
      const started = await this.ssh.request('process.start', { id: this.id }, z.object({ pid: z.number().int() }).strict(), this.controller.signal)
...
export class SshSubprocessRuntime extends SubprocessRuntime {
    ...
    ctx.effect(() => async () => {
      ...
      const results = await Promise.allSettled([
        ...[...this.live].map(async (handle) => {
          try { await handle.waitForExit() } finally { handle.terminate() }
        }),
  ...
  override async resolveExecutable(command: string, env?: Readonly<Record<string, string>>, signal?: AbortSignal): Promise<string> {
    return await this.ctx.ssh.request('executable', { command, env: environment(env) }, remotePath, signal)
  }
  ...
  override spawn(spec: SubprocessSpawnSpec): SubprocessHandle {
    ...
    void handle.done.then(() => handle.waitForExit())
