...
在自定义 `dsh` 配置组合中，将本服务与 [`fs-ssh`](../fs-ssh/README.md)、[`subprocess-ssh`](../subprocess-ssh/README.md) 和 [`sandbox-ssh`](../sandbox-ssh/README.md) 组合。主机运行 Harness、模型传输和 Session 存储；远端机器提供文件与进程。headless 配置组合支持这种安排。
...
为了 PTC 就绪，配置两个引导字段，使 `ctx.ssh.nodeExecutable` 与 `ctx.ssh.bootstrapPath` 暴露经验证的远端坐标。仅使用文件系统和 Bash 时可以省略这对字段。未配置 PTC 部署时，`bootstrapPath` getter 会拒绝访问。
...
OpenSSH 主连接承载私有管理 RPC。每条程序流使用独立转发的 Unix 套接字和独立 SSH 通道。程序 stdout 无法伪造管理回复；SSH 传输拥塞仍会影响共享连接。
...
- [SSH 子系统](../../../docs/subsystems/ssh.md) — 执行坐标、传输语义及生命周期归属。
...
- 新进程 PTC 运行时仍通过本地进程 seam 生成子进程；完全远端的 PTC 组合随后续 seam 演进提供。
