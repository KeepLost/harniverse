...
describe.skipIf(process.platform === 'win32')('SSH collected output', () => {
  it('captures the full stream while the snapshot reader is paused and retains its spill after disconnect', async () => {
    ...
    try {
      ...
      const output = await helper.connectStream(prepared.streams.stdout as SshStreamEndpoint)
      const error = await helper.connectStream(prepared.streams.stderr as SshStreamEndpoint)
      error.end()
      error.resume()
      expect(output.readableFlowing).toBe(false)
      await helper.client.request('process.start', { id: prepared.id }, z.object({ pid: z.number().int() }).strict())
      ...
      expect(Buffer.from((completed.collected.stdout as { tail: string }).tail, 'base64').toString()).toBe(payload.slice(-16))
  ...
  it('uses raw byte offsets for the final tail and exposes the complete remote spill', async () => {
    ...
    try {
      ...
      const tail = (handle.collected.stdout as NonNullable<typeof handle.collected.stdout>).readFrom(0)
      expect(tail).toMatchObject({ text: '\ufffdefg', nextOffset: 8, lossy: true })
      expect((handle.collected.stdout as NonNullable<typeof handle.collected.stdout>).readFrom(7)).toMatchObject({ text: 'g', nextOffset: 8, lossy: false })
      spill = tail.spillPath
      expect(await readFile(spill!)).toEqual(bytes)
      expect((handle.collected.stderr as NonNullable<typeof handle.collected.stderr>).readFrom(0)).toEqual({ text: '', nextOffset: 0, lossy: false })
    } finally {
      await fiber.dispose()
      await helper.close()
      if (spill !== undefined) await rm(spill, { force: true })
