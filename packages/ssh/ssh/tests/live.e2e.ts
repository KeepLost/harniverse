...
describe.skipIf(!enabled)('POSIX SSH runtime acceptance', () => {
  it('refuses same-user sandboxed connectors before granting a process stream', async () => {
    ...
    try {
      ...
      const endpoint = prepared.streams.stdout!
      ...
      await test.ctx.ssh.request('process.start', { id: prepared.id }, z.object({ pid: z.number().int() }).strict())
  ...
  it('shares guarded file mutations, byte reads, shell cwd and symlink identity', async () => {
    ...
    try {
      ...
      expect(Buffer.from(await ctx.fs.readBytes(target, undefined, 64)).subarray(1, 4).toString()).toBe('eta')
      const chunks = []
