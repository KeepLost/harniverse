...
function ordinary() {
  ...
  const finish = () => { stdout.end(); stderr.end(); completion.resolve(outcome) }
  const terminate = vi.fn(finish)
  const waitForExit = vi.fn(async () => { await completion.promise; return true })
  const handle: SubprocessHandle = {
    pid: 7, stdin, stdout, stderr, collected: {}, done: completion.promise, terminate, waitForExit,
  }
  return { handle, finish, completion, stdin, stdout, stderr, terminate, waitForExit }
}
...
describe.skipIf(process.platform === 'win32')('SSH helper process settlement', () => {
  ...
  it('pipes stdin bytes and distinguishes an unstarted reservation from a completed process', async () => {
    ...
    try {
      ...
      await expect(test.owner.terminal(run.id, 'write')).rejects.toThrow('does not own a terminal')
      const bytes: Buffer[] = []
