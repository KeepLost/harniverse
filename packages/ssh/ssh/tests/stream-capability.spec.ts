...
describe.skipIf(process.platform === 'win32')('SSH stream capabilities', () => {
  it('refuses an unknown TLS identity without consuming a legitimate reservation', async () => {
    ...
    try {
      ...
      const endpoint = prepared.streams.terminal as { path: string; capability: string }
  ...
  it('refuses another endpoint capability without consuming the legitimate reservation', async () => {
    ...
    try {
      ...
      const target = prepared.streams.stdin as SshStreamEndpoint
      await expect(connect(target, prepared.streams.stderr!.capability)).rejects.toThrow()
      const legitimate = await connect(target, target.capability)
      sockets.push(legitimate)
      expect(legitimate.destroyed).toBe(false)
      expect(target.capability).not.toBe(prepared.streams.stdout!.capability)
      expect(target.capability).toMatch(/^[0-9a-f]{64}$/)
    } finally {
      for (const socket of sockets) socket.destroy()
  ...
  it('does not treat administrative-looking bytes as a helper request on a data listener', async () => {
    ...
    try {
      ...
      socket = createConnection((prepared.streams.stdout as { path: string }).path)
      socket.on('error', () => {})
      await once(socket, 'connect')
      ...
      await closed
      const published = prepared.streams.stdout as SshStreamEndpoint
      const legitimate = await connect(published, published.capability)
      socket = legitimate
      expect(legitimate.destroyed).toBe(false)
    } finally {
      socket?.destroy()
