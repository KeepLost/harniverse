...
| [`subprocess-ssh`](subprocess-ssh/README.md) | Executable lookup, remote processes and terminals | `ctx.subprocess` |
...
Remote capability implementations retain the shared asynchronous confinement and cancellation interfaces. Local path access must never be inferred from a remote path string.
