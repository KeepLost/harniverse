...
Skills can be directory bundles (`SKILL.md` at any depth under a root, never entering hidden directories, `node_modules`, or `.git`, bounded to ten levels) or root-level flat Markdown files (`<name>.md`). Frontmatter is parsed as an open YAML object with the `yaml` package; this provider interprets required `name` and `description`, plus optional `whenToUse` and `metadata`. Names must be kebab-case.

The removed invocation-policy fields `disable-model-invocation`, `user-invocable`, and their camelCase spellings are rejected at parse time: the skill drops from discovery with a warning instead of silently misloading, because every discovered skill is now model- and user-invocable. Wrong-typed optional `whenToUse` and `metadata` values are omitted because neither grants loading.
