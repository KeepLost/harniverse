...
- **删除按生命周期身份阻止迟到写回。**`delete(meta)` 移除记录并记住被删除的 header 身份；已经排队、属于该确切生命周期的写入会被拒绝，不会重建 checkpoint。使用同一 id 但 header 身份不同的重建 Session 可以正常写入。
...
持久化 Session 发布时，缓存会尝试把经过身份校验的行播种进注册表。播种失败会按缓存未命中处理，绝不会使权威会话日志不可用。
...
- **没有按时间或配额淘汰**：显式 Session 删除会移除匹配 checkpoint，而策略驱动的保留机制仍未提供。
