 ...
 *
 * The same Koffi bindings also hold the session write lease as a named kernel
 * semaphore (count 1) derived from the lock path — never a file lock or
 * handle, so readers, searches, and directory removal proceed freely while
 * the lease is held.
 *
...
/**
 * Acquire the session write lease as a named kernel semaphore (count 1) whose
 ...
 * while the lease is held; a second acquirer's zero-timeout wait times out
...
/**
 * Release a lease from {@link acquireLockHandleWin32}: restore the semaphore
