...
`readHistoryPage()` reads complete raw lines from the tail of an uncompressed artifact or complete Zstandard frames from the end of a compressed artifact. `readRawEventPage()` uses the same stable tail reader but stops after decoding the requested number of raw events. Both return only the bounded page; malformed or legacy input falls back to the full validated-prefix reader.
...
- **Cold deletion.** `delete(id)` resolves the unique transcript, validates its header and derived path with the same identity checks as reads, unlinks only that file, and `fsync`s its containing directory on POSIX before returning. It never recursively removes the session directory and never follows a link-shaped path. The shared coordinator rejects live or exclusively prepared identities and serializes deletion against same-id operations.
