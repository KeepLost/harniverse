...
- **同引用即无工作。** 对与单元无关的事件，`apply` 必须返回同一个状态引用；驱动以 `Object.is` 双重把守变更流——不匹配的事件只花一次调用，不产生任何下游工作；状态引用虽变但原始 `view` 结果保持引用不变（对象或数组 view 在仅内部状态变化时复用其引用）同样保持安静，而结构相等的新对象仍算一次变化。
...
- **主动驱动（eager drive）逐事件触达每个单元**——按构造开销很低（全量值规则、状态/视图引用闸门），但若出现热点路径，可加按单元的事件类型预过滤，约定不变。
