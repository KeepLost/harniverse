 ...
 * unit's watermark at emission (the seq of the event that caused the
 * change).
 */
...
export class SessionProjectionRegistry extends Service {
   ...
   * @param listener - called once per unit whose raw view changed by `Object.is`, per committed event.
  ...
  /** Eager drive: pass one committed event through every registered unit; notify on changed raw view references. */
    ...
    for (const registration of this.registrations.values()) {
      ...
      if (changed) {
        ...
        if (this.listeners.size > 0) {
          views[1] = registration.def.view(next)
          if (!Object.is(views[0], views[1])) {
            const value = registration.def.schema.parse(views[1])
