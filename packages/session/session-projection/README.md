...
- `ctx.sessionProjections.onChanged(listener): () => void` Subscribe to the change feed: one call per unit whose raw view changed by `Object.is`, per committed event, carrying the schema-validated view and the causing seq. Effect-tied like `register`.
...
- `ctx.sessionProjections.hydrate(session, checkpoint, events, baseSeq): ProjectionSnapshot` Validate a persisted checkpoint and fold only its forward tail into a live Session. Invalid nonzero-base rows are rejected as a cache miss, never silently initialized from an incomplete window.
...
- **Same-reference means no work.** `apply` MUST return the same state reference for events that do not concern the unit; the drive gates the change feed on `Object.is` twice — a non-matching event costs one call and nothing downstream, and a changed state whose raw `view` result keeps its reference (an object or array view reusing its reference across internal-only state changes) stays quiet, while a structurally equal new object is still a change.
...
- **Eager drive touches every unit per event** — cheap by construction (whole-value rule, state/view reference gates), but a hot path would justify per-unit event-type prefilters, addable without contract change.
