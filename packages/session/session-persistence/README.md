...
| `delete(id): Promise<boolean>` | Delete one cold materialized Session. Coordinator-backed providers serialize the operation against same-id work, wait retirement, reject a live Session or an exclusive unpublished preparation, invalidate reusable preparation state, and return whether storage existed. Shared attachments and external derived stores are not part of this seam. |
...
| `readHistoryPage(id, request): Promise<{ meta; events; hasMore }>` | Return one detached backward display-history page without resuming a Session. `beforeSeq` is exclusive and `maxMessages` counts append-origin user/assistant messages. On an initial request, `preferLatestCheckpoint` returns the latest compact-plugin replacement checkpoint transaction and every later raw event even when that exceeds `maxMessages`; `hasMore` keeps the superseded prefix reachable. The search extends past the quota by at most `CHECKPOINT_SEARCH_MESSAGE_BUDGET` further append-origin messages and never walks to the head of the log, so a compaction-free session costs the ordinary bounded tail read instead of a full-artifact scan. Without a checkpoint inside that window, the ordinary quota page is returned. SQLite seeks candidates and the contiguous range; JSONL reads backward from the artifact tail and falls back to full validation for malformed or legacy data. |
...
| `deleteStored(id)` | Delete one authoritative stored identity and return whether it existed. The coordinator owns live/preparation exclusion and same-id serialization; the backend owns physical durability. |
...
- **No retention scheduler or attachment garbage collector** — callers choose explicit Session deletions; shared content-addressed attachments and age/quota policy remain separate concerns.
