...
Schema 18 stores ordinary `SessionEvent` values as scalar rows and compatible runs of at least three consecutive `assistant/chunk` text, reasoning, or tool-call deltas as packed physical rows. It also stores hash-bound surface state for replacement checkpoints, including reset and partial compaction. Packing preserves every logical event, timestamp, sequence number, chunk boundary, and optional surface field. One row represents at most 1,024 logical events and 1 MiB of uncompressed UTF-8 `data`; payloads of at least 4 KiB use Zstandard level 3 only when the compressed frame is smaller. Scalar `source_event_seqs` values use lossless Varint/ZigZag delta encoding, including a distinct representation for an empty list. The full physical rules and rationale live in the [SQLite chunk-row Agent Note](../../../.agents/notes/implemented/architecture/2026-08-22-sqlite-physical-chunk-row-compression.md).
...
- **Windowed checkpoint restore.** A checkpoint binds the surface after the last completed turn before a replacement to the physical prefix through that replacement. The coordinator restores this surface state and its suffix, resolving earlier events by absolute sequence. Known packed-row tags remain physical encoding, not unknown required Session events. Missing or damaged checkpoint metadata falls back to ordinary full recovery, and JSONL keeps the full-load path.
...
- **Checkpoint verification scans the physical prefix.** It avoids payload decoding, not all prefix I/O. Full-history consumers can materialize earlier events; assembled Agent memory is not bounded by the restore window.
