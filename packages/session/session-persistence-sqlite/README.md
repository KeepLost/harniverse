...
`readHistoryPage()` uses indexed `seq` order to find the latest append-origin message candidates, then reads one contiguous logical event range. It does not reconstruct the complete session log for a cold backward history page, even when the requested range starts inside a packed row.
...
- **Logical suffixes over physical rows.** `readFrom()` and `readHistoryPage()` inspect the bounded packed predecessor that may contain the requested sequence, decode it as one logical span, and return only members inside the requested range.
