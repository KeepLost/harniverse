 ...
 */
export async function appendAccessRecord(record: AccessRecord, options: AccessLogOptions = {}): Promise<void> {
  ...
  await withPrivateFileLock(path, async () => {
    ...
    if (await fileSize(path) + Buffer.byteLength(line) > maxBytes) await rotate(path, maxFiles)
