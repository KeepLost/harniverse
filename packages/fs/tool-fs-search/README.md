...
| `grepMaxLineBytes` | `2000` | Producer-side byte cap per matched-line preview. ripgrep bounds an overlong line before stdout capture and marks the omitted tail; Native rendering preserves UTF-8 boundaries and marks its inline cut `(line truncated)`. |
...
| `grep` | `pattern`, `path?`, `include?` | Line-oriented ripgrep output with NUL-separated path and line-number fields (no colon or newline ambiguity), plus `--max-columns` / `--max-columns-preview` to bound matching lines at the producer. `pattern` is a ripgrep regex; `path` is an optional **file or directory** target; `include` is ONE positive glob filter — a comma-separated list or a negated (`!…`) value is rejected up front (brace alternation like `*.{ts,tsx}` is fine). Returns matches grouped by file as `Line N: <preview>`. |

Routine budgets stay out of the model-facing schema (no `head_limit`/`offset`/`case_insensitive`/output modes): a model that needs surrounding context reads the matched file with `read`. For a truncated line, it uses the reported line number as `offset` and follows same-line `line_byte_offset` cursors; one that needs later results follows the returned spill locator's retrieval hint.

## Two budgets, two artifacts

Raw `rg` stdout and stderr are internal transport details. Each search requests collect-mode budgets from the subprocess seam — complete stdout within `rawOutputMaxBytes` and a `stderrMaxBytes` diagnostic tail — with no spill files on either stream (the tool never reads a raw spill path). Grep additionally gives ripgrep `grepMaxLineBytes` as `--max-columns` with `--max-columns-preview`; JSON mode is not used because ripgrep ignores max-column controls there. Every grep record is therefore bounded before capture by the preview cap plus a fixed omission marker, an OS-bounded path, and numeric framing, so the parser may safely consume complete stdout under the aggregate raw-output cap without collecting a giant matching line. If the seam still reports a lossy stdout read, the search fails with `SEARCH_RAW_OUTPUT_OVERFLOW` and tells the model to narrow the query; a lossy stderr read only marks the diagnostic excerpt `[stderr truncated]`. A successful `glob` keeps the displayed search root and every acquired path in `{ root, paths }`; when sampling is enabled, `root` lets the Native renderer group an explicit relative or absolute search path by entries beneath that root rather than by its workdir prefix. `grep` keeps every acquired `{ path, lineNumber, line }` in `{ matches }`, where an overlong line is the producer-bounded preview and omission marker rather than the complete line. Inline item retention and the final UTF-8-safe preview pass apply in the Native renderer. For a direct surface call with more logical results than the inline cap, post-policy best-effort saves the complete formatted previews through `ctx.spillStore.saveText()` and replaces only presentation with the configured page plus locator. Nested Code dispatches skip that spill because their full canonical value does not enter model context. Missing/failed spill keeps the inline page and reports that the complete result could not be saved—never an `isError`.

## Errors

Search failures carry the package-owned `SearchError` (a `HarnessError` subclass), surfaced as `{ name, code }` on `isError` results: `SEARCH_INVALID_PATTERN` (ripgrep rejected the regex/glob), `SEARCH_FAILED` (a failed `rg` launch, inaccessible target, signal kill, malformed output), `SEARCH_RAW_OUTPUT_OVERFLOW` (raw output over `rawOutputMaxBytes`, or still lossy after the requested stdout capture budget), and `SEARCH_ABORTED` (cooperative tool timeout or caller cancellation). ripgrep exit semantics are tool-owned: exit 0 is success with results, exit 1 is a successful empty search (`No files found` / `No matches found`), and only other exits are failures. Model argument mistakes (blank pattern, a list-valued `include`) stay ordinary tool argument errors.
