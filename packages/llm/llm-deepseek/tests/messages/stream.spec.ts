/** Protocol invariants at the Messages JSON/SSE boundaries, including partial and failed responses. */

import { describe, expect, it } from 'vitest'
import { translate } from '../../src/protocols/messages/translate.ts'
import { parseSse } from '../../src/protocols/messages/sse.ts'
import { providerError, providerErrorDetail } from '../../src/protocols/messages/transport.ts'
...
describe('Messages stream', () => {
  ...
  it('emits first-seen blocks, raw tool JSON and cumulative disjoint usage before finish', async () => {
    ...
    expect(result.output.at(-2)).toEqual({ type: 'usage', usage: { inputTokens: 12, outputTokens: 5, cacheReadTokens: 30, cacheWriteTokens: 7 } })
  ...
  it('preserves signature-only thinking and prunes truncated tools with their replay entries', async () => {
    ...
    expect(result.message.source.kind === 'model' && result.message.source.replayState).toMatchObject({ blocks: [{ type: 'reasoning', signature: 'opaque' }] })
  })
...
describe('SSE framing and provider failures', () => {
  ...
  it('reads only classified provider detail fields', () => {
    expect(providerErrorDetail({ error: { code: 'a', type: 'b', message: 'c', extra: true } })).toBe('a b c')
    expect(providerErrorDetail(null)).toBe('')
    expect(providerErrorDetail({ error: null })).toBe('')
    expect(providerErrorDetail({ error: 'text' })).toBe('')
  })
  it.each([
    [401, {}, 'AUTH'], [403, {}, 'AUTH'], [402, {}, 'QUOTA'], [429, {}, 'RATE_LIMIT'],
    [400, {}, 'INVALID_REQUEST'], [413, {}, 'INVALID_REQUEST'], [503, {}, 'SERVER'], [404, {}, 'HTTP_404'],
    [undefined, { type: 'authentication_error' }, 'AUTH'],
    [undefined, { type: 'rate_limit_error' }, 'RATE_LIMIT'],
