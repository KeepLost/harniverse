...
`baseURL` sets the endpoint of every model on the route, so private proxies such as `https://proxy.example.com:8443` remain supported; a catalog route that omits it keeps each catalog model's own endpoint. A custom endpoint is named with its version segment — `https://gw.example/v1`, the spelling OpenCode and every OpenAI-compatible gateway publish — and that one address serves whichever protocols the route mixes: the OpenAI SDKs join their paths onto it directly, while for a model speaking `anthropic-messages` the trailing `/v1` comes off at resolution because the Anthropic SDK's own paths re-add it (`/v1/messages`), so the request lands at `{baseURL}/messages` exactly as OpenCode sends it; the official endpoints' root spelling (`https://api.anthropic.com`) serves unchanged too. Naming `api` on a catalog route repoints every model that does not name its own at that protocol, which is how a deployment moves a provider between, say, Responses and Chat Completions — and how one route serves a mixed-protocol façade: the route names the protocol most of its models speak, and the exceptions name theirs per entry.
