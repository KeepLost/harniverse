 ...
 *
 * Only OpenAI-compatible and Anthropic Messages protocols are interrogated,
 * each through its native listing endpoint. The parser accepts the standard
 ...
 * falls back to hand-entry rather than guessing a response shape. The
 * Anthropic path is reached only by an explicit declaration — the draft's own
 * protocol or the named route's profile — never by probing an undeclared
 * endpoint over a second protocol.
 *
...
export interface StoredModelDiscoveryProfile {
  /**
   * Wire protocol the named route's profile explicitly declares, read only
   * when the draft carries none. This is the one way a stored declaration —
   * not a guess — selects the native Anthropic listing path.
   */
  readonly api?: string
...
export async function discoverModels(
  ...
  // A draft that has chosen no protocol yet falls back to the named route's
  // own explicit declaration, and only then to OpenAI Chat Completions: it is
  // the shape a gateway is overwhelmingly likely to speak, and the
  // alternative — refusing until the field is filled — would withhold the
  // action from the case it exists for. Neither fallback ever probes an
  // endpoint over a protocol nobody declared, so an undeclared route is asked
  // exactly once, OpenAI-shaped, as it always was.
