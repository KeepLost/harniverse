 ...
 *
 * Durable content is authoritative; replay metadata only restores native
 * fidelity. Unusable metadata degrades the one message to provider-neutral
 * history instead of failing the request.
 * @param message - assistant content with optional adapter-owned replay metadata.
 * @param onDegrade - called with the diagnostic reason when metadata is unusable.
