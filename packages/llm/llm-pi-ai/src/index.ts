...
export function apply(ctx: Context, config: Config): void {
  ...
  ensureDirectory()
  /**
   * The host-owned request inputs for discovery of one configured route: the
   * protocol and headers its profile declares, plus the credential it
   * resolves. A route being declared for the first time names no profile yet,
   * and a profile that names no credential defers to pi-ai's own discovery,
   * so both answer `undefined` for what they do not carry.
   */
  ...
  // except the stored protocol, deployment-owned headers, and credential: the
  // curated UI accepts none of them, so an already-configured route supplies
  // its own here rather than widening the discovery request.
