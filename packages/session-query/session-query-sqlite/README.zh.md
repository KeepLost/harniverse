...
该索引使用 FTS5 `unicode61`，并在索引与查询两侧都做二元词组（bigram）折叠：每个超过两个字符的 CJK/假名/谚文连续串都以空格连接的重叠二元词组入库，因此“压缩”这类双字子词可以命中更长的连续串，多词查询以无序 AND 短语匹配。空白分隔的查询词各自独立引用并取 AND，不再要求相邻。片段展示会剥离插入的二元连接空格，摘录仍按原文呈现。对这些字符集之外的文字，取舍仍是 token/短语召回而非任意子字符串召回：`AI` 不匹配 token `BRAID`。需要执行字面的空白弹性子字符串扫描时，使用 `ctx.sessionQuery.filterEvents()` 并传入 `text` 子句。查询会拒绝 NUL；文档中的保留高亮标记和 NUL 会在索引前被规范化，使展示标记无法与源文本冲突。
