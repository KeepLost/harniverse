...
- Chromium's own sandbox must be waived (`sandbox: 'none'`) when the harness runs as root, which is the common container posture; the process still runs scrubbed and profile-isolated, but a page exploit then faces one fewer boundary.
