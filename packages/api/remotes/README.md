...
`createApiRemoteAgentResolver()` reuses live Agents, resumes ordinary cold sessions, deduplicates concurrent resumes, preserves the subagent ownership fence, and configures the same resolver for Typert `agent` and `session` lookups. Its pre-resume checks read the stored header, not the log: the ownership fence and the recorded Profile are header facts, and the resume loads the log itself, so reading it here would traverse every event a second time on the same per-session persistence chain and delay the caller's own history page. A project-less identity is rejected against the resumed session's header, which is that authoritative load. The standard Web API Proxy supplies its Agent defaults and scope setup, then uses the returned resolver for legacy methods, so migrated and unmigrated methods share one policy implementation.
