...
/**
 * One foreground observation of the shell. Harniverse's terminal seam exposes
 * `inspectForeground` rather than the official `inspectActivity`, so the
 * controller maps `{processGroupId, inputWaiting} | undefined` onto the
 * activity vocabulary retention reasons about; `revision` is the process group
 * id because that is the seam's only monotonic-enough identity.
 */
export interface TerminalActivity {
  readonly state: 'unknown' | 'idle' | 'busy'
...
export class TerminalRetention {
  ...
  constructor(
    ...
    private readonly inspect: () => Promise<TerminalActivity>,
