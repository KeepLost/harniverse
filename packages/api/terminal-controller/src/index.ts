...
import { BrowserTerminal, terminalActivityOf } from './terminal.ts'
...
export { TerminalFollower } from './stream.ts'
export { TerminalRetention, type TerminalActivity, type TerminalRetentionPolicy } from './retention.ts'
export { BrowserTerminal, terminalActivityOf } from './terminal.ts'
export { discoverShells, resolveShell } from './shells.ts'
...
export class TerminalController extends TypertRemoteService {
   ...
   */
  @Remote({ exportName: 'shells', requiredCapability: 'harniverse.observe' })
   ...
   */
  @Remote({ exportName: 'write', requiredCapability: 'harniverse.operate' })
   ...
   */
  @Remote({ exportName: 'rename', requiredCapability: 'harniverse.operate' })
  ...
  private async spawn(agent: Agent, owner: OwnedSession, request: TerminalCreateRequest, signal: AbortSignal): Promise<BrowserTerminal> {
    ...
    // A user terminal inherits the FULL harness environment — these are the
    // user's own system-user terminals, not model PTYs — with explicit entries
    // layered on top: a color terminal type and the owning Session identity.
    ...
    try {
      ...
      // The allocation retention is born closing; its observation callback exists only to satisfy
      // the retention constructor and is never scheduled.
      /* v8 ignore next 1 -- allocation retention never observes */
      const cleanup = new TerminalRetention(this.config, () => terminalActivityOf(handle), async () => {
