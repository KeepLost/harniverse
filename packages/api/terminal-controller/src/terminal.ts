...
import { TerminalRetention, type TerminalActivity, type TerminalRetentionPolicy } from './retention.ts'
...
/**
 * Project the seam's foreground observation onto retention's activity view.
 * @param handle - terminal process range to observe.
 * @returns unknown when no foreground job exists, idle when it waits for
 * input, busy otherwise; the process group id doubles as the revision.
 */
export function terminalActivityOf(handle: SubprocessTerminalHandle): Promise<TerminalActivity> {
  return handle.inspectForeground().then(foreground => foreground === undefined
    ? { state: 'unknown', revision: 0 }
    : { state: foreground.inputWaiting ? 'idle' : 'busy', revision: foreground.processGroupId })
}
...
export class BrowserTerminal {
  ...
  monitor(policy: TerminalRetentionPolicy, closing: () => void, closed: () => void, failed: (error: unknown) => void): void {
    this.retention = new TerminalRetention(policy, () => terminalActivityOf(this.handle), async () => {
      closing()
