...
import type { TerminalActivity } from '../src/retention.ts'
...
function fixture(policy: Partial<TerminalRetentionPolicy> = {}) {
  const inspect = vi.fn(async (): Promise<TerminalActivity> => ({ state: 'idle', revision: 1 }))
...
it('discards an asynchronous observation invalidated by a reconnect and its subsequent disconnect', async () => {
  const h = fixture()
  await vi.advanceTimersByTimeAsync(90)
  const pending = Promise.withResolvers<TerminalActivity>()
...
it('waits for pending cleanup when the observation settles first during disposal', async () => {
  const h = fixture()
  const observed = Promise.withResolvers<TerminalActivity>()
