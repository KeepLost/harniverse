...
import { BrowserTerminal, terminalActivityOf } from '../src/terminal.ts'
...
const info: WebTerminalInfo = { id: 'terminal-test' as WebTerminalId, title: 'bash', shell: { path: '/bin/bash', name: 'bash', args: ['-l', '-i'] }, cwd: '/workspace', cols: 80, rows: 24, state: 'running', exitCode: null }
...
describe('terminalActivityOf', () => {
  it('maps absent foreground, input-waiting and busy states with process-group revisions', async () => {
    const absent = { inspectForeground: async () => undefined } as unknown as SubprocessTerminalHandle
    await expect(terminalActivityOf(absent)).resolves.toEqual({ state: 'unknown', revision: 0 })
    const idle = { inspectForeground: async () => ({ processGroupId: 7, inputWaiting: true }) } as unknown as SubprocessTerminalHandle
    await expect(terminalActivityOf(idle)).resolves.toEqual({ state: 'idle', revision: 7 })
    const busy = { inspectForeground: async () => ({ processGroupId: 9, inputWaiting: false }) } as unknown as SubprocessTerminalHandle
    await expect(terminalActivityOf(busy)).resolves.toEqual({ state: 'busy', revision: 9 })
  })
})
