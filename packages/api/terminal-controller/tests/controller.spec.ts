...
const config: Config = { shellCandidates: ['zsh', 'bash', 'sh'], shell: { path: '/bin/bash', name: 'bash', args: ['-l', '-i'] }, maxTerminals: 2, maxCols: 200, maxRows: 100, scrollback: 100, maxBufferedBytes: 100_000, maxInputBytes: 1000, disposeGraceMs: 100, unattendedTimeoutMs: 7_200_000, activityPollIntervalMs: 30_000, cleanupRetryMs: 60_000 }
...
function fixture(overrides: Partial<Config> = {}) {
  ...
  const handle = {
    ...
    inspectForeground: vi.fn<SubprocessTerminalHandle['inspectForeground']>(async () => undefined), signalForeground: async () => 123,
    terminate: vi.fn(async () => { output.end(); done.resolve({ exitCode: 0, signal: null }) }),
  }
  const checked: SubprocessTerminalHandle = handle
  const subprocess = { resolveExecutable: vi.fn(async (path: string) => path), spawnTerminal: vi.fn(async () => checked) }
...
describe('TerminalController', () => {
  ...
  it('creates the platform default user shell once and reuses the committed identity', async () => {
    ...
    expect(created.shell.path).toBe(interactive.path)
  ...
  it('restores a running terminal after its default executable becomes unavailable', async () => {
    const { controller, agent, subprocess } = fixture({ shell: undefined })
    await controller.create(agent, request, signal())
    subprocess.resolveExecutable.mockRejectedValue(new Error('default shell was removed'))
  ...
  it('terminates the spawned process and records nothing when construction observes cancellation', async () => {
    const { controller, agent, subprocess, handle } = fixture()
    const pending = Promise.withResolvers<undefined>()
    subprocess.spawnTerminal.mockReturnValueOnce(pending.promise.then(() => handle))
    const abort = new AbortController()
    const creating = controller.create(agent, request, abort.signal)
    abort.abort(new Error('spawn outlived the request'))
    pending.resolve(undefined)
    await expect(creating).rejects.toThrow('spawn outlived the request')
    expect(handle.terminate).toHaveBeenCalledOnce()
    expect(controller.list(agent.id)).toEqual([])
    await expect(controller.close(agent, id)).resolves.toBeUndefined()
  })

  it('reports allocation cleanup failure alongside the construction error', async () => {
    const { ctx, controller, agent, subprocess, handle } = fixture({ cleanupRetryMs: 1 })
    handle.terminate.mockRejectedValue(new Error('pty already gone'))
    const pending = Promise.withResolvers<undefined>()
    subprocess.spawnTerminal.mockReturnValueOnce(pending.promise.then(() => handle))
    const abort = new AbortController()
    const creating = controller.create(agent, request, abort.signal)
    ...
    await expect(creating).rejects.toThrow('Terminal allocation cleanup failed')
    await vi.waitFor(() => { expect(errors).toHaveBeenCalledWith('Browser terminal allocation cleanup failed', expect.any(Error)) })
    expect(controller.list(agent.id)).toMatchObject([{ id, state: 'failed', error: 'spawn outlived the request' }])
  })

  it('honors cancellation before reading environment limits', () => {
    const { controller, agent, sandboxPolicy } = fixture()
    const abort = new AbortController()
  ...
  it('deduplicates concurrent creates, scopes access by Session, and preserves terminals when a stream aborts', async () => {
    ...
    expect(subprocess.spawnTerminal).toHaveBeenCalledOnce()
    expect(subprocess.spawnTerminal).toHaveBeenCalledWith(expect.objectContaining({ cwd: '/workspace' }))
  ...
  it('rejects invalid dimensions, unavailable shells and oversized input before executing them', async () => {
    ...
    subprocess.resolveExecutable.mockRejectedValueOnce(new Error('shell unavailable'))
  ...
  it.each(['read-only', 'workspace-write', 'danger-full-access'] as const)('starts a user shell with the full harness environment and without confinement under %s Agent permissions', async (mode) => {
...
describe('shell resolution', () => {
  ...
  it('resolves the platform default shell with login user-startup arguments', async () => {
    const { subprocess } = fixture()
    const interactive = defaultInteractiveShell()
    const runtime = subprocess as unknown as SubprocessRuntime
    const requestSignal = signal()
    expect(await resolveShell(runtime, undefined, requestSignal)).toEqual({
      path: interactive.path, args: ['-l', '-i'],
      name: interactive.path.slice(interactive.path.lastIndexOf('/') + 1),
    })
    expect(subprocess.resolveExecutable).toHaveBeenCalledExactlyOnceWith(interactive.path, undefined, requestSignal)
  })

  it('uses an explicit profile without consulting the platform default and preserves its name and arguments', async () => {
    const { subprocess } = fixture()
  ...
  it('profiles cmd without interactive arguments and powershell without its logo banner', async () => {
    const h = fixture({ shellCandidates: ['cmd', 'powershell'] })
    h.subprocess.resolveExecutable.mockImplementation(async (path: string) => path)
    const shells = await h.controller.shells(h.agent, signal())
    expect(shells.slice(1).map(found => found.args)).toEqual([[], ['-NoLogo']])
    expect(shells.slice(1).map(found => found.name)).toEqual(['cmd', 'powershell'])
  })

  it('reports default resolution failure without attempting fallback', async () => {
    const { subprocess } = fixture()
    const failure = new Error('missing default')
  ...
  it('reports an unavailable configured shell without consulting the platform default', async () => {
    const { subprocess } = fixture()
    const failure = new Error('missing configured shell')
    subprocess.resolveExecutable.mockRejectedValue(failure)
    const runtime = subprocess as unknown as SubprocessRuntime
...
it.skipIf(process.platform === 'win32')('runs a real interactive shell with completion, TERM and live window dimensions', async () => {
  ...
  try {
    const handle = await ctx.subprocess.spawnTerminal({ argv: ['/bin/bash', '--noprofile', '--norc', '-i'], cwd, rows: 24, cols: 80, graceMs: 100, env: { PS1: 'READY> ', PS2: '' } })
    let output = ''
...
it('discovers installed shells once per path, preserves default arguments, and refuses unlisted paths', async () => {
  ...
  const shells = await h.controller.shells(h.agent, signal())
  expect(shells).toEqual([config.shell, { path: '/bin/zsh', name: 'zsh', args: ['-l', '-i'] }])
  ...
  expect(h.subprocess.spawnTerminal).toHaveBeenCalledWith(expect.objectContaining({ argv: ['/bin/zsh', '-l', '-i'] }))
...
it('skips candidates whose lookup fails and propagates discovery cancellation', async () => {
  const h = fixture({ shellCandidates: ['fish'] })
  h.subprocess.resolveExecutable.mockImplementation(async (path) => {
    if (path === 'fish') throw new Error('lookup transport failed')
    return path
  })
  expect(await h.controller.shells(h.agent, signal())).toEqual([config.shell])
...
it('reclaims confirmed idle processes and preserves closed identity exclusion after removing their screens', async () => {
  ...
  try {
    h.handle.inspectForeground.mockResolvedValue({ processGroupId: 0, inputWaiting: false })
    await h.controller.create(h.agent, request, signal())
    await vi.advanceTimersByTimeAsync(500)
    expect(h.handle.terminate).not.toHaveBeenCalled()
    h.handle.inspectForeground.mockResolvedValue({ processGroupId: 1, inputWaiting: true })
