...
describe('TerminalController', () => {
  ...
  it('creates the platform default shell once and reuses the committed identity', async () => {
    ...
    expect(created.shell.args).toEqual(interactive.args)
    expect(subprocess.spawnTerminal).toHaveBeenCalledWith(expect.objectContaining({ argv: [interactive.path, ...interactive.args] }))
...
describe('shell resolution', () => {
  ...
  it('resolves the platform default interactive shell with its no-rc arguments', async () => {
    ...
    expect(await resolveShell(runtime, undefined, requestSignal)).toEqual({
      path: interactive.path, args: interactive.args,
