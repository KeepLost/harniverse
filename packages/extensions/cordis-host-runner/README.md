...
- The process-global Cordis Inspect registry keeps ordinary Host providers exclusive. Its explicit shared-registration path accepts only an identical provider manifest, keeps one lease per contributing fiber, routes through the latest surviving lease, and restores the preceding lease on disposal. This lets old and new standing Agent Profile generations coexist without weakening duplicate detection for unrelated providers.
