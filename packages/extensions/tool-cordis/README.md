...
The built-in read-only Host Inspect providers use compatible shared leases in the process-global registry. A capability-composition edit can therefore mount a new Creator Profile generation while an existing Session retains the old generation; both see one provider directory, while the model-facing tools and prompt remain owned by their respective Profile generations.
