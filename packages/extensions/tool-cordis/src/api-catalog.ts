...
export const SERVICE_API: readonly ServiceApiEntry[] = [
    ...
    methods: [
      {
        ...
        description: 'Consider compaction for one explicit trigger. Pressure policy uses the latest durable routed request, context-overflow policy may force a useful balanced reduction without retention, and an agent request bypasses the pressure threshold while retaining the configured recent tail. Return `null` when no safe range can be compacted. A single oversized retained unit or request envelope cannot be repaired through surface compaction.',
        parameters: [{ name: 'agent', description: 'agent context owning the session surface and routing options.' }, { name: 'trigger', description: 'normal pressure, provider-confirmed overflow, or an active agent request.' }, { name: 'signal', description: 'cancellation signal; model-backed implementations must forward it.' }],
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        parameters: [{ name: 'listener', description: 'called once per unit whose raw view changed by `Object.is`, per committed event.' }],
  ...
  {
    key: 'shell',
    summary: 'Abstract POSIX shell execution service.',
    description: 'Abstract POSIX shell execution service. Subclass, implement the abstract methods, and load the subclass as a plugin — it registers as `ctx.shell` (one implementation per context; loading a second throws, which is cordis\' standard duplicate-service behavior).\n\nImplementations must honor these semantics:\n\n- run rejects only for infrastructure failures. Nonzero exits, timeout kills, and abort kills resolve with a ShellRunResult.\n- start returns immediately; no timeout applies to background processes. `done` settles at process close and never rejects; spawn failures settle as `killed` with the error on stderr.\n- ShellProcess.readOutput is incremental: consecutive reads never repeat output. Lossy reads report truncation and available spill files.\n- A still-running background process is stopped and awaited when its owning composition tears down. With the subprocess seam that boundary is `ctx.subprocess` disposal, so a background process survives an executor-only reload.',
    methods: [
      {
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Retain an existing terminal for a window without activating its Agent or taking input control. Not a Remote invocation: harniverse\'s Gateway surface is request/response, so a future window-hold transport will wrap this generator with its own lease protocol.',
      ...
      },
      {
        ...
        description: 'Attach to a terminal without binding its process lifetime to the transport. Not a Remote invocation: harniverse\'s Gateway surface is request/response, so a future output transport will broadcast the follower frames it drives.',
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  {
    name: 'tools/finalize-result',
    mode: 'waterfall',
    signature: '\'tools/finalize-result\'(this: Scoped<ToolRuntime>, exec: Readonly<ToolExecution>, result: Readonly<ToolExecutionResult>, next: () => Promise<ToolExecutionResult>): Promise<ToolExecutionResult>',
    summary: 'Transform a complete normalized result after definition-owned `finalizeContent` and before authoritative lossless materialization.',
    description: 'Transform a complete normalized result after definition-owned `finalizeContent` and before authoritative lossless materialization. `next()` preserves the result. Listeners may perform asynchronous policy work but must settle before `tools/result` notification. Scope-filtered dispatch (`@deepseek-ai/dsh-scope`): agent-scoped listeners receive only that agent\'s calls.',
    parameters: [{ name: 'exec', description: 'the execution whose result is being finalized.' }, { name: 'result', description: 'the complete candidate outcome.' }],
  },
  {
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  },
  {
    ...
    declaration: 'export type CompactionTrigger = \'pressure\' | \'context-overflow\' | \'agent-request\';',
  },
  {
    name: 'ConfinedArgv',
  ...
  {
    name: 'LlmWireAttempt',
    declaration: 'export interface LlmWireAttempt {\n    readonly exchangeId: string;\n    readonly attempt: number;\n    readonly api: string;\n    readonly provider: string;\n    readonly model: string;\n    readonly url: string;\n    readonly method: string;\n    readonly request: {\n        readonly bytes: number;\n        readonly fingerprint: string;\n        readonly parameters?: Record<string, unknown>;\n    };\n    readonly response?: {\n        readonly status: number;\n        readonly headers?: Record<string, string>;\n    };\n    readonly failure?: LlmFailure;\n    readonly outcome: LlmWireAttemptOutcome;\n    readonly durationMs: number;\n}',
  },
  {
    name: 'LlmWireAttemptEventData',
    declaration: 'export interface LlmWireAttemptEventData extends LlmWireAttempt {\n    readonly turn: number;\n    readonly step: number;\n    readonly historyCutSeq?: number;\n    readonly requestHeaderSeq: number;\n    readonly requestContextSeq?: number;\n}',
  },
  {
    name: 'LlmWireAttemptOutcome',
    declaration: 'export type LlmWireAttemptOutcome = \'success\' | \'http-error\' | \'stream-error\' | \'transport-error\' | \'aborted\';',
  },
  {
    name: 'LspHover',
