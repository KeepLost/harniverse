...
skill 调用 source 的浏览器端：把 `/` 触发的 `skill` source 注册进 `ctx.inputTriggers`。普通 Session 的候选来自 `skill.list` RPC，以每次调用的 `ClientSessionContext` 投影中的 `{sessionId}` 寻址，host 从 Session header 解析 `cwd`。宿主提供每一个已发现的 skill。由目录寻址的可继续 subagent 在客户端解析为没有 skill 候选，因为现有 skill RPC 要求 Session 已挂载。目录按普通 Session 缓存，拉取走 single-flight；不可变 Agent Profile 身份让每份目录保持稳定，`connection/reset` 清空全部缓存。结果按 `startsWith(query)` 过滤。
