...
function projectionFramesOf(id: SessionId, log: readonly SessionEvent[], event: SessionEvent): Extract<MuxFrame, { type: 'session/projection' }>[] {
  ...
  // Standing-plan fold: writes replace the list; turn boundaries do not clear it.
  if (type === 'todo/write') {
    return [{
      type: 'session/projection',
      sessionId: id,
  ...
  const api: ApiProxy = {
    ...
    workspace: {
      ...
      unarchiveSession: (request) => {
        const index = archivedSessionIds.indexOf(request.payload.sessionId)
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'session.selectModelTarget': return this.fixtureApi.sessions.selectModelTarget?.(request)
        ?? Promise.resolve({ rpcId: request.rpcId, result: { ok: false, error: { code: 'internal', message: 'model target selection is unavailable', details: {} } } })
      case 'session.selectModelProfile': return this.fixtureApi.sessions.selectModelProfile?.(request)
      ...
      case 'workspace.git.commits': return this.fixtureApi.workspaceGit?.commits(request, signal)
        ?? Promise.resolve({ rpcId: request.rpcId, result: { ok: false, error: { code: 'internal', message: 'workspace Git inspection is unavailable', details: {} } } })
      case 'workspace.git.diff': return this.fixtureApi.workspaceGit?.diff(request, signal)
        ?? Promise.resolve({ rpcId: request.rpcId, result: { ok: false, error: { code: 'internal', message: 'workspace Git inspection is unavailable', details: {} } } })
