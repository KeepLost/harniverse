...
import { DEFAULT_CONTENT_FONT_SIZE, type ThemePreference } from '../theme-settings.ts'
...
/** Store state mirrored from the theme snapshot's content font size. */
export interface FontSizeRowState {
  /** Persisted content font size in px (one of the offered tiers). */
  fontSize: number
  /** Service revision; -1 until first sync so revision 0 lands as a change. */
  revision: number
}
...
export function createFontSizeRowStore(): EngineStoreHandle<FontSizeRowState, FontSizeRowActions> {
  return defineStore({
    init: (): FontSizeRowState => ({ fontSize: DEFAULT_CONTENT_FONT_SIZE, revision: -1 }),
    actions: {
