...
主题 section 同时拥有会话正文字号（`ui-theme.fontSize`，档位 14/16/18px，默认 16，以 `z.number().step(2).min(14).max(18)` 校验）。通用设置区承载该功能自有的字号设置行——注册在外观行旁的三档分段控件（小／中／大），沿用同一 store／face 模式；`setContentFontSize` 是唯一写入口，非档位值直接抛错。引导脚本把持久值嵌入每份 index 响应，ui-layout 的呈现器将其发布为 body 级的 `--dsw-content-font-size` 轴变量（dispose 时收回）。`gradient-shadow-text.css` 由该字号派生 `--dsw-content-font-delta`，并以此移动 markdown h1–h4／base 阶梯；密集次级 markdown 各档（table／small／code）与应用 chrome 字阶保持固定，对话记录的流程家具则在各组件内以 `calc(默认值 + delta)` 采纳——由 content-font-axis 样式契约钉住，默认渲染逐像素不变。
