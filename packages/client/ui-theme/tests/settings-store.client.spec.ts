/** Appearance and font-size row stores: snapshot-mirror actions and revision guards. */
...
describe('createFontSizeRowStore', () => {
  it('init shape: default content font size with revision at -1', () => {
    const store = createFontSizeRowStore().create()
    expect(store.getSnapshot()).toEqual({ fontSize: 16, revision: -1 })
  })

  it('sync mirrors the font size and advances the revision', () => {
    const store = createFontSizeRowStore().create()
    store.actions.sync(14, 0)
    expect(store.getSnapshot()).toEqual({ fontSize: 14, revision: 0 })
    store.actions.sync(18, 2)
    expect(store.getSnapshot().fontSize).toBe(18)
    expect(store.getSnapshot().revision).toBe(2)
  })

  it('revision guard drops stale and duplicate writes', () => {
    const store = createFontSizeRowStore().create()
    store.actions.sync(18, 3)
    store.actions.sync(14, 2)
    store.actions.sync(14, 3)
    expect(store.getSnapshot().fontSize).toBe(18)
    expect(store.getSnapshot().revision).toBe(3)
  })
})
