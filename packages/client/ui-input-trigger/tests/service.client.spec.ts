...
describe('arbitrate', () => {
  ...
  it('tab completes the highlight like enter', async () => {
    const { controller, cmd } = await menuBench()
...
describe('adjudicate', () => {
  ...
  it('polls matchEnter in registration order with the projection and full line; first non-undefined wins', async () => {
    ...
    const { controller } = controllerBench([
      enterSource('/', 'silent'),
      enterSource('/', 'first', (session, line, _signal, envelope) => {
        expect(session).toEqual({ sessionId: sid('a') })
        expect(envelope).toEqual({ images: 2 })
    ...
    const result = await controller.adjudicate('/goal make it fast', new AbortController().signal, { images: 2 })
