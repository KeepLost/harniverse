...
每个已打开的普通 `Session` 保留一段连续、权威的事件窗口，并只把该窗口尾部暴露为重连游标（已打开的空日志为 `-1`）。首个页面优先返回最近一次持久压缩事务及其后的全部事件，即使超过普通的 50 条消息配额；`hasMore` 与既有 `beforeSeq` 路径仍使被替代的原始前缀可经“加载更早内容”抵达。没有检查点的会话仍使用普通有界尾页。连接 generation 把窗口游标交给 `events.mux`；回放从各游标之后追加，重复 seq 被忽略。实时缺口通过前向 `session.history` 页面修复，且不会替换已经加载的更早事件。重连会保留已打开窗口，而属于失效 generation 的 loading/error 操作会重新启动；当 `session/subscribed.lastSeq` 低于本地尾部时，可证明 Host 已重启或回滚，runtime 会强制执行持久化支持的重新建基。按地址访问的 subagent 窗口仍使用尾页修复，因为 `subagent.history` 没有前向模式。详见[压缩优先历史决策](../../../.agents/notes/implemented/architecture/2026-08-22-compaction-first-session-history.md)。
...
`ConversationSnapshot.queue` 是 Host 提供的 `agent.inbox.nextTurn` 权威瞬态快照；待处理的 next-step steering（中途引导）不进入此投影。每行携带其 `MessageId`、所有内容块均为文本时的完整可编辑文本，以及扁平化预览。Host 根据持久 `agent/inbox/spliced` 变更派生完整 `session/queue` 快照，并在重连时发送基线；面向单条消息的 `agent/inbox/inserted`、`claimed` 与 `discarded` 通知不用于重建该投影。`Session.updateQueue()` 经 Host 侧 `Inbox.splice()` 发送编辑／移除操作，客户端不做乐观变更；成功结果现在直接携带变更后的 work status 和实际 delivery，而下一份 Host 快照仍是队列可见提交的唯一来源，claim 竞态则可能呈现 `queue-item-not-found`。
...
Chat builder 为每个 Session 保留一个 mutable keyed store。内容更新只通知受影响的 node key；结构变化才重建顺序和 Location 成员关系；prepend 只增加行，不替换既有 keyed value。实时或未结算的 Assistant chunk 逐条更新 Definition State，但最多每个 animation frame 请求一次物化。在批量历史 replace 或 prepend 中，Definition 可以通过 `skipHistoryUpdates()` 省略已被同一批次后续证据覆盖的 State transition；Assembler 仍保留全部 Match，未声明该 hook 的 Definition 仍 fold 完整事件流。内建 Chat 与 Trajectory Assistant Definition 各自拥有 finalized-chunk 策略，并保留计时与用量核算所需的第一条 token delta 和 usage chunk。实时 append 绝不使用这项优化。final message 与 Turn/Step 关闭会立即发布。参见 [Client Tool 展示所有权决策](../../../.agents/notes/implemented/architecture/2026-08-08-client-tool-presentation-ownership.md)。
