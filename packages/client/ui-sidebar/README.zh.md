...
根作用域的 `sidebar.header.status` seat 在展开时位于品牌与折叠控件之间，在窄栏中位于面板切换控件下方。它的 owner share 只有 `wide`；[ui-connection-status](../ui-connection-status/README.md) 提供只读图标。侧边栏既不拥有认证状态，也不执行续期工作。
