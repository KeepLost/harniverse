...
The store family (`defineStore` spec in / `StoreHandle<T, A>` out) types the store seat: `init` infers the state schema, `actions` is the complete draft-transform write set, and `BakedActions` strips the draft parameter into the callbacks components and inject factories receive. Optional `persist` accepts either a key for whole-state JSON storage or `StorePersistence<T>` with `select` and `restore` for a validated projection that rebuilds transient fields from a fresh initial state. The `defineStore` value implementation lives in the store package (`@deepseek-ai/dsh-client-store`) and satisfies the `DefineStore` contract re-exported here. Engine products and the renderer host contract carry bare snapshot sources (`getSnapshot`/`subscribe`), never React hooks — hook binding belongs to the render machinery; only the props-contract hook type (`SnapshotSelectorHook`) lives here.
