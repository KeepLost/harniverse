...
第一批覆盖 shell 执行器（`bash`）、agent 循环的工具调用并行度（`agent-loop`），以及一张横跨提供方选择器（`web`）、DeepSeek（`web-search-deepseek`）、Exa（`web-search-exa`）和 Perplexity（`web-search-perplexity`）设置的网页搜索卡片。
...
保存时，每个暂存字段都通过客户端 settings scope 写入，该 scope 用读取时的命名空间 revision 为每次写入设栅，因此已与文档脱节的表单会被拒绝，而不是覆盖并发变更。网页搜索卡片让四个表单始终存活、只渲染当前所选提供方的控件，因此隐藏草稿在切换提供方后仍会保留；它明确采用非事务保存，依次运行选择器、DeepSeek、Exa 和 Perplexity 表单，让成功表单落定、让每个失败表单保留自己的草稿，并报告任何部分失败。某个值是否被接受只有 Host 说了算——schema 表达不了的约束归它的校验器所有——因此每个表单都会在写入后回读自己的分节。

密钥也可能从别的表层写入——模型页寻址的是同一个引用——而那不改变任何 settings 分节，因此卡片会分别跟踪每个提供方当前使用的引用，并只在转发来的 `credentials/updated` 事件匹配这些引用时重读。
