...
The first batch covers the shell executor (`bash`), the agent loop's tool-call parallelism (`agent-loop`), and one Web search card spanning the provider selector (`web`) plus DeepSeek (`web-search-deepseek`), Exa (`web-search-exa`), and Perplexity (`web-search-perplexity`) settings.
...
Saving writes each staged field through the client settings scope, which fences every write with the namespace revision it read, so a form that has drifted from the document is refused rather than overwriting a concurrent change. The Web search card keeps all four forms alive while only the selected provider's controls render, so hidden drafts survive provider switches; its explicitly non-transactional save runs the selector, DeepSeek, Exa, and Perplexity forms in that order, settles successful forms, retains each failed form's drafts, and reports any partial failure. The Host is the only authority on whether a value was accepted — its validators own the constraints no schema can express — so each form reads its section back afterwards.

A key can also be written from another surface — the Models page addresses the same reference — which changes no settings section, so the card independently tracks each provider's current reference and re-reads only matching references on forwarded `credentials/updated` events.
