...
Saving writes each staged field through the client settings scope, which fences every write with the namespace revision it read, so a form that has drifted from the document is refused rather than overwriting a concurrent change. The Web search card keeps all provider forms alive while only the selected search provider and, when selected, Firecrawl fetch controls render, so hidden drafts survive provider switches; its explicitly non-transactional save runs the selector, DeepSeek, Exa, Perplexity, Tavily, Brave, Kagi, and Firecrawl forms in that order, settles successful forms, retains each failed form's drafts, and reports any partial failure. The Host is the only authority on whether a value was accepted — its validators own the constraints no schema can express — so each form reads its section back afterwards.

A key can also be written from another surface — the Models page addresses the same reference — which changes no settings section, so the card independently tracks each provider's current reference and re-reads only matching references on forwarded `credentials/updated` events. Firecrawl's search and fetch capabilities share one `FIRECRAWL_API_KEY` reference and one key control.
