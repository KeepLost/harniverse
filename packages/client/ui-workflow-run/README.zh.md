...
只有所有实时事实同时成立时，成员才可打开子 Session：成员仍在运行，且当前父 Session 的权威 subagent 目录包含同一个健康、活动状态为运行中的 child。存在运行成员的节点会在 effect 作用域内把该目录标记为打开，从而触发初始刷新，并持续接收实时成员变化，直到成员结算或组件卸载。带下划线的成员文字是唯一可见导航提示；键盘聚焦时，名称区显示 2 像素 business-primary 焦点环，右侧状态仍只显示“运行中”。注入动作调用 `sessions.open(id)`，由 manager 解析并保留目录派生的直接父级地址；目录加载中、诊断行、父级不符、非活动或终态的成员都不可交互。subagent Session 仍从普通 Session 列表中排除。
...
- 只有经 `dsh-tool-workflow` 发起的顶层调用会生成这些记录；嵌套 PTC 调用和直接 `WorkflowEngine` 消费方不会生成。
