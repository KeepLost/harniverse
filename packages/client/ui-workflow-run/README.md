...
A member opens a child Session only while every current fact agrees: the member is running and the current parent's authoritative subagent catalog contains the same healthy child with running activity. A node with a running member effect-scopes that catalog as open, triggering the initial refresh and retaining live membership updates until settlement or unmount. Underlined member text is the only visible navigation affordance; keyboard focus draws a two-pixel business-primary ring around the name area, while status copy remains `Running`. The injected action calls `sessions.open(id)`, whose manager resolves and retains the catalog-derived direct-parent address; loading, diagnostic, wrong-parent, inactive, or terminal rows remain non-interactive. Subagent Sessions stay excluded from the ordinary Session list.
...
- Only top-level calls through `dsh-tool-workflow` produce these records; nested PTC calls and direct `WorkflowEngine` consumers do not.
