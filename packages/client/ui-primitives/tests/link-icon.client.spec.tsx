...
describe('classifyLinkPath', () => {
  ...
  it('derives only file categories: url and folder are consumer-stated, never classified', () => {
    // A path that IS a web URL still classifies by its extension: destination
    // categories (`url`, `folder`) are decided by the consumer that knows the
    // destination, not by the path shape.
    expect(classifyLinkPath('https://example.com/a.ts')).toBe('code')
    expect(classifyLinkPath('https://example.com')).toBe('other')
  })
})
...
describe('LinkIcon', () => {
  ...
  it.each(kinds)('%s renders an aria-hidden svg riding currentColor', (kind) => {
