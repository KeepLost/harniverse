...
模型设置与产品引导插件。同一个 client Cordis 插件会注册 Models 页面与版本化内测声明；提供方凭据需要在 Models 页面中显式配置，首次使用时不会出现针对特定提供方的步骤。Models 平面把三个协议领域汇聚为一个共享快照：`llm.providers`（可配置提供方目录，含每条路由的存活／休眠状态）、`settings.describe`（序列化 schema、分层脱敏值、secret slot）与 `credentials.describe`（不含值的 configured/source/writable 徽标）；页面据此渲染提供方行，一次只展开一张编辑卡片，且不把路由存活状态呈现为提供方状态。
...
pi-ai profile 的 `models` 列表就在卡片上编辑：一行一个模型，行上显示 id 与显示名称，上下文窗口与输出上限收在该行的展开区内，右侧是两个无文字的操作——展开与删除。同一个展开区还承载该行的**能力声明**：一个图像输入勾选框（写入 `input: [text, image]`；不勾选则把该字段留给已安装条目与路由默认值），以及一节推理声明——勾选后声明可选的思考档位（`off`/`minimal`/`low`/`medium`/`high`/`xhigh`/`max`，至少一档非 `off`），每个档位可另填线上拼写（留空即规范拼写，`off` 表示不发送），并指定发起选择时的起始档位（「未设置」沿用路由默认值；「默认」显式表示不发送档位）。OpenAI-completions 路由还会提供思考分派格式选择，且在 `chat-template` 下可添加自定义 `chat_template_kwargs` 字段，其值为字面量或随思考开关／档位变化；离开该格式会连同丢弃这些字段。取消勾选推理会移除声明而不是存入 `false`——从 catalog 模型上剥除推理仍是 `settings.yaml` 的编辑。适配器会拒绝的声明（没有 `off` 之外的档位、默认档位不在声明集内、自定义字段没有字段名）会在任何写入前失败，并按行位点名。空列表意味着「使用该路由的内置 catalog」，因此每一行都只会被刻意添加；清空容量会丢弃它，而不是存入一个 schema 会拒绝的值，配置留空的部分由适配器的路由级回退值定尺寸——留空的容量以这些回退值的量级作为占位符，那只是提示而非镜像：该字段按 1000 计 `K`，且部署可以覆盖这些回退值。不是正整数的容量根本不会被存下。
