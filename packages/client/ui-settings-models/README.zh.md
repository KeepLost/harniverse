...
声明步骤在 `src/onboarding-copy.ts` 中持有完整 Harniverse RC 文案和独立于 package 与 product version 的确认版本。回环访问会通过既有 settings API 比较并写入 `ui-onboarding.welcomeNoticeVersion`；只有明确点击「继续」才会记录当前版本。principal generation 变化会同步清空确认状态，即使写入仍在等待中也一样；前一 principal 的结算无法恢复新状态或为其发布失败。非回环浏览器无法使用这项仅限 Host 的 namespace，因此确认仅在当前进程有效，重载后声明会再次出现。
