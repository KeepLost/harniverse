...
每次带文本的 Queue、Steer 或 slash 未命中发送都会独占一个 `submitting` 事务，直到其 `SubmitOutcome` 结算，因此未决期间再次按 Enter 会被忽略。成功时只消费已提交前缀和捕获的图片 id，并保留快照之后追加的纯文本后缀；错误结果或 promise rejection 会保留草稿，并释放该席位以供重试。纯图片发送拥有独立的单事务生命周期，并在未决期间预留捕获的图片 id，因此后续文本发送只能提交未预留的图片。失败会释放预留，成功则只移除捕获的 id，而且仅在此后没有命令或 adjudication（判定）事务取得输入状态机所有权时，才消费捕获的空文本快照。释放输入壳会中止活跃的状态机事务和纯图片生命周期；同一信号会覆盖支持取消的引用序列化、浏览器图片编码与 Host 接纳操作，迟到的完成结果不会再发布任何状态。scope 释放会等待每个已保留的事务完成；任一引用序列化器失败时会中止其余序列化器，并等它们全部结算后才解锁草稿或允许重试。
