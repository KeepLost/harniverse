...
Resolution branch order (`import(specifier)`): platform seed word → shell instance; memoized record → surface; shell-own static registry (`registerStatic`, app-shell) → module; registered factory → materialize; graph row (`window.__DSH_BOOT__`) → load its external classic script + materialize; anything else throws — the runtime mirror of the build-time bundle purity gate. The synchronous `require` handed to factories walks the same order minus the asynchronous load branch and records observed edges into the module record. Initial boot calls `prefetchGraph` once to execute the revision-addressed bootstrap script and register every graph factory without materializing one; `prefetch(id)` retains independent bundle arrival for HMR and as the fallback when aggregate registration fails. `invalidate` drops one factory and materialized record so the next prefetch/import reloads that plugin.

The Node half scans enabled Loader entries for web `dsh.client` packages, resolves each `exports["./client"]`, hashes the built bundle into the boot graph, and serves it with its source map under `/plugins` only after Connection admits an `harniverse.observe` principal. It also caches one graph-revision bootstrap script: every bundle body remains a separate factory registration, per-bundle source-map comments are removed, and the route serves the aggregate gzip-compressed with immutable revision caching. Initial boot therefore requires one protected script admission instead of one admission per bundle and source map; individual resources and maps remain available for HMR and diagnostics. Source launch maps host imports to TypeScript source but still consumes this built client export; missing files share one build instruction followed by a package/path list, while unrelated filesystem errors remain separate failures.
