...
chip 以部署默认值打开。选择另一个 Profile 会通过 `session.create({ workspaceId, agentProfile })` 开启独立 Session；工作区复用空白 Session 时只接受 Profile 相同的 Session。暂存 id 会保持显示，直到新建 Session 成为当前项，随后下一次新会话流程重新以默认值打开。

chip 从不修改当前 Session，无论它空白还是已经开始。Profile 身份从 Session 创建起即不可变。
...
复制旁边是对话式入口：名单携带自指的 `cordis` Profile 时，一张虚线添加卡会用它开启独立 Session 并关闭设置面板。seat 会防止晚到的名单加载回退显示：暂存选择优先，其次是当前 Session 携带的 Profile，最后才是部署默认值。
