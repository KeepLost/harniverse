...
/** Injected business face of the resources tab (the governor Remote verbs). */
export interface GovernorResourcesActions {
...
export type GovernorResourcesTabProps =
  PropsRuntime<'governor.center.tab'>
  & InjectFace<GovernorResourcesActions>
...
/**
 * The resources tab: one row per session with live metering, quota state,
 * breach badges, and inline quota negotiation, under the enforcement tier and
 * the global budget bar with host sentinels.
 * @param props - tab slot currency, the Remote verbs, and the translator.
 * @returns the resources surface.
 */
export function GovernorResourcesTab({ overview: fetchOverview, adjustQuota, pollMs, t }: GovernorResourcesTabProps) {
  ...
  return (
    <div className={css.tabBody}>
      <div className={css.tabToolbar}>
        {overview !== undefined ? <span className={css.tier}>{t(`tier.${overview.tier}` as const)}</span> : <span />}
