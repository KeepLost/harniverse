...
The row follows the shared authorization-aware Settings mirror. Every mutation validates the Host-attached principal identity before accepting the redacted namespace response; a principal generation change synchronously clears the permission value, options, writability, revision, and saving state even while the write is pending. A rejected settlement publishes neither the selected value nor an error from the prior principal.

The current-session surface remains a popupSelect DECORATION hung on the host `/permission` command (`ctx.commandUi.decorate`). A decoration is not a second command — the host command keeps its slash-menu row, the argued path (`/permission <preset>` switches directly), and the durable lifecycle logging; the decoration replaces only the bare invocation with the picker: one flat preset list with the current value marked active and built-in presets under their locale product names (`workspace-write` → `工作区内修改` in Chinese, the composer chip's display transform twin) while host-configured presets keep their own names, where a pick submits the `/permission <preset>` command line. Options and the active mark read the session's `permissions` projection (the same host-computed select the composer chip renders), so both current-session surfaces share one read source and one write path, and the pushed projection frame is the single confirmation both follow. The decoration is available exactly while the projection key is present; a permission-less composition shows neither picker nor Settings row.
