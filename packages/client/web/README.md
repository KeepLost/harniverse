...
Both authentication documents render the non-transparent Harniverse artwork from `/whale-logo.svg`; the post-authentication loading page and new-session hero use `/whale-logo-light.svg`, and `/whale-logo-transparent.svg` ships for the future dark theme. The sidebar brand and browser tab use `/whale-logo.ico`. These assets live in `apps/web/public/`, and the built web host serves the same root URLs before authenticated plugin loading.
