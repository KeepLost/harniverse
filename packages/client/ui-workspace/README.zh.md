...
共享 Workspace 浏览器、选择器与只读工作台插件。`WorkspaceBrowser` 填充侧边栏的 `sidebar.workspaces` slot，`WorkspacePicker` 填充页面局部 Session Intent 主视觉区的 `conversation.hero.workspace` slot，`WorkspaceWorkbench` 则填充外壳根作用域的 `workbench` slot。
...
Session 页头通过与相邻 Session-log 控件一致的胶囊操作打开 `WorkspaceWorkbench`；blank 会话的页头被隐藏时，Hero 上 Workspace 胶囊的文件夹／名称部分打开同一个工作台，箭头则保留现有选择器。工作台占用 `ui-layout` 的共用右侧区域并与会话详情互斥，文件、变更和搜索作为三个同级页签位于顶部——贡献区段（浏览器与终端面板）通过 `workbench.section.tab`/`workbench.section.panel` 槽在旁边注册标签与主体，选中的区段记录在 layout store 中，`ctx.layout.openWorkbenchSection(section, request?)` 可以直达某个区段。区段的渲染不依赖已解析的 Workspace；文件/变更/搜索的主体及其标签则需要。根作用域工作台 store 按 Workspace id 保存彼此独立的内存记账，因此同一 Workspace 的多个 Session 会共享文档标签、目录展开、文件名搜索、glob 字段和 Git 导航，而不同 Workspace 不会继承这些状态。搜索接受逗号分隔的包含与排除 glob：`*.py` 这类不含分隔符的模式匹配任意深度的 basename，`src/**` 这类路径模式匹配相对路径，`dist/` 这类尾斜杠模式排除整个目录子树。外壳只持久化各 Workspace 的右侧模式、打开状态以及彼此独立的 details／workbench 宽度；文件内容、二进制数据、Git 结果和搜索结果都不进入浏览器持久化。根目录会自动加载，嵌套目录按需加载；切换 Workspace 会中止在途读取并推进 generation fence，迟到结果无法发布；同一资源的新请求也会取代前一个请求。经过认证的 `workspace.files.*` API 提供上述有界递归搜索、1 MiB UTF-8 文本前缀，以及最大 8 MiB 的完整白名单图片／PDF 数据；它不提供公开文件 URL，也不跟随符号链接。Git 状态、暂存区或工作区 diff 与最近提交均保持只读。打开文件会从左侧显示独立且由焦点管理的非模态预览：工作台停靠时，预览占用 `shell.overlay` 并覆盖对话直至工作台边缘；drawer 模式下，由于 overlay 层为 inert，预览会切换为占满整个工作台。关闭预览会在先前焦点目标仍可操作时恢复焦点并保留文档标签；关闭或切换工作台会清除预览可见性并保留这些标签，关闭最后一个标签则会收起预览。标签页可渲染 Markdown、沙箱 HTML、高亮代码、折行文本、有界 CSV 表格、统一 diff、图片 object URL 和沙箱 PDF frame；标签变化或关闭时会撤销 object URL。确定性客户端 fixture 实现这些只读界面，Grant 认证浏览器 E2E 则通过 ARIA golden 固定桌面交互与模态移动 drawer。
...
浏览器、选择器、工作台、预览 overlay 和 Session 页头目标 slot 均由其他插件声明，因此 `apply` 使用 `slots.inject()` 在各自的声明生命周期内完成注册，并在目标 slot 的声明恢复后重新注册。工作台与预览注册共享一个在 `apply` 内创建的 store handle；组件不会跨包读取 layout store。`layout` 是显式服务依赖，因为工作台打开与关闭不能静默退化为无操作。
...
- **归档 Session 只读**：浏览器提供归档列表、分页消息预览、取消归档，以及单个或批量永久删除；删除 Session 会为全局垃圾回收保留共享附件。
...
- **Office 文档预览暂缓**：工作台不渲染 `.docx`、`.xlsx` 或 `.pptx`；用户可以看到这些文件，但需要外部应用读取其内容。
