...
Session 页头会在 `ui-layout` 的共用右侧区域打开 `WorkspaceWorkbench`，并与会话详情互斥。根作用域工作台 store 按 Workspace id 保存彼此独立的内存记账，因此同一 Workspace 的多个 Session 会共享标签、目录展开、文件名搜索和 Git 导航，而不同 Workspace 不会继承这些状态。外壳只持久化各 Workspace 的右侧模式、打开状态以及彼此独立的 details／workbench 宽度；文件内容、二进制数据、Git 结果和搜索结果都不进入浏览器持久化。根目录会自动加载，嵌套目录按需加载；切换 Workspace 会中止在途读取并推进 generation fence，迟到结果无法发布；同一资源的新请求也会取代前一个请求。经过认证的 `workspace.files.*` API 提供有界递归文件名搜索、1 MiB UTF-8 文本前缀，以及最大 8 MiB 的完整白名单图片／PDF 数据；它不提供公开文件 URL，也不跟随符号链接。Git 状态、暂存区或工作区 diff 与最近提交均保持只读。标签页可渲染 Markdown、沙箱 HTML、高亮代码、折行文本、有界 CSV 表格、统一 diff、图片 object URL 和沙箱 PDF frame；标签变化或关闭时会撤销 object URL。确定性客户端 fixture 实现同一组只读界面，Grant 认证浏览器 E2E 则通过 ARIA golden 固定桌面交互与模态移动 drawer。
...
浏览器、选择器、工作台和 Session 页头目标 slot 均由其他插件声明，因此 `apply` 使用 `slots.inject()` 在各自的声明生命周期内完成注册，并在目标 slot 的声明恢复后重新注册。`layout` 是显式服务依赖，因为工作台打开与关闭不能静默退化为无操作。
