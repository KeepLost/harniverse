...
async function searchOnce(ctx: Context): Promise<{ url: string; body: Record<string, unknown> }> {
  const fetchSpy = vi.spyOn(globalThis, 'fetch')
    .mockImplementation(() => Promise.resolve(jsonResponse({ citations: [] })))
  fetchSpy.mockClear()
...
describe('web-search-perplexity settings section', () => {
  it('serves stored endpoint and request options to the next search without re-registering', async () => {
    ...
    expect(await searchOnce(bench.ctx)).toMatchObject({
      url: 'https://search.entry.test/chat/completions',
      body: { model: 'sonar-entry', max_tokens: 128 },
    })
    ...
    expect(await searchOnce(bench.ctx)).toMatchObject({
      url: 'https://search.stored.test/chat/completions',
      body: { model: 'sonar-stored', max_tokens: 256, search_recency_filter: 'month' },
    })
  ...
  it('falls back to the composition entry when the settings provider detaches', async () => {
    ...
    expect((await searchOnce(bench.ctx)).url).toContain('https://search.stored.test')

    await bench.settingsFiber.dispose()

    expect((await searchOnce(bench.ctx)).url).toContain('https://search.entry.test')
    await bench.ctx.fiber.dispose()
  })
