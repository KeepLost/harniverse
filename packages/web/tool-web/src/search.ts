...
/** Protocol maximum for the configurable query count. */
export const WEB_SEARCH_MAX_QUERIES_MAX = 16
 ...
 * @param maxQueries - the deployment's upper bound on accepted queries.
 ...
 *
 * @param args - the raw tool arguments; the query texts feed the view.
...
function mergeSearchResults(queries: string[], results: WebSearchResult[], maxResults: number): WebSearchResult {
  const seen = new Set<string>()
  const sources: WebSearchSource[] = []
  let sourceRanks = 0
  for (const result of results) {
    if (result.sources.length > sourceRanks) sourceRanks = result.sources.length
  }
  ...
  const contents = results.flatMap((result, index) => result.content === undefined || result.content.length === 0
    ? []
    : [`### ${queries[index]}
