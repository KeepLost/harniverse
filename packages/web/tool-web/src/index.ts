...
import { applyWebSearchTool, WEB_SEARCH_MAX_QUERIES, WEB_SEARCH_MAX_QUERIES_MAX, WEB_SEARCH_MAX_RESULTS } from './search.ts'
import { applyWebFetchTool } from './fetch.ts'

export { WEB_SEARCH_MAX_QUERIES, WEB_SEARCH_MAX_QUERIES_MAX, WEB_SEARCH_MAX_RESULTS, applyWebSearchTool, formatSearchOutput, presentSearchCall, presentSearchResult, searchMetaFromValue, searchMetaFromResult } from './search.ts'
...
/** Validate the query count against the fixed tool protocol maximum. */
function assertSearchMaxQueries(value: number): void {
  assertPositiveInteger('searchMaxQueries', value)
  if (value > WEB_SEARCH_MAX_QUERIES_MAX) {
    throw new Error(`tool-web: searchMaxQueries must not exceed ${WEB_SEARCH_MAX_QUERIES_MAX}`)
  }
}
...
export function apply(ctx: Context, config: Config): void {
  ...
  assertSearchMaxQueries(resolved.searchMaxQueries)
