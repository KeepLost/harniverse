...
describe('tool-web execution through the real registry', () => {
  ...
  it('executes multiple queries concurrently, deduplicates exact URLs, and labels answers', async () => {
    const seen: string[] = []
    ...
    const firstResult = new Promise<WebSearchResult>((resolve) => {
      releaseFirst = () => {
        resolve({
          ...
          sources: [{ url: 'https://a.test' }, { url: 'https://shared.test' }],
        })
      }
    })
    const provider: WebSearchProvider = {
      id: 'stub-search', available: () => available,
      search: (request) => {
        seen.push(request.query)
        return request.query === 'one' ? firstResult : Promise.resolve({
          content: 'answer two', truncated: false,
          sources: [{ url: 'https://b.test' }, { url: 'https://shared.test' }],
        })
      },
    }
    ...
    expect(out.value).toEqual({
      content: '### one\n\nanswer one\n\n### two\n\nanswer two',
      sources: [{ url: 'https://a.test' }, { url: 'https://b.test' }, { url: 'https://shared.test' }],
      truncated: false,
    })
    await fiber.dispose()
  })
  ...
  it('continues round-robin merging after a shorter result is exhausted', async () => {
    ...
    expect(out.value).toMatchObject({ sources: [{ url: 'https://a.test' }, { url: 'https://b.test' }, { url: 'https://c.test' }] })
    await fiber.dispose()
  })

  it('caps the combined multi-query result to searchMaxResults', async () => {
    const provider: WebSearchProvider = {
      id: 'stub-search', available: () => available,
    ...
    const out = await call('web_search', { queries: ['one', 'two'] })
    expect(out.value).toEqual({ sources: [{ url: 'https://a.test' }, { url: 'https://c.test' }], truncated: true })
    await fiber.dispose()
  })

  it('aborts sibling searches and waits for them to settle before reporting failure', async () => {
    let siblingAborted = false
    let releaseSibling: (() => void) | undefined
    const provider: WebSearchProvider = {
      id: 'stub-search', available: () => available,
      search: (request, signal) => request.query === 'one'
        ? Promise.reject(new Error('first search failed'))
        : new Promise((_resolve, reject) => {
          releaseSibling = () => {
            reject(new Error('sibling search stopped'))
          }
          signal?.addEventListener('abort', () => { siblingAborted = true }, { once: true })
        }),
    }
    const { fiber, call } = await mountTools({ webConfig: { searchProvider: 'stub-search' }, search: provider })
  ...
  it('aborts every fused signal on caller cancellation and waits for all settlements', async () => {
    const signals: AbortSignal[] = []
    const settle: (() => void)[] = []
    const provider: WebSearchProvider = {
      id: 'stub-search', available: () => available,
      search: (_request, signal) => new Promise((_resolve, reject) => {
        if (signal === undefined) throw new Error('missing search signal')
        signals.push(signal)
    ...
    const pending = ctx.tools.execute({ callId: CallId('search-cancel'), name: 'web_search', arguments: { queries: ['one', 'two'] }, signal: controller.signal })
    await vi.waitFor(() => {
    ...
    void pending.then(() => { operationSettled = true })
    settle[0]?.()
    await Promise.resolve()
  ...
  it.each([{}, { query: 'legacy-query' }, { queries: [123] }])('rejects absent, legacy, or wrongly typed queries with a structured INVALID_ARGS error', async (args) => {
  ...
  it('exposes only required queries in the model-visible schema', async () => {
    const { fiber, ctx } = await mountTools()
...
describe('searchMaxQueries is plugin config', () => {
  it('exposes and enforces the configured query cap before provider calls', async () => {
    const seen: string[] = []
    const provider: WebSearchProvider = {
      id: 'stub-search', available: () => available,
      search: (request) => { seen.push(request.query); return Promise.resolve({ sources: [], truncated: false }) },
    }
    const { fiber, ctx, call } = await mountTools({
      config: { searchMaxQueries: 2 }, webConfig: { searchProvider: 'stub-search' }, search: provider,
    })
    expect(ctx.tools.schemas().find(item => item.name === 'web_search')?.description).toContain('1–2 queries')
  ...
  it.each([0, -1, 1.5])('rejects invalid searchMaxQueries value %s at load', async (value) => {
    const ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
    await ctx.plugin(WebRuntime, {})
  ...
  it('rejects searchMaxQueries above the protocol maximum at load', async () => {
    const ctx = new Context()
    await ctx.plugin(SystemPrompt)
    await ctx.plugin(ToolRuntime)
    await ctx.plugin(WebRuntime, {})
    await expect(ctx.plugin(ToolWeb, { searchMaxQueries: WEB_SEARCH_MAX_QUERIES_MAX + 1 }))
      .rejects.toThrow(new RegExp(`tool-web: searchMaxQueries must not exceed ${WEB_SEARCH_MAX_QUERIES_MAX}`))
  })
})
