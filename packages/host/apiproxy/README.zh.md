...
已认证载体只增加稳定、非机密的 principal 身份。每个 unary `ServerResponse` 与回答 receipt 都携带 `authentication`；每条流先发送 `connection.authenticated`，载体客户端会在领域帧解析前截获它。两者都只使用 `{ kind: 'bypass' }` 或 `{ kind: 'grant', grantId, grantRevision }`，省略名称、capability、过期时间及所有凭据材料。客户端载体为每次 unary 调用捕获发起身份，在发布结算前要求返回身份一致；元数据缺失或不匹配时会同步撤回当前连接 generation。每个有 mutation 的 unary 请求还会把发起时的身份作为 `expectedPrincipal` 携带：普通 `ClientRequest` 写入和 `ClientResponse` 回答路径都适用。即使 `llm.discoverModels` 本身不存储内容，它仍因携带 secret 而走这条 principal-bound 路径。Host 载体会在解析业务载荷、分发路由、认领待处理交互或调用提供方网络之前，将该前置条件与刚完成认证的 principal 比较，身份已变化时返回类型化的不匹配结果。
...
`events.mux({since})` 把每个值解释为该客户端已应用的最后一个连续持久事件 seq。Host 会先安装实时观察，再采样每个已附加普通 Session 的回放切点，并惰性产出 `session/subscribed` 与 `cursor < seq <= cut`，使启动历史不会填满有界实时队列，最后排空实时缓冲并丢弃重叠项；临时问题、审批、队列、任务和投影仍走各自的快照或实时路径。subagent Session 仍不进入这条全会话基线。`subagent.prompt` 经直接父级授权并成功后，只会向同一个已认证 principal 的现有 mux 流授予已寻址 child 订阅：每条流先收到 subscribed 标记，再回放内存前缀以封闭冷历史到实时状态之间的竞态，随后接收 child 实时事件。重新连接时，只有该 principal 先前取得同一个已寻址授权，Host 才接受对应 child 游标。SSE 与 WebSocket 载体在 mux URL 中编码可选游标表，并在开始流传输前拒绝无效值。`streamQueueMaxFrames` 是每条 Host 流队列的正整数帧数上限（默认 `1024`）；溢出时先排空已接纳帧，再使流失败，让下一连接 generation 从已经推进的游标继续，而不是静默丢帧。
...
`session.prompt` 返回确切获准消息的 `MessageId`；斜杠命令使用独立的 command API，绝不会进入该方法。`session.workStatus({sessionId, messageId})` 是直接针对单条消息的生命周期查询：它从持久 inbox 与轮次记录折叠出 `unknown`、`queued`、`claimed`、`discarded` 或 `settled`，并在可用时为每个已知状态携带实际生效的 `delivery`（`queue` 或 `steer`）。claimed/settled 值还携带轮次，settled 还携带持久 `turn/end` 原因。调用方不需要读取整个 Session 历史。这是工作生命周期关联，不是 assistant 输出归因：多条消息可以共用一个轮次，API 绝不会把某条 assistant 消息命名为某次 prompt 的结果。
...
待处理的 queued 输入属于实时控制平面约定，而非对话历史。网关根据持久 `agent/inbox/spliced` 变更派生完整的 `next-turn` 队列，并在每次变更后及重连时广播权威 `session/queue` 快照；待处理的 `next-step` steering（中途引导）不进入此 Web 投影。在 `next-step` 内，用户来源的消息携带 `steering` placement，而注入上下文（审批通知、任务完成、附加快照）携带 `context`，领取前不对外呈现。面向单条消息的 `agent/inbox/inserted`、`claimed` 与 `discarded` 通知仍供生命周期观察方使用，但不用于构建队列视图。`session.updateQueue` 通过 `MessageId` 寻址单个项；编辑、移除和 steering 经已挂载 Agent 的 `Inbox.splice()` 修改队列，成功响应会直接携带该消息变更后的 `status` 和实际生效的 `delivery`。认领操作的纯删除 splice 会在 pre-step 准入前赢得竞态，因此之后的操作返回 `queue-item-not-found`。`session.cancel` 仅中止活动轮次并保留待处理 inbox 工作；取消达到完全停稳且结束中的轮次完成 flush 后，AgentLoop 按 FIFO 顺序认领下一条可唤醒消息，浏览器绝不重发或提升它。队列操作绝不恢复冷会话，客户端也绝不根据轮次或状态事件推断某项已退出队列。
...
只读 `workspace.files.*` 领域通过已注册的 Workspace id 与规范包含关系解析每个请求。列表返回一个目录层级；搜索递归匹配普通文件名且不跟随符号链接，并在扫描 20,000 个条目或得到 200 个结果时停止。每次搜索最多接受 20 个包含 glob 和 20 个排除 glob，每个模式最多 200 个字符。不含分隔符的模式匹配任意深度的 basename，包含 `/` 的模式匹配完整相对路径，尾斜杠覆盖目录子树；`*`、`?`、`**`、花括号备选与字符类均不区分大小写。排除列表缺失或为空时，默认跳过依赖、缓存和构建输出目录；非空调用方列表会取代默认项，从而允许有意搜索通常跳过的子树。候选文件使用 no-follow、非阻塞 descriptor，因此特殊文件无法在普通文件验证前造成阻塞。文本读取返回至多 1 MiB 的有效 UTF-8 前缀。`readBinary` 只接受白名单图片和 PDF 扩展名，并且只在完整普通文件不超过 8 MiB 时将其作为 base64 返回；不支持与超限文件使用不同的结构化错误。这些方法要求 `harniverse.observe`、分类为 read，且不暴露公开文件服务路由。配套的 `workspace.git.*` 方法运行只读状态、提交历史和 diff 命令，并停用 prompt、pager、仓库 fsmonitor、外部 diff driver、textconv 和 optional lock；子进程只接收清理后的环境，不读取系统或全局 Git 配置，且每项操作有 10 秒 deadline。仓库校验要求工作树根和规范 Git 元数据都位于已注册的 Workspace 根内；deadline 与包含关系失败都保持为结构化 `workspace-git-failed` 响应。
...
`agentPreset.list` 领域向浏览器暴露部署的 Agent Profile 名单，供其选择传给 `session.create` 的 Profile；每一行携带 `trust`、默认状态、可选 `permissionPreset`，以及 Profile 无法组装 Session 时的 `broken` 原因。`session.create({ workspaceId, agentProfile })` 把解析后的 id 记录为不可变的 `SessionHeader.agentProfile`，挂载 Profile 组装，并在发布 Agent 前应用默认权限。resume、fork、冷记录展示与本地投递都从该 header 重建。不同 Profile 必须创建新 Session；协议中没有 Profile 切换方法。

`agentPreset.read`、`copy`、`openDocument` 与 `remove` 管理 Profile 定义本身。`read` 返回组装文本与 `trust`。创作只有复制一种写入：`copy` 接收 `{ from, agentPreset, name? }` 并整目录复制来源，包括默认权限；组装文本不经过协议。`openDocument` 把 id 解析为本地创作 Profile 的目录，`remove` 拒绝随附 Profile。这四个方法在 [`dsh-client-connection`](../../client/connection/README.md) 中固定为仅回环可用；`list` 保持普通方法，并报告不含路径的 `authorable` 与 `hasDocument` 能力标志。

`command.*` 与 `skill.*` 领域向客户端暴露宿主命令注册表和 skill 目录。每个方法都通过 `sessionId` 寻址一个 Session 的 Agent；`command.*` 经由与 `session.*` 相同的路径恢复冷 Session，而 `skill.list` 从 Session header 解析项目根目录与不可变 Agent Profile，不触碰 Agent 注册表。`skill.list` 返回每个用户可调用的 skill 及其 `modelInvocable` 标志。调用本身是普通 `session.prompt`，`dsh-tool-skill` 会在 pre-step 边界识别以空白为界的 `/name` token，因此 Web、TUI 与 ACP（Agent Client Protocol）共享同一路径。`command.execute` 报告准入结果与生命周期 `commandId`，结局由持久化的 `command/run`/`command/done` 事件对承载。`commands/change` 是注册表级失效信号，而不可变 Profile 身份让 Session 的组装派生目录保持稳定。

`settings.*`、`credentials.*` 与 `llm.*` 领域是配置页协议。settings 领域服务于已注册可配置提供方所指向的 namespace（`ctx.llm.listConfigurableProviders()`），并额外服务于一份小型、显式的 allowlist——Web 偏好 `locale`、`permission`、`ui-conversation` 与 `ui-theme`、插件配置页所编辑的宿主平面插件分节（`agent-loop`、`bash`、Web 选择器 `web`，以及其提供方分节 `web-search-deepseek`、`web-search-exa`、`web-search-perplexity`、`web-search-tavily`、`web-search-brave`、`web-search-kagi` 与 `web-firecrawl`），以及产品持有的 `ui-onboarding`；仅新增一项 Settings 注册，绝不会使其可被远程读取或写入。其他任何 namespace 都只会得到 `settings-not-exposed`——未注册的 namespace 得到的是同一个答复，因此没有调用方能靠逐个探测把注册表枚举出来。`settings.describe` 为每个已暴露 namespace 提供其序列化 schemastery schema、脱敏后的分层值（resolved/`base`/`user`——字段出现在 `user` 中即标记其被用户覆盖）、`secrets` 槽位列表、该分节的 `revision`，以及布尔型 `hasDocument` 能力标志。浏览器不会收到 Host 路径：无路径参数的 `settings.openDocument` 会请求提供方准备文档，再把由 Host 解析出的结果交给原生打开器，因此任何浏览器载荷都无法选择任意文件系统目标。`settings.update`/`settings.replace` 写入用户层；`settings.mutate` 则在已存分节上施加路径 op（`set`/`unset`），这是持有脱敏视图的客户端的删除路径——据此重建分节再整体替换，会删掉协议从未回传过的那些机密。任何写入都可携带 `expectedRevision`；陈旧的期望值会以 `settings-conflict` 连同两个 revision 作答，而不是覆盖先落地的那个写方，其余每种 seam 拒绝则折叠为 `settings-rejected`。secret 角色的值绝不在任何一层搭乘任何响应；secret 只沿一个方向跨越协议——在 `update`/`mutate` 载荷或 `credentials.set` 之内。`credentials.describe` 返回不含值的视图（`configured`/`source`/`writable`），`credentials.set`/`credentials.unset` 则把被遮蔽引用的拒绝映射为 `credential-rejected`。`llm.providers` 把可配置提供方目录与存活路由合并（休眠条目携带 `active: false`；未声明的存活路由追加在后，不带 settings 地址），`llm.models` 则是与会话无关的目录。`llm.discoverModels` 询问页面尚在起草的提供方端点：`settingsNs` 选出懂得读取该列表的适配器家族，端点、协议与密钥则来自表单而非存储。它什么都不写——回复是候选，只有随后的 `settings.mutate` 才决定路由服务什么——因此其 `apiKey` 是 secret 可以搭乘的第三个载荷（另两个是 `settings.update`/`mutate` 与 `credentials.set`），且绝不被存储或回显。host 从不存储或回传它；与另两者一样，它确实会搭乘客户端的出站信封，`subscribeEnvelopes()` 的观察者能看到——为该 tap 做脱敏是整个配置面的改动，而非本方法一家的事。每一种拒绝（无人服务的 namespace、没有可读列表的协议、不可达端点、被拒凭据）都折叠为 `model-discovery-failed`，其消息是适配器自己的文本，details 点名被询问的端点，绝不点名所提供的凭据。失效通知让每个面无需轮询即保持收敛。`settings/document-updated` 与 `credentials/updated` 搭乘原样转发事件帧（见下），因此解析值未变的原始设置变更同样能到达客户端，凭据失效通知也仍然只带引用名、绝不带值。`llm/adapters-updated` 与 `settings/document-updated` 一并原样转发；具体模型消费方直接订阅这两个 owner 事件，因为拓扑提交和设置文档都能独立改变其目录。浏览器载体把整个配置面（含读取与原生操作：`settings.describe`/`openDocument`/`update`/`replace`/`mutate` 与 `credentials.describe`/`set`/`unset`）限制为仅接受来自回环地址的同源请求——即 `host.pickDirectory` 所在的特权集合。未装 settings 或凭据提供方的组合会以指名缺失插件、包含解决建议的 `internal` 错误应答这些领域。

Host 会把每一种可能改变已授权 Settings namespace 集合的已提交来源投影为 `settings/exposure-changed`：Settings namespace 注册／注销、`llm/adapters-updated` 与权威 `capabilities/change`。capability settings 写入由 capability 服务自己的事件表示，而不再从 settings 文档重复推断，因此一次提交不会产生重复的 exposure revision。Agent Profile 名单与组装变化不改变 Settings descriptor，继续作为独立转发的 `agent-presets/change` 事件。
