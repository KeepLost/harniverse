...
由服务端 preset 目录定义 Agent Profile。每个 **Profile** 目录包含一份 `agent.cordis.yml` 和可选的 `preset.yml` 元数据；roster 在整个进程内只挂载一次其组装，使用该 Profile 创建的每个 Session 都通过 scope 父链加入。挂载的工具、提示词段落与投影单元只存在一份，而每个 Session 都在持久化 header 中保有不可变的 Profile 身份。
...
服务会在复制或删除后发出受包含保护的 `agent-presets/change`；当 roster 读取发现行、元数据字段、健康结果或组装文件 stamp 变化时，也会发出该事件。这是权威的 Profile 拓扑信号：观察方据此重新读取自己的投影，且任何观察方失败都不能否决发现或创作。
...
- `ctx.agentPresets.capabilityRecipes(id?): Promise<CapabilityDescriptor[]>` 直接从健康 Profile YAML 文件读取部署级顶层插件配方集合，包括声明的内置 Tool 成员、PTC presentation 的 `run_code` member 与 Persona 的 Profile 安全配置契约。传入 Profile 时使用该文件的原生选择、成员与配置默认值；省略 Profile 时表示显式的全局 override，不会把任意 Profile 的原生行合并为全局默认值；读取配方不会挂载任何插件。
...
- **代际以组装和有效能力组装为键** —— stamp 察觉 `agent.cordis.yml`，签名察觉全局/Profile 选择、可见成员和配置变化；组装旁的 Skill 文件或资产编辑仍需组装文件变化或进程重启才能到达新会话。
