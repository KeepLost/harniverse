...
- `ctx.agentPresets.capabilityRecipes(id?): Promise<CapabilityDescriptor[]>` The deployment-wide top-level plugin recipe set read directly from healthy Profile YAML files, including declared built-in Tool members, the `run_code` member of PTC presentation, and Persona's Profile-safe configuration contract. Supplying a Profile selects that file's native selection, member, and configuration defaults; omitting it describes explicit global overrides without unioning any Profile's native rows; reading recipes mounts nothing.
