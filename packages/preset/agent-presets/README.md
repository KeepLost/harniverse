...
- `ctx.agentPresets.capabilityRecipes(id?): Promise<CapabilityDescriptor[]>` The deployment-wide top-level plugin recipe set read directly from healthy Profile YAML files, including declared built-in Tool members, the `run_code` member of Code Mode presentation, and Persona's Profile-safe configuration contract. Supplying a Profile selects that file's native selection, member, and configuration defaults; omitting it describes explicit global overrides without unioning any Profile's native rows; reading recipes mounts nothing.
- `ctx.agentPresets.compositionRuntime(agentCtx)` The immutable generation id and assembly result captured for one live Agent, used by the Session Capabilities view.
...
The agent factory's `setup(agentCtx)` hook is the one supported call site. Only there is the join installed while the agent is still unpublished, so a rejected composition rolls the whole creation back rather than leaving a half-composed session. The standing subtree is owned by the roster service's own fiber — deliberately its UNTRACED context, because a subtree minted from a traced `this.ctx` resolves every service through the caller's shadow fiber instead of each entry's own inject store — so it survives every agent and unwinds only with the whole tree. Each generation records its composition file stamp plus its effective selection, visible-member, and configuration signature. Before mounting, the roster compiles changed selections and owner-declared configuration into complete `Include` patches that disable source rows, insert canonical rows, configure Persona, or activate config-gated members; native adapters install the remaining scoped member restrictions after mount. A later session that finds either identity stale starts the next generation, while every session already joined keeps the one it runs on.

Provider-owned private configuration captured by capability adapters also participates in generation identity. The roster installs those captures before mounting consumers and retries catalog discovery when configuration changes during assembly. Existing generations retain their captures; MCP credentials and process settings are never serialized into the public signature.
...
- **A generation is keyed on composition plus effective capability assembly** — the stamp notices `agent.cordis.yml` and the signature notices global/Profile selection, visible-member, and configuration changes, but an edit to a skill file or asset beside the composition still requires the composition file to move or the process to restart.
