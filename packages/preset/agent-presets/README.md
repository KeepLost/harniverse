...
- `ctx.agentPresets.capabilityRecipes(id?): Promise<CapabilityDescriptor[]>` The deployment-wide top-level plugin recipe set read directly from healthy Profile YAML files, including declared built-in Tool members and Persona's Profile-safe configuration contract. Supplying a Profile selects that file's native selection, member, and configuration defaults; reading recipes mounts nothing.
