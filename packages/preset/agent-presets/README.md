...
- `ctx.agentPresets.capabilityRecipes(id?): Promise<CapabilityDescriptor[]>` The deployment-wide top-level plugin recipe set read directly from healthy Profile YAML files. Supplying a Profile selects that file's native loaded defaults; reading recipes mounts nothing.
...
- **A generation is keyed on composition plus capability selection** — the stamp notices `agent.cordis.yml` and the selection signature notices global/Profile load/unload changes, but an edit to a skill file or asset beside the composition still requires the composition file to move or the process to restart.
