...
**`PtcRuntime`**（`ctx.ptcRuntime`）定义 PTC 运行时做什么，即针对宿主提供的一组异步绑定运行一段模型编写的程序，并报告 `{ value, logs, error? }`，而不规定如何实现。

此包承担该能力的 Service Definition 角色（以 bash 三包结构为模板，参见[能力 seam](../../../.agents/notes/implemented/architecture/2026-06-13-capability-seams.md)）：提供方通过继承 `PtcRuntime` 并注册服务接入；Consumer 是工具注册表的 PTC，它生成面向模型的 SDK，并桥接工具分发。这两项职责均由 [PTC Agent Note](../../../.agents/notes/implemented/feature/2026-06-15-code-mode.md) 规定。[Node 进程提供方](../ptc-runtime-node/README.md)运行 TypeScript，显式选择的 [Python 进程提供方](../ptc-runtime-python/README.md)运行 Python。运行时不了解工具或会话：调用方只向它提供具名异步函数与程序字符串；所有与工具有关的内容都留在 Consumer。

## 服务 API（`ctx.ptcRuntime`）
...
通过 `dsh-tools` 中的 PTC 间接提供；后者公开 `run_code`，并将程序日志、值或失败作为保留的工具结果 token 返回。
...
- **持久 REPL 风格内核已记录为未来工作**：在持久内核后端带来自己的日志方案前，运行之间不保留状态的约定继续有效（参见 [PTC Agent Note](../../../.agents/notes/implemented/feature/2026-06-15-code-mode.md)）。
- **目前不提供强安全边界后端**：Node 与 Python 进程提供方都属于同宿主约束；`'container'` 仍是已声明但没有实现的 `isolation` 值。
