...
Fresh-process PTC implementation of the [`@deepseek-ai/dsh-ptc-runtime`](../ptc-runtime/README.md) seam: `NodePtcRuntime` runs each program in ONE fresh Node child process over the shared control channel ([`dsh-control-channel`](../../subprocess/control-channel/README.md)) — TypeScript in, type-stripped host-side, bindings bridged as control calls, `{ value, logs, error? }` out. **Containment, not a security boundary**: trust posture is bash-equivalent by design (the [PTC Agent Note](../../../.agents/notes/implemented/feature/2026-06-15-code-mode.md) § Trust posture), with containment bash does not have — separate process, no inherited host environment, sandbox parity with the deployment's Bash policy, heap cap, deadline with forced kill.
...
Source mode loads erasable-only `src/child.ts` through Node's native type stripping; its transitive closure is Node built-ins, relative source modules, and the shared `dsh-control-channel` contract (resolved through workspace exports with a source fallback), so it does not require this package's own `lib/`. Built mode loads the sibling CommonJS bundle `lib/child.cjs`. The repository-wide requirement to exercise this published entry belongs to the [testing policy](../../../docs/testing.md).

| Host and entry | Child argv | Bootstrap environment |
| --- | --- | --- |
| Node, source or built | Node, heap flag, `src/child.ts` or `lib/child.cjs` | Empty |
| Electron self executable, source or built | Electron, heap flag, the same entry | `ELECTRON_RUN_AS_NODE=1` |
| Separate configured Node | Configured Node, heap flag, selected entry | Empty |
| Single-file executable (yao-pkg/pkg), no explicit entry | Self executable; its bin routes to the bundled child | `DSH_PTC_RUNTIME_NODE=1`, heap flag in `NODE_OPTIONS` |
| Packaged host, explicit Node and entry | Configured Node, heap flag, `bootstrapPath` | Empty |

Electron packaging must leave the `runAsNode` fuse enabled for self execution; a deployment with that fuse disabled must configure a separate Node executable and child entry. The child removes `ELECTRON_RUN_AS_NODE`, `DSH_PTC_RUNTIME_NODE`, and `NODE_OPTIONS` before accepting a program. This does not undo the heap cap already applied at startup. Operating systems can still add their mandatory environment keys.

A host binding may invoke `ctx.ptcRuntime.run()` again while the outer program awaits it. Each invocation gets its own child, control channel, budgets, and output ledger; bootstrap selection repeats from the host's executable. The `run_code` tool itself is excluded from the generated tool SDK. Processes spawned directly by model code inherit its cleaned environment; an Electron executable therefore requires an explicit Node-mode switch if model code chooses to spawn it.

The SDK API is the default/named `NodePtcRuntime` class plus `Config`. The operational `./child` subpath exists only as the packaged spawn entry; the frame mapping and executor helpers are source-private implementation details.

## Model Experience

Indirectly, through PTC in [`dsh-tools`](../../core/tools/README.md), which renders the exact outer value when it fits or an explicit `invalid-output` / `output-limit` failure. Only the outer `run_code` result enters model context and its ordinary spill policy; binding traffic and intermediate values remain execution-local.
