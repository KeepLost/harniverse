...
it('boots the source child without this package\'s outputs', async () => {
  ...
  try {
    ...
    const child = spawn(process.execPath, [join(directory, 'child.ts')], {
      stdio: ['pipe', 'pipe', 'ignore'],
      env: { ELECTRON_RUN_AS_NODE: '1', DSH_PTC_RUNTIME_NODE: '1', NODE_OPTIONS: '--max-old-space-size=128' },
    })
    ...
    channel.call('run', ['return { answer: 21 * 2, bootstrap: [process.env.ELECTRON_RUN_AS_NODE, process.env.DSH_PTC_RUNTIME_NODE, process.env.NODE_OPTIONS].map(value => value ?? null) }', [], 65_536, 60_000, 1_000_000]).catch(() => undefined)
    const outcome = await channel.outcome()
    expect(outcome.kind).toBe('value')
    expect(decodeWorkerJson(outcome.kind === 'value' ? outcome.value : undefined)).toEqual({ answer: 42, bootstrap: [null, null, null] })
