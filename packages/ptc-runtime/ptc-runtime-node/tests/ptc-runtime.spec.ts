...
import { childSpawnPlan, NodePtcRuntime } from '../src/index.ts'
import type { Config } from '../src/index.ts'
import type { CodeBindingFunction, CodeBindingNamespace, CodeRunResult } from '@deepseek-ai/dsh-ptc-runtime'
...
async function setup(config: Config = {}, sandbox: SandboxMode = 'raw-danger', workspaceRoot?: string) {
  ...
  const fiber = await ctx.plugin(NodePtcRuntime, config)
  const runtime = ctx.ptcRuntime as NodePtcRuntime
...
describe('NodePtcRuntime — programs and bindings (real children)', () => {
  ...
  it('lets a host binding run a nested program in a separate child with independent output', async () => {
    const { fiber, runtime } = await setup()
    try {
      const result = await runtime.run({
        program: 'console.log("outer"); return { pid: process.pid, nested: await tools.nested({}) }',
        bindings: tools({
          nested: async () => await runtime.run({
            program: 'console.log("inner"); return { pid: process.pid, bootstrap: [process.env.ELECTRON_RUN_AS_NODE, process.env.DSH_PTC_RUNTIME_NODE, process.env.NODE_OPTIONS].map(value => value ?? null) }',
            bindings: [],
          }),
        }),
      })
      expect(result.error).toBeUndefined()
      const value = result.value as unknown as { pid: number; nested: { logs: string[]; value: { pid: number; bootstrap: null[] } } }
      expect(result.logs).toEqual(['outer'])
      expect(value.nested).toEqual({ logs: ['inner'], value: { pid: expect.any(Number) as unknown, bootstrap: [null, null, null] } })
      expect(value.nested.value.pid).not.toBe(value.pid)
...
describe('NodePtcRuntime — budgets and containment (real children)', () => {
...
describe('NodePtcRuntime — hostile programs (real children)', () => {
...
describe('NodePtcRuntime — sandbox parity', () => {
...
describe('NodePtcRuntime — seam misuse and lifecycle', () => {
  ...
  it('rejects config values that are not positive numbers', async () => {
    ...
    await expect(ctx.plugin(NodePtcRuntime, { computeMs: -1 })).rejects.toThrow(/positive number/)
  })
  ...
  it('rejects a maxWallMs above Node\'s maximum timer delay', async () => {
    ...
    const ctx = new Context()
    await ctx.plugin(SandboxPolicyService, { mode: 'danger-full-access' })
    await expect(ctx.plugin(NodePtcRuntime, { maxWallMs: 2_147_483_648 }))
      .rejects.toThrow(/maxWallMs must be at most 2147483647/)
    // The boundary itself is usable.
    await expect(ctx.plugin(NodePtcRuntime, { maxWallMs: 2_147_483_647 })).resolves.toBeTruthy()
  })

  it('requires maxOutputBytes to fit the smallest counted outer payloads', async () => {
    const ctx = new Context()
    await ctx.plugin(SandboxPolicyService, { mode: 'danger-full-access' })
    await expect(ctx.plugin(NodePtcRuntime, { maxOutputBytes: 3 })).rejects.toThrow(/safe integer of at least 4/)
    await expect(ctx.plugin(NodePtcRuntime, { maxOutputBytes: 4.5 })).rejects.toThrow(/safe integer of at least 4/)
  })

  it('rejects an empty nodeExecutable and a relative bootstrapPath', async () => {
    const ctx = new Context()
    await ctx.plugin(SandboxPolicyService, { mode: 'danger-full-access' })
    await expect(ctx.plugin(NodePtcRuntime, { nodeExecutable: '' })).rejects.toThrow(/nodeExecutable must be non-empty/)
    await expect(ctx.plugin(NodePtcRuntime, { bootstrapPath: 'relative/child.cjs' })).rejects.toThrow(/bootstrapPath must be absolute/)
  })
  ...
  it('disposal aborts in-flight runs, awaits child exit, and rejects later runs', async () => {
    const ctx = new Context()
    await ctx.plugin(SandboxPolicyService, { mode: 'danger-full-access' })
    const fiber = await ctx.plugin(NodePtcRuntime)
    const runtime = ctx.ptcRuntime as NodePtcRuntime
  ...
  it('removes ctx.ptcRuntime when the providing fiber disposes (HMR safety)', async () => {
    const ctx = new Context()
    await ctx.plugin(SandboxPolicyService, { mode: 'danger-full-access' })
    const fiber = await ctx.plugin(NodePtcRuntime)
    expect(ctx.get('ptcRuntime')).toBeInstanceOf(NodePtcRuntime)
    await fiber.dispose()
