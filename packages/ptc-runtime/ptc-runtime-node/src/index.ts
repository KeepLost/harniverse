 ...
 * boundary: model code has bash-equivalent trust despite no inherited host environment, a
 ...
 * @module @deepseek-ai/dsh-ptc-runtime-node
 */
...
import type { CodeBindingNamespace, CodeJsonValue, CodeRunFailure, CodeRunRequest, CodeRunResult } from '@deepseek-ai/dsh-ptc-runtime'
 ...
 * @returns the argv to confine and spawn, plus a bootstrap-only environment:
 *   Electron self execution needs ELECTRON_RUN_AS_NODE; pkg self execution
 *   needs its routing variable and heap cap through NODE_OPTIONS. An explicit
 *   bootstrap bypasses pkg routing. No ambient environment is inherited.
 */
export function childSpawnPlan(
  ...
  if (!packaged || config.bootstrapPath !== undefined) {
    const electronSelf = process.versions.electron !== undefined && config.nodeExecutable === process.execPath
    return {
      argv: [config.nodeExecutable, heapFlag, config.bootstrapPath ?? CHILD_PATH],
      env: electronSelf ? { ELECTRON_RUN_AS_NODE: '1' } : {},
    }
  }
...
/**
 * The shipped {@link PtcRuntime} backend (`ctx.ptcRuntime`). Registers as
 * the `ptcRuntime` service; every cap comes from validated config. See the
...
export class NodePtcRuntime extends PtcRuntime {
  ...
  constructor(ctx: Context, config: Config) {
    ...
    for (const [key, value] of Object.entries(this.config)) {
      if (typeof value === 'number' && !(Number.isFinite(value) && value > 0)) {
        throw new Error(`dsh-ptc-runtime-node: config.${key} must be a positive number, got ${String(value)}`)
      }
    }
    if (this.config.nodeExecutable.length === 0) throw new Error('dsh-ptc-runtime-node: config.nodeExecutable must be non-empty')
    if (this.config.bootstrapPath !== undefined && !isAbsolute(this.config.bootstrapPath)) {
      throw new Error('dsh-ptc-runtime-node: config.bootstrapPath must be absolute')
    }
    if (!Number.isSafeInteger(this.config.maxOutputBytes) || this.config.maxOutputBytes < MIN_OUTPUT_BYTES) {
      throw new Error(`dsh-ptc-runtime-node: config.maxOutputBytes must be a safe integer of at least ${MIN_OUTPUT_BYTES}, got ${String(this.config.maxOutputBytes)}`)
    }
    ...
    if (this.config.maxWallMs > MAX_TIMER_DELAY_MS) {
      throw new Error(`dsh-ptc-runtime-node: config.maxWallMs must be at most ${MAX_TIMER_DELAY_MS} (Node clamps a longer setTimeout delay to 1ms), got ${String(this.config.maxWallMs)}`)
    }
    ctx.effect(() => () => { void this.teardown() }, 'Node PTC runtime teardown')
  }
  ...
  async run(request: CodeRunRequest): Promise<CodeRunResult> {
    if (this.disposed) throw new Error('dsh-ptc-runtime-node: run() after disposal')
  ...
  private async execute(
    ...
    // Bootstrap variables select Electron's Node mode or pkg's child route;
    // the child entry removes them before executing the model program.
    ...
    const finished = (async (): Promise<CodeRunResult> => {
      try {
        ...
        for (const note of cleanupNotes) this.ctx.logger.warn(`dsh-ptc-runtime-node: ${note}`)
