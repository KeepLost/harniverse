...
Fresh-CPython-process Service Provider for the [`@deepseek-ai/dsh-ptc-runtime`](../ptc-runtime/README.md) seam. It registers `ctx.ptcRuntime` with `language: 'python'` and `isolation: 'process'`; each `run()` starts one Python process, executes one async-function body, waits for process exit, and retains no state for the next run.

This provider is opt-in. No shipped Profile selects it. A deployment that already enables PTC replaces its `ptc-runtime` Cordis config entry with:

```yaml
- id: ptc-runtime
  name: '@deepseek-ai/dsh-ptc-runtime-python'
```

`dsh-tools` reads `ctx.ptcRuntime.language`, so the existing `run_code` Consumer projects its Python SDK and Python schema strings without a Python-specific tool or agent-loop branch.

## Configuration

| Key | Default | Contract |
|---|---:|---|
| `pythonExecutable` | `python3` | Non-empty executable name or path, resolved at plugin load to an executable regular file (bare names search the Host `PATH`). An isolated probe must report CPython 3.10 or newer, or plugin setup fails. |
...
Invalid config fails plugin setup, as does an executable that does not resolve to an executable file or fails the CPython version probe. An interpreter removed after load, interpreter exit, resource death, malformed oversized frame, program exception, timeout, abort, invalid completion, or output overflow resolves through `CodeRunResult.error`; invalid portable bindings, disposal misuse, and invalid config reject as Service Definition misuse.

## Process and protocol

Node resolves `pythonExecutable` once at plugin load, probes it as CPython 3.10 or newer under a five-second force-kill bound, and every run spawns that exact file with `['-I', '-B', py/bootstrap.py]`, `shell: false`, and only the Host `PATH` in the child environment. The bootstrap clears its environment before model code runs. Standard input carries Host-to-child control frames and fd 3 carries child-to-Host frames, with one versionless JSON object per line: Host sends `boot`, waits for `boot-ack`, sends `run`, services `call` frames with `reply`, and accepts one `done`. `src/protocol.ts` and `py/protocol.py` mirror every required and optional field; the real-`python3` mirror test checks both rosters.

The Host treats every child frame as hostile: it caps lines before parsing, rejects integer tokens that JavaScript would round, validates and rebuilds known frames field by field, ignores unknown/malformed frames and duplicate call ids, uses own-property binding lookup over a validation-time snapshot, bounds in-flight binding calls and unflushed reply frames at 1024 each (settling the run as a worker-exit when a binding never settles or the child stops consuming its replies), and revalidates completion values. Child errors and Host process failures expose bounded diagnostics without process paths, environment values, stacks, or raw protocol payloads.
...
### Python PTC selection

#### What the model sees

PTC in [`dsh-tools`](../../core/tools/README.md) selects its existing Python SDK and `run_code` wording from this provider's `language` descriptor, then renders bounded logs, a lossless JSON completion, or a sanitized failure into the retained tool result.

#### Token effect

Selecting this provider replaces the TypeScript PTC SDK and tool-description tokens with their Python forms; run results retain only the bounded logs, completion, or failure selected by the Consumer.

#### KV Cache effect

Selecting this provider changes the PTC SDK and `run_code` schema from TypeScript to Python for new assembled requests; unchanged Python assemblies keep a stable prefix.
...
- **Plugin load runs one interpreter probe** — resolving and probing `pythonExecutable` spawns one short subprocess at setup; a deployment without a probeable CPython 3.10+ interpreter cannot load this provider, and a wrapper that ignores SIGTERM is force-killed after five seconds.
- **Binding backlogs are count-bounded, not fairness-scheduled** — at most 1024 in-flight binding calls and 1024 unflushed reply frames coexist per run; a hostile program exceeding either settles the run as a worker-exit instead of queueing unbounded work.
