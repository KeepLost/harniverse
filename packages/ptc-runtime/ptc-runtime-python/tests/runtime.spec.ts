...
async function setup(config: Config = {}) {
  ...
  return { ctx, fiber, runtime: ctx.ptcRuntime as PythonPtcRuntime }
}
...
describe.skipIf(!python3Available)('PythonPtcRuntime real subprocess', { timeout: runtimeTestTimeoutMs }, () => {
  ...
  it('removes ctx.ptcRuntime when its fiber disposes', async () => {
    const { ctx, fiber } = await setup()
    expect(ctx.get('ptcRuntime')).toBeInstanceOf(PythonPtcRuntime)
    await fiber.dispose()
    expect(ctx.get('ptcRuntime')).toBeUndefined()
  })
})

describe('PythonPtcRuntime service contract validation', () => {
  it('rejects invalid config and portable binding names', async () => {
    const ctx = new Context()
    await expect(ctx.plugin(PythonPtcRuntime, { pythonExecutable: '' })).rejects.toThrow(/pythonExecutable/)
    await expect(ctx.plugin(PythonPtcRuntime, { cpuSeconds: 0 })).rejects.toThrow(/cpuSeconds/)
