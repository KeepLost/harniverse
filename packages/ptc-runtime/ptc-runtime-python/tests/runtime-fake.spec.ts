...
import type { CodeBindingNamespace, CodeRunRequest } from '@deepseek-ai/dsh-ptc-runtime'
...
import { PythonPtcRuntime, resolvePythonExecutable } from '../src/index.ts'
...
async function setup(config: Config = {}) {
  ...
  const fiber = await ctx.plugin(PythonPtcRuntime, { pythonExecutable: process.execPath, ...config })
...
describe('PythonPtcRuntime deterministic process boundary', () => {
  it('validates every deployment limit and executable guard', async () => {
    ...
    for (const [config, message] of invalidConfigs) {
      const ctx = new Context()
      await expect(ctx.plugin(PythonPtcRuntime, { pythonExecutable: process.execPath, ...config })).rejects.toThrow(message)
    }
  })
  ...
  it('normalizes spawn throws, missing fd3, and child startup errors', async () => {
    ...
    await expect(ctx.ptcRuntime.run(request())).resolves.toEqual({
