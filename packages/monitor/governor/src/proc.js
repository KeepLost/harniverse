 ...
 */
export async function scanBuckets(pgids, sids, internals = defaultInternals) {
    ...
    for (const name of names) {
        ...
        if (stat === undefined)
            continue;
        const keys = [];
        if (pgids.has(stat.pgrp))
            keys.push(`pgid:${stat.pgrp}`);
        if (sids.has(stat.session))
            keys.push(`sid:${stat.session}`);
        if (keys.length === 0)
            continue;
        const owned = [];
        for (const key of keys) {
            ...
            const bucket = owned[owned.length - 1];
 ...
 */
export async function readProcMemory(pid, internals = defaultInternals) {
    let statm;
    try {
 ...
 */
export async function readProcIo(pid, internals = defaultInternals) {
 ...
 */
export async function readProcFds(pid, internals = defaultInternals) {
 ...
 */
export async function readHostNet(readFile = defaultInternals.readFile) {
    let dev;
    try {
    ...
    for (const line of dev.split('\n').slice(2)) {
        ...
        if (name === undefined || counters === undefined)
            continue;
 ...
 */
export async function readFreeBytes(path, internals = defaultInternals) {
    try {
