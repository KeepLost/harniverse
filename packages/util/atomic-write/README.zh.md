...
- **有界的 Windows 重试**：瞬态 `EACCES`/`EBUSY`/`EPERM` 替换干扰下保持同一份完整临时文件并按指数退避重试，协作写锁之外的软件不会把一次安全替换变成即时失败；理由与被否决的替代方案见[重试决策](../../../.agents/notes/implemented/bug-fix/2026-09-05-windows-stability-batch.md)。
- 自动创建父目录；最终失败仍会移除临时文件并重新抛出；读取方只会观察到旧内容或完整的新内容。

`withFileLock` 跨进程串行化同一文件的写入方，服务于单靠原子提交无法保证安全的读-渲染-提交循环。锁是以 `wx` 创建的同目录 `<filename>.lock`，因此读取方从不参与竞争；等待方按指数退避，超时即失败而非无限阻塞。竞争者绝不移除现有锁：锁龄无法区分已经崩溃的所有者与被暂停但仍存活的写入方。在 Windows 上，与前一个持有者尚未完成的删除相竞速的锁创建会以 `EPERM` 浮现，它将按普通竞争等待化解，而不会让写入方直接失败。
