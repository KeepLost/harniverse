 ...
 *
 * Nothing here imports Node transport modules, so the module stays loadable anywhere the policy
 * must be evaluated without a transport.
 ...
 *
 * `::1` and `[::1]` are both listed because the resolved string is also handed to consumers that
 * read an environment, whose matchers read a bare `::1` as host `:` port `1` and therefore never
 * bypass it.
 */
export const LOOPBACK_NO_PROXY: readonly string[] = ['localhost', '127.0.0.1', '::1', '[::1]']

/**
 * The environment names each policy field owns, lowercase first — Node's own env-proxy support
 * reads the lowercase name first, so both casings are always written or cleared together.
 */
...
/**
 * Read one environment name in the env-proxy precedence order — lowercase first, uppercase as the
 * fallback — treating a blank value as unset. Blank matters: Node's own env-proxy chain lets an
 * empty lowercase name shadow a populated uppercase one.
 *
 ...
 * a bracketed literal takes its port from after the bracket. Getting this wrong is how a naive
 * split turns `::1` into host `:` port `1`.
 *
 ...
 * Node's own env-proxy order so this function and the installed dispatcher never disagree about one
 * URL.
 *
...
export function resolveProxyPolicy(env: EnvLookup): ProxyResolution {
  ...
  // HTTPS falls back to the HTTP proxy last, matching Node's own env-proxy order — but never past a value the user named
