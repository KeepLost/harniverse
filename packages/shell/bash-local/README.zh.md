...
- **每次调用都 spawn，不保留 shell 状态**：每次调用都启动新的命令 shell 进程；macOS 使用 `/bin/zsh -c`，其他 POSIX 宿主使用 `bash -c`。调用之间不共享 shell 状态。
...
- **仅支持 POSIX**：macOS 使用 `/bin/zsh`，其他 POSIX 宿主使用 `bash`；不支持 Windows。
