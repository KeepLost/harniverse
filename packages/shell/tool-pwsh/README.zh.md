...
- **本工具不提供持久 shell** — 每次调用都启动全新的 `pwsh -Command`；显式 opt-in 的 [`dsh-tool-pwsh-persistent`](../tool-pwsh-persistent/README.md) 对应物会跨调用保留一个 owner 作用域的 PowerShell PTY，但不会替换随发布提供的默认组合。
