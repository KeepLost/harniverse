...
- **No persistent shell in this tool** — every call starts a fresh `pwsh -Command`; the explicit opt-in [`dsh-tool-pwsh-persistent`](../tool-pwsh-persistent/README.md) counterpart keeps one owner-scoped PowerShell PTY across calls without replacing shipped defaults.
