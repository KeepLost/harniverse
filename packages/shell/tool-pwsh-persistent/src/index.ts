/* jscpd:ignore-start -- deliberate PowerShell counterpart to tool-bash-persistent. */
/**
...
/** Escape a command body for a PowerShell double-quoted wrapper string. */
...
function commandOutput(snapshot: RetainedOutput, marker: CommandMarkers, wrapper: string): CapturedOutput | undefined {
...
function retainedScrollback(
  ...
  while (offset < latest.totalLines) {
...
function persistentShells(ctx: Context, config: ResolvedConfig): PersistentShells {
  ...
  const get = (owner: Agent, signal: AbortSignal): Promise<TerminalSessionId> => {
    ...
    const tracked = creation.finally(() => { creating.delete(tracked) })
  ...
  while (true) {
    ...
    if (timedOut !== undefined) {
      const partial = renderCaptured(
        partialOutput(retainedScrollback(ctx, owner, id, latest), marker, wrapped, fallback, fallbackTruncated),
    ...
    if (promptCompleted(result)) {
      return renderCaptured(
        partialOutput(
          retainedScrollback(ctx, owner, id, latest), marker, wrapped, fallback, fallbackTruncated,
        ),
        config.maxOutputChars,
      )
    }
