...
describe('tool-pwsh-persistent', () => {
  ...
  it('handles inferred idle, prompt fallback, shell exit, clipping, and cleanup', async () => {
    const { ctx, owner, stub, fiber } = await setup({ backendType: 'stub', maxOutputChars: 10 })
    await call(ctx, owner, 'warm up')
    const session = stub.sessions[0]!
  ...
  it('rejects invalid config and invalid calls', async () => {
    ...
    expect(() => { ToolPwshPersistent.apply(new Context(), { backendType: '' }) })
      .toThrow('backendType must be non-empty')
    expect(() => { ToolPwshPersistent.apply(new Context(), { timeoutMs: 0 }) })
      .toThrow('timeoutMs must be a positive safe integer')
    expect(() => { ToolPwshPersistent.apply(new Context(), { maxOutputChars: 0 }) })
      .toThrow('maxOutputChars must be a positive safe integer')
    expect(() => { ToolPwshPersistent.apply(new Context(), { description: ' ' }) })
