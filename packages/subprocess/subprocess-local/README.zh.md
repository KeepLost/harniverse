...
- **终端进程所有权**：`spawnTerminal` 以 `spec.term`（默认 `'dumb'`）作为终端名分配 `node-pty`，而 node-pty 正是用它作为子进程的 `TERM` 发布——它会覆盖 `spec.env` 中的任何 `TERM`，因此决定终端能力的是 seam 字段。它桥接 UTF-8 终端文本，检查当前前台进程组并向其发送信号，还会公开一项须等待的终止操作，在终止顶层 shell 前后清理后代进程。每次前台检查只读取一次进程表，并从这一次观察中保留精确身份，因此一次轮询的开销与运行中命令派生的后代数量无关；而每个信号在发出时重新读取最窄的单身份存活源，过期的观察永远无法把原始进程的信号交给被复用的 pid。Linux 还会在 POSIX 会话 leader 退出后枚举该会话。因此，之前观察到的 macOS 后代以及同会话 Linux 成员在重新设定父进程后仍受围栏保护，pid/start 身份则防止清理跟随 PID 复用。在 Windows 上，基于 koffi 的检查器仅在表问题时枚举 Toolhelp32 进程行——存活判断在那里是按句柄的检查——以 `GetProcessTimes` 创建身份围栏保护存活判断与 taskkill 信号发送，并在 node-pty 遗漏退出事件时验证 shell 已消失。上层 PTY 后端负责提示符就绪、缓冲区与面向模型的操作。
