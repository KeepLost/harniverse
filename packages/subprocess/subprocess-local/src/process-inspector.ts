...
export interface ProcessSnapshot {
  ...
  /** Return observed session members, empty where the platform's table omits session ids. */
...
export interface ProcessInspector {
  ...
  /** Return whether the foreground group waits on the terminal shell's stdin. */
   ...
   * whole table; callers filtering many members at once want
   * {@link ProcessSnapshot.alive} instead.
   */
  isAlive(identity: ProcessIdentity): boolean
  signalGroup(pgid: number, signal: SubprocessTerminalSignal): void
  /**
   * Signal one exact process identity, fenced against PID reuse. The fence
   * reads current state immediately before the signal; an observation taken
   * earlier in the same round cannot stand in for it, because it preserves the
   * original PID-to-start-time pairing a recycled PID would still match.
   */
