...
nonWindows('fails clearly when the lazy Win32 bridge is invoked off Windows', () => {
  expect(() => createWindowsProcessInspector().snapshot().tree(process.pid))
    .toThrow('Windows process inspection requires koffi and kernel32.dll')
})
