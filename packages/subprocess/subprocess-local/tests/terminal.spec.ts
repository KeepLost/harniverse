...
describe('LocalTerminalHandle', () => {
  ...
  it('uses node-pty only when the shell start identity was unavailable', () => {
    ...
    const racingHandle = makeHandle(racingPty, racingInspector, 10)
...
describe('LocalTerminalHandle on Windows', () => {
  ...
  it('settles done when taskkill removes the shell without a node-pty exit event', async () => {
    const pty = new FakePty()
    pty.autoExitOnKill = false
    const inspector = new FakeInspector()
    inspector.alive.add(123)
...
describe('process-table read amplification', () => {
  // The macOS inspector answers every table question by forking `/bin/ps`, so
  // a readiness poll that asks per descendant scales its blocking cost with
  // the command's process tree. These pin the read count, not the wall time.
