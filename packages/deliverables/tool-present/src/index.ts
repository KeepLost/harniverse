 ...
 * @module @deepseek-ai/dsh-tool-present
 */
...
// Type-only: activates the `turnBoundary` key this tool reads from
// SessionProjectionMap; the projection itself is owned by agent-loop.
import type {} from '@deepseek-ai/dsh-agent-loop'
import type {} from '@deepseek-ai/dsh-session-projection'
import type { Session } from '@deepseek-ai/dsh-session'
import type { PresentedFile } from './types.ts'
// The `deliverables/presented` declaration lives in src/types.ts (its one
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const boundary = ctx.sessionProjections.snapshot(exec.agent.session).values.turnBoundary
    ...
    presentCall: args => ({ card: 'generic', title: 'Present deliverables', kind: 'other', rawInput: args.files }),
  }))
