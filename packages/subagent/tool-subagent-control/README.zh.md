...
可选的全局具名 `subagent_history` 工具不会恢复 child，而是读取已授权直接或嵌套 child 的原始事件，每次最多 50 条，并通过 `before_seq` 翻页；第一方持久化后端使用原生 raw-event 分页，第三方后端可能回退到完整检查；可选的会话存储与持久化服务在调用时解析。

可选的全局具名 `send_message`、`interrupt_agent`、`subagent_history` 与 `list_agents` 工具是 `ctx.subagents` 之上的轻量适配器。绑定提供方的 `@deepseek-ai/dsh-tool-subagent` 实例会为每种传输注册不同的委派工具；这个单独加载的包只注册一次共享控制工具，因此多个委派工具绝不会重复注册全局控制工具。根插件注册 `send_message`、`subagent_history` 与 `interrupt_agent`，且只要求 `subagents`；可单独加载的 `./list-agents` 插件注册 `list_agents`，并将 `subagents` 与 `agents` 声明为加载时依赖。其目录读取在调用时还要求会话存储与投影注册表，但不要求任何查询服务。部署可保留根插件工具并省略列表工具。是否加载这些工具不会决定委派工具是否启动可继续工作。这些工具只负责父到子的方向；单独安装的 [`@deepseek-ai/dsh-tool-subagent-report`](../tool-subagent-report/README.md) 负责子到父的方向。
...
`subagent_history` 返回格式化 JSON 字符串，包含已授权 child 的原始事件序列、事件类型、可选的 surface operation 与 data。非后代、非法边界或不可用的持久化服务会返回出错结果；活动 child 从当前快照读取，冷的第一方 child 使用有界持久化页面，冷的第三方 child 使用检查回退路径，均不会恢复 child。

#### Token 影响

随请求的事件数量和事件 payload 大小增长，但每次调用最多返回 50 条完整事件。

#### KV Cache 影响

仅追加；每个页面都位于可复用请求前缀之后。
...
- **第三方持久化回退**：未覆写 `readRawEventPage()` 的后端可能会先实体化所选冷 child 的完整已验证日志，再截取有界结果。随附的 SQLite 与 JSONL 后端使用原生 raw-event 分页读取器。
