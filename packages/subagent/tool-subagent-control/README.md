...
The optional, globally named `subagent_history` tool reads at most 50 raw events from an authorized direct or nested child without resuming it. It accepts `before_seq` for older pages; first-party persistence backends use a native raw-event page, while third-party backends may fall back to a complete inspection. Its optional SessionStore and persistence dependencies are resolved at call time.

The optional, globally named `send_message`, `interrupt_agent`, `subagent_history`, and `list_agents` tools are thin adapters over `ctx.subagents`. Provider-bound `@deepseek-ai/dsh-tool-subagent` instances register distinct delegation tools per transport; this separately loaded package registers shared control tools once, so multiple delegation tools never register duplicate global controls. The root plugin registers `send_message`, `subagent_history`, and `interrupt_agent` and requires only `subagents`; the separately loadable `./list-agents` plugin registers `list_agents` and declares `subagents` plus `agents` as load-time dependencies. Its catalog reads additionally require the session store and projection registry at call time, but no query service. A deployment can keep the root tools while omitting the list tool. No tool's presence determines whether a delegation tool starts continuable work. These tools own only the parent-to-child direction; the independently installed [`@deepseek-ai/dsh-tool-subagent-report`](../tool-subagent-report/README.md) owns the child-to-parent direction.
...
### Subagent history result

#### What the model sees

`subagent_history` returns a formatted JSON string containing the authorized child's raw event sequence, event type, optional surface operation, and data. A non-descendant, invalid bound, or unavailable persistence service is an errored result; live children are read from their current snapshot, cold first-party children use the bounded persistence page, and cold third-party children use the inspection fallback rather than being resumed.

#### Token effect

Grows with the requested event count and event payload size, capped at 50 complete events per call.

#### KV Cache effect

Append-only; each page follows the reusable request prefix.
...
- **Third-party persistence fallback** — a backend that does not override `readRawEventPage()` may materialize the selected cold child's complete validated log before slicing the bounded result. The shipped SQLite and JSONL backends use their native raw-event page readers.
