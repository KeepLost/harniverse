...
- `tools/result` 观察器只会在该次执行的权威最终工具结果成功后提交暂存值；PTC 子分派外层的 `run_code` 结果也包括在内。
...
共享驱动器把任务逐字作为子 agent 的用户消息发送；若有请求，还会在未发布子 agent 的全新作用域中遮蔽 persona，并限制全局工具 schema、查找、执行和 PTC SDK 绑定。父 agent 的限制不会被继承，独立的工具指导段仍会保留。spawn 不提供历史；fork 提供平衡的初始内容。
