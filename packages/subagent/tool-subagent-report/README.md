...
- **A parent whose host-owned disposal already started rejects new reports** — `AgentHandle.dispose()` opens the Agent admission cutoff before awaiting quiescence and before the handle leaves the registry. Reports accepted before that cutoff remain in the parent's transcript; reports racing after it fail with the closing-lifecycle error. A continuation-manager-owned parent also rejects forest teardown through the manager's admission boundary.
