...
- **父级在宿主启动 dispose 后会拒绝新的报告**：`AgentHandle.dispose()` 会在等待完全停稳和 handle 离开注册表之前打开 Agent admission cutoff。在 cutoff 前接受的报告仍保留在父级 transcript 中；之后竞争到达的报告会以 closing-lifecycle 错误失败。对于由继续执行管理器拥有的父级，管理器的准入边界也会拒绝整片森林拆卸期间的新报告。
