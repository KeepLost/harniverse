...
对于命令与文件审批，无人值守的提供方会从请求给出的决策选项中选择一项不予批准的决策，并优先选择 `cancel`；稳定的 0.147.0 请求形态没有决策选项列表，因此回退到 `decline`。它对权限请求返回作用域限于当前轮次的空权限集，不向用户输入请求提供任何答案，并拒绝 MCP elicitation。若请求在无人值守模式下没有合法响应，或是未知服务器请求，此次运行就会失败。失败运行最多报告固定的请求类别与决策，绝不会复制请求或 item payload。

本地取消会在结果竞态中胜出并映射为 `aborted`。失败轮次的白名单 `codexErrorInfo` 类别若为 `contextWindowExceeded`，则映射为 `max-tokens`；其他任何远端中断或失败轮次都映射为 `error`，且该提供方不会产生 `refusal`。未完成与被扁平化的结果会附带安全 diagnostic，其中只有固定的产品、阶段和白名单类别字段、受认可连接或流变体提供的有界数值 HTTP 状态，以及独立观察到的进程退出码或信号。Stderr 会转发给 Host，但提供方只识别固定的审批拒绝与沙箱违规特征，绝不会把原始 stderr 或协议数据复制到 diagnostic。`dispose()`（资源释放）具有幂等性：如果当前的两个标识符均已知，它会尽力请求 `turn/interrupt`，关闭 JSON-RPC 通信链路，结束标准输入，调用共享的进程树逐级终止机制，并等待整棵进程树退出。结果失败与独立的清理失败仍彼此分离。
...
通过 `dsh-tool-subagent`，父级模型只会看到选定的 Codex 最终答案，或者看到消费方针对未成功完成的结果给出的错误。失败可以附加一条分离且有界的 `Diagnostic:` 行，其中只含固定安全事实。Codex 的过程说明、推理（reasoning）、工具活动、原始 stderr、协议 payload、路径、提示词、环境值、凭证、工作区差异和产品标识符均不会复制到父会话。
...
- **最终文本与安全失败事实**：推理、过程说明、中间消息、工具通信、用量信息、原始 stderr、协议 payload 和工作区差异仍只保留在产品内部。
