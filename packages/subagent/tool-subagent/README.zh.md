...
`backgroundMode` 仍是部署级默认策略。面向模型的 schema 使用 `mode: sync|async`：`sync` 等待结果，`async` 在接受持久化 child 回合后返回。可继续的异步工作要求提供方具备 `prepareContinuable`，通过统一 Invocation 服务启动，并保持 child 可接受后续消息。结果保留在 child transcript 中；每当 Activation 结束，继续执行服务会投递一条结算通知。启动异步工作不要求加载 `send_message`。
...
当提供方存在时，以当前实例配置的名称公开已生成的默认 [`subagent` schema](../../../docs/tool-catalog.md#deepseek-aidsh-tool-subagent)。schema 公开 `mode: sync|async` 和可选的 `profile_id`，但不会接受原始命令、endpoint、凭据或 Profile 路径。启用 Profile 管理后，模型还会看到父级私有的 define/list 工具。当工具在本次组装的作用域中可见时，一个 `tool:<toolName>` 系统提示词 section 会说明配置的异步默认策略，并要求仅在下一步动作依赖结果时选择 `mode: sync`。
