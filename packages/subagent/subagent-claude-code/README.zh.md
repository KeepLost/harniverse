...
SDK 接收由文本块原样拼接成的任务。提供方会完整迭代 SDK 消息流，而且只接受满足以下条件的 `result` 消息：其 `subtype: "success"`、`is_error: false` 且 `result` 非空白，之后迭代器还须正常结束。所有 SDK 错误子类型、标记为错误的成功消息、缺失答案、迭代器失败、协议失败或进程失败都映射为 `error`；该提供方不会产生 `max-tokens` 或 `refusal`。已发布运行失败时会附带安全 diagnostic，其中只有固定的产品、阶段和类别字段，以及观察到的直接进程退出码或信号；原始 SDK 错误与 payload 只进入 Host 错误日志。
...
每次 query 都设置 `persistSession: false` 并禁用 `AskUserQuestion`。固定回调会拒绝工具审批、拒绝 MCP elicitation，并取消受支持的阻塞式对话，因此无人值守交互绝不会等待本提供方不负责的用户界面。失败运行的 diagnostic 最多包含固定的请求类别与决策，绝不会复制回调输入。
...
通过 `dsh-tool-subagent`，父级模型只会看到符合严格成功条件的 Claude Code 最终答案，或者在结果未完成时看到消费方给出的错误。失败可以附加一条分离且有界的 `Diagnostic:` 行，其中只含固定安全事实。Claude Code 的推理、工具活动、中间消息、原始错误、stderr、路径、提示词、环境值、凭证、协议 payload、工作区差异、用量信息和产品标识符均不会复制到父会话。
...
- **没有人工交互路径**：`AskUserQuestion` 被禁用，固定回调会拒绝、回绝或取消其他受支持的交互请求，因此需要新审批或输入的任务会失败而不会挂起。
