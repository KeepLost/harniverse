...
共享的 `normalizeRedundantEscalation` 辅助函数仅在请求模式等于已解析的有效模式时移除模型传入的 `sandbox_permissions` 和 `justification`。bash、pwsh、write、edit 在配对字段验证前调用它；真正升权仍要求非空理由与用户审批，更窄请求仍被拒绝。
