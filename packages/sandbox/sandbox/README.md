...
The shared `normalizeRedundantEscalation` helper removes a model's `sandbox_permissions` and `justification` only when the requested mode equals the resolved effective mode. Bash, pwsh, write, and edit use it before paired-field validation; real widening still requires a non-empty reason and user approval, while narrower requests are refused.
