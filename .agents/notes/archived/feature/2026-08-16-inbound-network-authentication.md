...
`dsh-authentication` defines one process-wide admission service. `dsh-authentication-local` stores named token digests under `$DSH_HOME`, acquires the sole network-instance lease before WebServer bind, creates bounded process-memory browser sessions, limits invalid credentials by carrier and direct peer, watches registry commits with periodic reconciliation, and writes privacy-minimal rotated JSONL access records. In authenticated mode, a watcher or registry-read failure rejects admission and closes current sessions and sockets until reconciliation succeeds; bypass does not depend on registry freshness. The Web composition selects authenticated mode by default. `--dangerously-skip-authentication` retains the lease, trust fence, and access records but is accepted only on loopback. An all-interfaces listener requires the paired `--tls-cert` and `--tls-key` paths.
...
The browser shell calls `/auth/status` and, when needed, `/auth/login` before it parses the boot manifest or constructs the module system. A successful HTTPS login sets a process-memory-backed `__Host-` cookie with Secure, HttpOnly, SameSite=Strict, and Path attributes; loopback HTTP retains the HttpOnly strict cookie. Authentication responses are not cacheable. Non-browser clients use Bearer tokens. Host, Origin, Fetch Metadata, media-type, and DNS-rebinding checks remain independent and run alongside authentication. The WebServer terminates configured TLS directly and refuses a plaintext all-interfaces listener.
...
All external HTTP API and WebSocket admission, localhost included, requires a valid credential unless loopback bypass is explicit. Token values appear once on add/reset and never enter registries, logs, URLs, browser storage, or model input. Access-record failure turns an accepted admission into a rejection, while a committed token revocation still closes sessions and sockets when its secondary application record fails. Invalid credentials eventually return a bounded `rate-limited` decision and retry interval without growing limiter state beyond its configured capacity. Direct network exposure requires operator-supplied certificate and key files; automatic certificate issuance and trusted-proxy handling remain outside this decision.

Focused tests pin token management, permissions and rotation, lease mutual exclusion, sealed recovery, targeted registry events, targeted WebSocket closure, localhost 401 responses, failure limiting and recovery, HTTP and HTTPS cookie attributes, plaintext remote-listener rejection, real HTTPS serving, startup mode selection, and browser boot ordering. The assembled Web replay remains the product-level check for login before plugin startup.
