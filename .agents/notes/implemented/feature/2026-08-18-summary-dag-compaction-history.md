...
The package also registers `ctx.compactionHistory`, an in-memory projection rebuilt from each live Session's append-only event log. A `compaction/summary` becomes searchable only after its matching compact checkpoint commits. A leaf node cites raw message seqs; a condensed node cites earlier committed summary nodes and separately retains raw messages introduced in its own replacement. Stable node ids derive from the owning Session id and summary event seq.
