...
Harniverse ships `@deepseek-ai/dsh-compaction-lossless` as the selected compaction Provider in base, minimal, standard, code, Cordis, and standalone headless compositions. It inherits `BasicCompactionEngine`, so automatic pressure, overflow recovery, retention, summarization, cancellation, tool pairing, durable event ordering, and surface replacement keep one implementation. Its `auto` default remains `true`, and all inherited policy fields remain configurable.
...
`@deepseek-ai/dsh-tool-compaction-history` consumes that service through `compaction_history_search` and `compaction_history_expand` in standard, code, Cordis, and standalone headless compositions. Minimal keeps the projection internal and mounts neither history Consumer. Search performs case-insensitive all-term matching over committed summary text. Expansion follows parent links and optionally returns raw message sources. Configuration caps result count, DAG depth, and a deterministic token estimate; the expansion tool applies the token cap to its complete rendered result, including metadata.
...
Compacted details remain recoverable from canonical history under explicit count, depth, and output bounds, while automatic compaction behavior stays aligned with the basic Provider. Profiles that mount the two history tools pay fixed prompt and schema tokens plus ordinary tool-result tokens when the model calls them; Minimal pays no model-facing history-tool tokens.
