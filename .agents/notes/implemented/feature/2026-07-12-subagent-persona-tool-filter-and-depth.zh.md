...
| 控制 | 问题 | 结果 |
|---|---|---|
| `persona` | 什么角色指令替换该子 agent 的部署人设？ | 一个子 agent 局部的提示词上下文遮蔽 `deployment:persona` |
...
人设控制改变一个子 agent 的行为，而不改变部署级的提示词组装。在未发布的设置阶段，进程内提供方在子 agent 作用域中注册一个名为 `deployment:persona` 的动态上下文；普通的最具体者优先解析规则仅在该子 agent 的组装中替换全局上下文。

其值与部署人设具有相同的严格模板语义。省略时通过全局层继承部署上下文；显式空字符串则以空上下文遮蔽全局人设。父级和兄弟级的人设永远不会进入子 agent 的扁平作用域。
