...
Denied file effects return a `[sandbox: file access denied under <mode> mode]` marker and instructions not to work around the denial. A confining executor adds paired `sandbox_permissions` and `justification` fields for one approved retry that must be strictly wider than the session's effective mode; a declaration equal to the effective mode is an ordinary call ([redundant-field decision](../bug-fix/2026-09-26-redundant-sandbox-escalation.md)). A grant widens only that retry; rejection executes nothing, returns `the user rejected escalating this command to "<mode>"`, and permits no re-ask. The owner-derived pending policy context states the current file policy without replacing those enforcement boundaries. When `dsh-permission-presets` is composed with a UI adapter, one preset selects both knob values; unmatched values fold to `custom`. The [ACP automation composition](../../../../examples/acp-agent/README.md) does not mount that UI service and selects its deployment mode explicitly.
...
- The escalation fields exist exactly when the mounted executor confines; a request equal to the call's effective mode is redundant and runs under standing policy, while a narrower target fails closed without prompting; a deployment with no ApprovalService fails genuine escalating calls closed and leaves plain calls untouched.
...
- **The advertised target set is static while the effective mode is per-session** (schemas are registry-global) — a session already at the widest mode is still offered the fields. The per-call equality check discards redundant fields without prompting; the strict-wider check still rejects narrower requests and protects genuine escalations.
