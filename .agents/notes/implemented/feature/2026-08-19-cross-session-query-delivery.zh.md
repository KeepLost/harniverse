...
`ctx.sessionQuery` 还暴露不会恢复会话的运行状态、最终模型可见消息的有界尾部，以及完整原始日志的有界尾部。状态在可用时采样精确的 live Agent/Session 身份；消息尾部使用规范 surface fold 和 `deriveEventMessage()`，而非 Host 展示历史投影。`session_log_tail` 返回最近的完整原始事件；`session_event_read` 将其有界原始窗口内的每个事件都作为完整 JSON 返回。
...
- 原始日志工具保留 shadowed 和 log-only 轨迹；`session_message_tail` 仍是折叠后的当前模型消息视图。
