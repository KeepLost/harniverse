...
`@deepseek-ai/dsh-tool-session-query` 是 `ctx.sessionQuery` 面向模型的消费方。它注册九个职责单一的只读工具：`session_find`、`session_search`、`session_event_search`、`session_status`、`session_message_tail`、`session_log_tail`、`session_trace`、`session_event_trace` 和 `session_event_read`。该包依赖接口而非 SQLite 实现，负责模型参数校验与易读文本渲染，并贡献一个精简的提示词段，区分元数据发现、内容搜索、当前消息和原始日志读取。

该包入口仅作为配置、提示词注册与工具注册的公开组合根。内部模块沿执行边界划分：`input.ts` 负责模型 schema、规范化与过滤条件构造；`service-boundary.ts` 包含提供方调用与面向模型的安全错误转换；`workspace-access.ts` 负责调用者身份、id 绑定观察、标题访问与谱系投影；`operations.ts` 编排九个服务工作流；`presentation.ts` 渲染工具结果与调用卡片。这样可让策略留在其所属层，同时不改变包约定。

`session_find` 按当前标题、创建时间或原始事件活动时间发现会话元数据，不返回内容匹配事件或摘录。`session_search` 按会话聚合内容全文匹配，并返回最强事件 seq 与摘录；`session_event_search` 搜索一个会话内的内容，默认目标为调用者的当前会话。`session_status` 读取不会恢复会话的运行状态。`session_message_tail` 返回折叠当前模型消息尾部，`session_log_tail` 则返回最近的完整原始事件。`session_trace` 返回完整可见祖先链与递归后代树。`session_event_trace` 返回一个事件所有已知的位置替换关系与直接引用来源事件的关系。`session_event_read` 把有界原始窗口内的每个事件作为未删节 JSON 返回；省略 `before` 与 `after` 时只返回目标。
