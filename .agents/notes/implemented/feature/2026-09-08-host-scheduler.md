...
The service now extends `TypertRemoteService` (namespace `scheduler`) with session-scoped `@Remote` methods — `list` and `runs` (observe), `create`, `update`, and `delete` (operate) — each re-annotated with an `exportName` so the generated client reads `ctx.remote.scheduler.list(sessionId)` like the goals namespace. `ScheduleCreateInput` moved to `types.ts`: the typert analyzer requires Remote boundary types to live on a public non-root type subpath. The generated `./remote` client mounts through `dsh-api-remotes`, and `dsh-client-ui-scheduler` contributes both the `schedule-list` header action (order 10) and the `schedules` settings section (order 30). The settings section aggregates session-scoped reads for the current workspace and provides human CRUD plus recent run history; records expose server-derived prompt revision metadata. Live dispatch updates and a workspace-native scheduler query remain deferred.
