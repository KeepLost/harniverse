...
The service now extends `TypertRemoteService` (namespace `scheduler`) with four session-scoped `@Remote` methods — `list` (observe), `create`, `update`, `delete` (operate) — each re-annotated with an `exportName` so the generated client reads `ctx.remote.scheduler.list(sessionId)` like the goals namespace. `ScheduleCreateInput` moved to `types.ts`: the typert analyzer requires Remote boundary types to live on a public non-root type subpath. The generated `./remote` client mounts through `dsh-api-remotes`, and `dsh-client-ui-scheduler` contributes the `schedule-list` header action (order 10) whose verbs bind per-session through slot `inject`; it renders only when the Remote reports an owned record, keeping existing snapshots untouched. Live dispatch updates and a dedicated management page stay deferred.
