...
`@deepseek-ai/dsh-code-runtime-python` is an opt-in Service Provider. `PythonCodeRuntime extends CodeRuntime`, reports `language: 'python'` and `isolation: 'process'`, and starts a fresh configured executable for every run with `shell: false`, only the Host `PATH` needed to resolve a bare executable name, and a package-owned standard-library bootstrap that clears the environment before model code runs. No shipped Profile or bundle selects it; replacing the existing `code-runtime` Cordis row is a deployment decision, and the existing `dsh-tools` language dispatch supplies the Python SDK and `run_code` strings.
...
Failure text from the process boundary is static or path-scrubbed and bounded. Host binding rejections become the typed program-side message `binding call failed`, rather than carrying an arbitrary Host exception that could contain a path or credential. Model code receives no ambient environment, but it still shares the Host filesystem, network, working directory, and identity; the provider explicitly makes no sandbox claim.
