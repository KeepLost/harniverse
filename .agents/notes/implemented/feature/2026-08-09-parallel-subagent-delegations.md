...
Two asynchronous delegations in one message acquire their model-visible child Session and Invocation ids through the subagent runtime. The identities are logged, so replay stays valid, but a snapshot scenario that distinguishes its concurrent children would inherit the same determinism constraint as twin child sessions.
