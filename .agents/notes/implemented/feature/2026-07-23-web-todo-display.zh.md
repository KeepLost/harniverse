...
`applyEventSideEffects` 新增一个 `todo/write` 分支（整份列表，后写覆盖先写），并跨越 `turn/start` 保留。`rebuildDerivedFromWindow` 从空计划扫过窗口，仅当窗口从未判定计划（无 `todo/write`）时恢复尾页种子；否则以窗口内写入折叠为准。`installWindow` 的每个调用方都是尾页请求（`doOpen`、其补洞重拉、`repairGap`；`loadOlder` 只往前拼接、不再播种），而 host 对尾页请求要么带上投影、要么在没有计划状态时省略——因此字段缺失就是权威的空列表，直接照此赋值。`ConversationSnapshot.todos` 是读取面。这遵循事件自身的约定（「仅存在于日志中的 UI 状态；绝不纳入派生历史」）：把每次写入作为对话节点呈现，会让已被取代的列表看起来仍然有效。
