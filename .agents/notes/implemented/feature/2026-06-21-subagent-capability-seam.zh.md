...
- **阻塞父轮次。** 前台收集在子 agent 的整个持续时间内保持父 agent 的步骤打开。异步委派使用持久化 Session/Invocation 生命周期和 session 控制工具；通用 `ctx.jobs` 仍是 shell 与终端后台工作的 seam，而 subagent seam 本身仍不感知任务。
