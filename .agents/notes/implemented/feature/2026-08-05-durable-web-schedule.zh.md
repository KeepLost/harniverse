...
Retired 2026-09-20：owner 在 wave-3 W02 退役中移除了官方仅限 Session 内的 Schedule 包及其 overlay；宿主级 dsh-scheduler 家族现在负责定时投递。examples/web-schedule overlay 显式加载 `@deepseek-ai/dsh-time-context` 与 `@deepseek-ai/dsh-schedule`；默认 Web 配置树保持不变。Schedule 只观察插件加载后发布的根 Agent，并在该 Agent scope 中安装三个工具和一个可丢弃 owner。cold history 读取、已发布的根、child Agent 与其他 host 都不会激活它。
