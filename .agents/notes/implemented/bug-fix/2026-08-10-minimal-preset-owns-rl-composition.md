...
> The Web Minimal runtime described here was later replaced by [Shipped Agent Profile capability defaults](../architecture/2026-08-30-agent-profile-capability-defaults.md): the current Web Profile uses one-shot platform shell tools and an internal lossless recovery provider. The standalone JSON-RPC composition remains separate.
...
The preset persona remains exactly `You are a helpful software engineer assistant.`, but it is registered as the same dynamic `deployment:persona` context used by every built-in Profile. It does not set `complete` or suppress runtime context; the [unified dynamic-prompt decision](../architecture/2026-08-30-unified-dynamic-prompt-and-runtime-defaults.md) owns the shared assembly path. The preset still owns the Web RL tool roster and its entry-local runtime services, while the stable Harniverse identity and any mounted dynamic contexts are assembled by the common system-prompt registry.
...
System-prompt and persona package tests prove the shared dynamic-context assembly path. The shipped-preset composition test asserts the exact static identity, dynamic runtime-context snapshot, Bash description, absolute editor schema, and two-tool catalog under the default native presentation. The keyless Web replay sends a real request through a `minimal` agent while global identity, Web-orientation text, dynamic policy contexts, and a test context are registered, asserts that the contexts arrive through the common runtime-context path, the entry-local filesystem is bare, and compaction is absent, then executes two persistent Bash calls to prove environment and cwd state survive and executes the editor through an absolute path.
