...
> 本 note 描述的 Web Minimal runtime 后来由[已发布 Agent Profile 能力默认值](../architecture/2026-08-30-agent-profile-capability-defaults.md)取代：当前 Web Profile 使用 one-shot 平台 shell 工具和内部 lossless recovery Provider。standalone JSON-RPC composition 仍然独立。
...
preset persona 仍然恰好是 `You are a helpful software engineer assistant.`，但现在通过所有内置 Profile 共用的动态 `deployment:persona` 上下文注册。它不再设置 `complete` 或抑制 runtime context；[统一动态提示词决策](../architecture/2026-08-30-unified-dynamic-prompt-and-runtime-defaults.md)负责这套共享组装路径。preset 仍然拥有 Web RL 工具 roster 及其 entry 本地运行时服务，而稳定的 Harniverse 身份和所有已挂载的动态上下文由共用的 system-prompt 注册表组装。
...
系统提示词与 persona 包测试证明了共用的动态上下文组装路径。交付 preset 组合测试在默认原生呈现下断言精确的静态身份、动态 runtime-context 快照、Bash 描述、要求绝对路径的编辑器 schema 和双工具目录。无密钥 Web 回放通过 `minimal` agent 发送一个真实请求，同时注册全局身份、Web 定位文本、动态策略上下文和一个测试上下文；它断言这些上下文经共用的 runtime-context 路径进入请求、entry 本地文件系统是裸后端且压缩不存在，随后执行两次持久 Bash 调用，证明环境与 cwd 状态能够保留，并通过绝对路径执行编辑器。
