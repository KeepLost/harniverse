...
HMR contains live refresh rejection. Its `registerConfig(filename, refresh)` method resolves one exact path through the nearest existing canonical ancestor, owns a stat baseline and asynchronous polling, and returns an async disposer that stops polling and drains active work ([readiness decision](2026-09-11-ci-readiness-boundaries.md)). Both exact-path and ordinary config-file refreshes use the serialized, coalescing queue. A failure is normalized to `Error`, logged, and broadcast through the parallel `hmr/config-update-failed(filename, error)` event; rejecting observers are logged without stopping later refreshes. Creation, change, and removal are observed.
...
- Exact config polling adds timers and stat I/O only for registered paths and drains them with their owning HMR fiber.
