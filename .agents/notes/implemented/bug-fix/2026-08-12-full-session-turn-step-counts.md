...
2. A cancelled step with no visible prefix has no `assistant/message`, while a step with a visible prefix may now persist an interrupted `assistant/message`; counting messages would still silently drop cancelled steps without content.
