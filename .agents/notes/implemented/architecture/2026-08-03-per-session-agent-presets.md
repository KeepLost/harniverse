...
**The effective default is read per resolution, never snapshotted.** The new value applies to the next Session created, while every running Session keeps the composition it was built from. `SessionHeader.agentProfile` is the immutable identity used by resume, fork, cold presentation, and delivery; a different Profile creates a different Session.
...
**A Session's Agent Profile is immutable from creation.** The Profile determines tools, prompts, hooks, and default permission. The protocol exposes it on `session.create({ agentProfile })` and has no Profile-switch method; choosing a different Profile always creates another Session, even when the existing one is blank.

**Authoring a Profile is a privileged RPC.** `read`, `copy`, `openDocument`, and `remove` are loopback-pinned because a Profile names the plugins and default permission a Session runs. `list` stays ordinary so clients can offer Profile ids and trust at Session creation. Containment remains a property of the id (`[a-z0-9][a-z0-9-]*`), and shipped Profiles remain read-only.
...
**The Profile id is model-visible and durable.** It determines the tool set, prompt, hooks, and default permission, so a resumed Session restores the same composition. It rides the Session header beside `cwd`, and summaries carry it as `agentProfile`.

**A durable header field must be mapped by every backend.** The JSONL header, SQLite `sessions` row, derived query index, and cold summary all carry `agentProfile`. Its tests cross real stores rather than proving only the declaring type.

**Profile choice belongs to Session creation.** The Profile seat starts a Session through the workspace service. That service reuses a blank Session only when both workspace and Profile match; otherwise it creates one. A running Session exposes its Profile as a read-only label.
