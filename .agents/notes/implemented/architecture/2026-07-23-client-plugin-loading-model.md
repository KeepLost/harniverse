...
The lower layer supplies four capabilities: externals (the platform list), remote arrival (a same-origin aggregate bootstrap plus independent plugin scripts, both registering lazy factories), versioning (content-hash revs), and hot update (invalidate/prefetch).
...
The manifest owns the package's loading contract: its `inject` dependency edges, plus the optional `immediately` per-bundle fallback mark. The composing app owns only the roster.
...
Initial arrival uses the graph's `bootstrapUrl`, a same-origin external classic `<script src>` containing every graph row's factory registration. The browser owns the network request and script execution; the node is removed as soon as `load` or `error` settles. Successful settlement requires every graph factory id to exist in the module table, or aggregate arrival fails. The shell then falls back to independent row scripts: it awaits all `immediately` rows as the synchronous infrastructure barrier, while entry imports load any remaining missing factory and own loud failures. Registration through either path still does not run a factory, so the side-effect boundary remains first materialization.

The shared tsdown preset emits `client.js.map` for every plugin and rewrites first-party source paths into the browser-resolvable repository shape `/packages/<group>/<package>/src/...`. Other workspace sources inlined into a bundle likewise resolve to their `packages/` owner, while dependency paths remain unchanged; `sourcesContent` carries the source, so the host only serves the map at `/plugins/<id>/client.js.map` and exposes no source route. The aggregate strips each bundle's `sourceMappingURL` comment to prevent initial source-map request fan-out; direct fallback and HMR scripts retain their maps. The Vite shell also emits source maps for shell code.

Each row's `rev` remains its script URL's query parameter and content-consistency anchor; individual bundles and maps use `no-cache`. The row set hashes into the graph `rev`, which addresses one gzip-capable bootstrap artifact with immutable caching. The authenticated Host therefore admits one initial plugin script instead of one script and map request per plugin. An external script's `error` event exposes neither response status nor body, so failure diagnostics name only the URL; the same-origin host and build-stamped handoff ids form the identity boundary, while post-`load` factory-presence checks reject incomplete artifacts.
...
**Phase one — the module face.** The shell builds the module system over the graph and loads `bootstrapUrl` in parallel with Context/Loader setup. The aggregate registers every factory, then verifies the complete graph before entry creation. If aggregate arrival fails, the shell prefetches each `immediately` row independently and swallows individual failures so phase two's import can retry and report them loudly. `immediately` is a fallback barrier mark — not an identity or activation order. Infrastructure plugins (connection, runtime, ui-theme, i18n, plus hmr) declare it so their synchronous require edges remain safe when the aggregate is unavailable; UI entries use their independent import path only if still missing.
...
One governance implementation runs on both sides of the wire; the browser-specific layer is one module system plus one reload plugin. Plugin packages have one shape, so the purity gate covers them all. Dependency edges and the fallback boot tier live with their owners — the manifests — while the composing app holds only the roster. The drift classes stay structurally closed: share-list hand-sync, load-order coupling, cross-plugin imports, roster/tier double bookkeeping. Browser-native external scripts keep one replaceable `loadBundle` hook for aggregate, fallback, and HMR arrivals.

Costs accepted: the vendored Loader carries idle machinery in the browser (EntryTree persistence is a no-op, groups/isolation unused); every plugin edit in dev pays a bundle rebuild plus fiber remount; graph `inject` rows are informational — activation truth is service-level — so a mismatch appears at the settled sweep, not at graph validation; the three not-yet-promoted libraries keep their static-import exports until their DI conversions land; every bundle gains a source-map artifact, but initial aggregate stacks do not map through those per-plugin maps; composing a changed graph reads and compresses all bundle bodies synchronously; and external-script failures provide only coarse URL diagnostics instead of the HTTP status available to an explicit fetch.
...
| One initial external script and source map per plugin | Multiplies authenticated admission and audit persistence across the graph; one revisioned external aggregate preserves factory boundaries with one initial request |
