...
共享 Web 组合把输入触发 Provider 和命令 source 标为关键启动插件。任一注册缺失时，可交互的 composer 都可能把首次 `/goal` 当成普通提示词；稍后加载延迟插件无法恢复这条命令。包自身的启动声明复用现有 Loader 屏障，不向外壳添加命令逻辑。可选呈现继续延迟加载，命令目录就绪仍由命令 source 现有的等待分发约定负责。
...
真实组合的目标栏回归会拦住延迟引导响应，立即提交 `/goal`，并要求没有普通用户消息节点。释放可选呈现后，测试要求命令气泡、活动目标快照及连续两次清除后的收敛行为。原始声明会产生一个普通用户消息节点并使回归失败；两个关键启动声明通过该回归及相邻的命令呈现场景。这复现了与偶发目标栏 CI 超时一致的启动风险；原始失败运行没有保留足够的浏览器状态，无法确认其确切原因。
