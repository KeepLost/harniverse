...
Compact-basic 拥有消费方策略。顶层字段定义 Profile 默认值；`modelPolicies` 包含以精确 `{ provider, model }` 组合为键的部分覆盖。根层专用的 `dsh-compaction-settings` 插件拥有一个可选的进程级 `thresholdRatio` 覆盖，但不会注册第二个 `ctx.compaction` 服务。有效优先级依次为精确目标策略、全局设置、Profile 默认值和 `0.8`。因此全局字段缺省时会保留 Profile 配置。重复目标、未知字段或无效字段都会让插件加载失败。保留策略默认为 `retainRatio: 0.16`；调用方也可以改用绝对 `retainTokens`，但两种保留形式互斥。完成继承后，如果保留比例不小于阈值比例，插件也会加载失败，因为任何模型容量都无法让该策略有效。若全局阈值不高于某个 Profile 的比例保留值，则只与该 Profile 不兼容；该 Profile 会保留自身的有效阈值。

对于主动压力检查，compaction-basic 读取最新持久请求路由，对其有效目标策略取快照，解析适配器容量，再把比例缩放为 `ResolvedCompactSpec`。同一次决策会在异步元数据查询及其启动的压缩中使用该快照，因此并发设置写入只会应用于下一次决策。每次检查都会重新解析，因此同一会话切换提供方或模型后，容量与策略会立即变化。若绝对保留预算不小于缩放后的阈值，系统会在目标容量首次允许比较两者时失败。
...
服务测试覆盖与适配器内部状态分离的上下文元数据、无效适配器输出、目录独立性与默认缺失行为。适配器测试覆盖 DeepSeek 的精确容量、默认容量、未列出模型解析及无效容量，以及 pi-ai 的精确描述符解析。压缩测试覆盖比例缩放、全局／Profile／精确目标优先级、全局设置缺省与实时更新、保留冲突、单次决策快照、加载期拒绝无效合并比例、运行时校验绝对预算、相同模型 id 的提供方切换、目标专用警告抑制与不依赖容量的溢出恢复。Loader fixture（测试前置数据）会拒绝已经移除的 token-meter 容量设置，示例则在适配器上配置容量。
...
- **让每个 Profile Provider 注册设置命名空间**——不予采纳，因为多个活动 Agent 会争用同一个进程级命名空间。根层专用设置插件拥有用户策略，scoped Provider 则保持彼此独立。
- **给全局 schema 设置 `0.8` 默认值**——不予采纳，因为解析后的设置值将无法区分缺省与用户覆盖，并会静默替换 Profile 的组合阈值。
...
- 实时全局阈值会改变下一次压力决策，而不会修改已经在等待模型元数据的决策。
