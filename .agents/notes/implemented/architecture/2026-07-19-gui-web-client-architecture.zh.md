...
- **Session**（session.ts）：懒建、常驻——建成后在后台持续吃帧，切走切回秒显。操作面：`prompt`/`cancel`（RPC 透传；失败落进快照的 `promptError`）、`open`（拉尾页 history，幂等）、`loadOlder`（向上翻页，防重入），以及显式 `resync`（清窗口重跑 open）。订阅面：`subscribe`/`getSnapshot`（恒返缓存引用）——`implements ObservableSnapshot<ConversationSnapshot>`，构造时挂 `useSelector = bindSnapshotSelector(this)`，Session 本身就是 uSES 源。帧分发是一个 switch：`session/event` 帧按 seq 去重，open 在途时缓冲，否则追加 + 增量投影。已打开的普通 Session 只把连续窗口尾部导出为 mux 游标；前向 history 修复缺口时不会丢弃已加载的更早事件，而订阅到的 Host 尾部低于本地尾部时会强制执行持久化支持的重新建基。
...
- **ConnectionController**（在 `packages/client/connection`）：开 mux/host 双流、for-await 泵入，代际围栏之内指数退避重连（500ms 翻倍至 10s 封顶、抖动、无限重试）；sinks 单向注入（Controller 不认识 Session）。每个 generation 开始前，它会采样 sink 当前的 mux 游标表。重连会刷新列表／目录基线、保留已打开的权威窗口供 Host 回放，并只重启由失败 generation 拥有的操作；Host 回滚仍会调用显式 Session resync。对象层只面向 `IApiClient`；Web 承载以 HTTP POST 载两个 client→server 象限、以[每逻辑流一条 WebSocket](2026-08-04-websocket-downlink-carrier.md)载两个 server→client 象限，客户端类族归分层笔记属地。
