...
这些常量置于 Service Definition 中，位于 worker 线程与 Python 进程提供方之上：该约定与语言无关，且由高于任何单一语言的层级拥有。违反它的 Service Provider 才是 bug，而共享集合正是复审者查看「可移植」含义的地方。

## Scope

本决策拥有 Service Definition 扩展与每个提供方对它的采用。`py-types` 渲染器与 Code Mode 的语言分发归[语言分发 note](../feature/2026-07-31-code-mode-language-dispatch.md) 所有；[Python 提供方](../feature/2026-08-25-python-code-runtime-provider.md)导入并执行这些共享常量。
...
获得：一个地方——Service Definition 包——定义什么是可移植绑定名称，两个可执行后端都通过导入执行同一约定。在一个后端上有效的命名空间列表在所有后端上都有效，这是可验证的，而非取决于调用方测试了哪个后端的巧合。
