...
- The stateful preset plugins (`plan-mode`, `token-meter`, `compaction-basic`) already key state by `Session`/`Agent` — they predate presets. Sharing one instance is a return to their design, not a rewrite. `jobs-local` shared that property and has since left the preset plane entirely: shell and terminal producers outside its realm (`tool-bash`, `tool-terminal`) resolve the registry with `ctx.get`, which an entry-local realm hides from them, so it is composed on the host plane and only the model-facing `tool-jobs` row stays per preset.
