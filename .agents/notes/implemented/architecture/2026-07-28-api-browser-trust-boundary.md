...
Reachability remains the webserver binding's policy (`host: 127.0.0.1 | 0.0.0.0`), while the separate [public-key Grant authentication service](2026-08-17-public-key-grant-authentication.md) establishes caller identity. This fence remains a confused-deputy defense, not an authentication or authorization decision. The old guard's loopback-socket check was dropped rather than generalized: with binding expressing reachability and `trustedHosts` naming remote authorities, the socket address adds nothing the header fence and authentication layer do not already cover.
...
- Non-loopback deployments must configure TLS and trust their serving authorities or requests are refused. The dsh CLI keeps its advertised HTTPS LAN URL working by deriving the machine's LAN IP literals into the connection row and offers `dsh web --trusted-host` for named authorities; compositions the CLI does not boot declare `trustedHosts` themselves. Non-browser automation rides the same fence: loopback, a derived LAN IP, or a declared authority passes; an undeclared DNS alias is refused.
- Clients must label POST bodies `application/json` (ours always did; raw-fetch tests gained the header).
- Authentication applies after the trust fence on loopback and declared non-loopback authorities; bypass is an explicit process mode rather than an implication of `0.0.0.0`.
