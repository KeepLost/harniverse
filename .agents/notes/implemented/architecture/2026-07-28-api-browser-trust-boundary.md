...
Reachability remains the webserver binding's policy (`host: 127.0.0.1 | 0.0.0.0`), while the separate [inbound authentication service](../feature/2026-08-16-inbound-network-authentication.md) establishes caller identity. This fence remains a confused-deputy defense, not an authentication or authorization decision. The old guard's loopback-socket check was dropped rather than generalized: with binding expressing reachability and `trustedHosts` naming remote authorities, the socket address adds nothing the header fence and authentication layer do not already cover.
