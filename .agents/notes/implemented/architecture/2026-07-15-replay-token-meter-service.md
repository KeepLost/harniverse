...
Automatic compaction uses one unified measurement for each threshold-and-retention decision. The region transaction measures after appending its durable `compaction/start` lock and again after asynchronous summarization, then compares the detached surface-node vectors. When the baseline is matching Provider usage, the transaction projects it onto the selected prefix by subtracting the unselected nodes' heuristic price; the default summarizer reuses that anchor only for the same provider/model and otherwise falls back to direct estimation. An intervening surface mutation prevents replacement; `logRevision` may advance for unrelated log-only facts without invalidating an unchanged selected span.
