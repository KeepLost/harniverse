...
Both specs were rewritten to drive real contexts — `SessionStore`, `InvariantRegistry`, then the companion — and to assert through `expect(() => session.append(...)).toThrow(...)`. The coverage lane's branch accounting for this file is unstable independently of the channel move: docs-only commits flipped it between passing and failing on identical source, and a local run of the context-reset and scheduler suites together once produced NEGATIVE branch counts (`[5,-3]`, `[22,-21]`), which correct accounting cannot produce. `istanbul-lib-coverage` merges branch arrays by index, so two differently shaped instrumentations of one file add counts into mismatched slots. The custom uncovered-locations reporter only prints entries whose count is exactly `0`, so those negatives never surfaced in CI logs while the zeroed neighbours were reported as uncovered paths. `context-reset` therefore keeps its whole fold in one `validateCandidate` frame — the smallest accounting surface that still measures every decision — instead of spreading it across helper frames. The merge defect itself is a toolchain issue and is not fixed here.
