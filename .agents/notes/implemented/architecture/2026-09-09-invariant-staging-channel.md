...
Both specs were rewritten to drive real contexts — `SessionStore`, `InvariantRegistry`, then the companion — and to assert through `expect(() => session.append(...)).toThrow(...)`. The coverage lane still records a branch measured only by its throwing exit unreliably — a docs-only commit flipped this file back to failing after the move — so `context-reset` resolves every decision to a message first (`markerProblem`, `pendingProblem`) and keeps a single `fail()` call site, which carries the one remaining ignore for the throw-unwinding arms. That shape leaves each decision branch measured by an ordinary return.
