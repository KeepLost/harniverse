...
The consumer job owns the [single Linux build](../process/2026-07-30-independent-ci-consumer-build.md), so `apps/web/dist` and the package `lib/` directories remain in its workspace for the browser suite. CI installs Chromium and its system dependencies at the Playwright version in the lockfile. The pull-request cache step restores a compatible operating-system entry when one is visible and saves the current merge-ref entry after a miss, so a rerun does not repeat the browser download.
...
The gate runs only in the Linux consumer job: these scenarios target POSIX, and the other pull-request jobs do not provision Chromium. A PR's `all checks passed` verdict already depends on the consumer job, so a browser compare failure blocks the merge without requiring a new branch-protection check name.
