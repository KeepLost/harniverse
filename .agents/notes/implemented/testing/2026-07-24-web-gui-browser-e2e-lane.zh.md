...
调用方在 `captureStableAria` 之前确认场景数据已就绪；相同的规范化样本只说明渲染稳定，并不说明在途命令或历史 I/O 已完成。[就绪条件决策](../bug-fix/2026-09-11-ci-readiness-boundaries.md) 负责具体等待条件，以及 Cordis 审批后续处理独立完成一个轮次的约定。
...
**客户端 `data-dsh-busy` 安定信号。** 暂缓：host 侧 `whenIdle` 屏障配合场景数据就绪条件和稳定 DOM 轮询，足以覆盖当前场景。必要状态在 DOM 中不可观察时，再重新考虑；若加载完成的内容已可观察，相同的加载占位内容不足以成为引入全局 busy 协议的理由。
