...
spill 写入方在目录校验和独占打开之间共用一次 ENOENT 重试。它重建会话目录并重新执行完整的路径、所有权、权限和包含关系检查，在重建和打开前检查取消信号。单次启动清扫只访问每个会话目录一次，因此一次重试即可覆盖其修剪操作；第二次目录消失会传递文件系统故障。安全检查失败和其他 I/O 错误仍会终止保存。
...
**保存前等待清扫，或无限重试。** 等待会使写入延迟取决于整个根目录的清扫。无限重试会在外部反复删除目录时让保存持续运行。一次可取消的重试既覆盖本服务的一次性修剪，也限制文件系统尝试次数。

**projection-cache 跨版本读兼容（#7 后半）。** 有证据的 NO-OP：我方缓存只有单一版本代际、无 lineage 字段、无按记录布局，仓库 pre-release 立场明确拒绝为旧格式建兼容层。

## 结果

[spill 故障回归测试](../../../../packages/spill/spill-local/tests/store-faults.spec.ts)在目录创建和校验之间确定性地运行真实清扫，然后要求产物可读。测试还固定了重试耗尽、校验与打开共用重试次数、重建前取消，以及拒绝不安全的替换目录和非缺失类校验错误的行为。宿主特定的文件系统行为仍需原生平台 CI 验证。
