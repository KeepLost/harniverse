...
- **Convert every job to `cache: pnpm`.** Rejected because the primary jobs intentionally use `restore-keys` prefix fallback across lockfile changes, which setup-node's built-in cache does not expose.
...
- **A composite action wrapping action-setup + setup-node.** Rejected for now: the remaining per-job variation (Node-version matrices, prefix-fallback caches, and cache-less native jobs) is deliberate policy, not boilerplate — a wrapper would grow mirroring inputs or flatten a real distinction, and the two-line pair is already near the floor.
