...
The published Web guide owns expanded enrollment and remote-serving instructions. The model guide owns provider-specific credentials, endpoints, and modality configuration. The root remains self-contained for first use but links those descendants rather than duplicating their advanced cases. [Package references](../../../../packages/README.md) and [subsystem references](../../../../docs/subsystems/README.md) own current contracts; the [architecture](../../../../docs/architecture.md) maps composition and extension points.

Container onboarding has a repository-owned `pnpm run web:container` path. It persists a development CA and server certificate under `$DSH_HOME/tls`, installs the CA in a Linux container trust store when the image permits it, supplies the non-loopback HTTPS flags, and preserves the normal authenticated owner-enrollment flow. `DSH_WEB_TLS_HOSTS` supplies certificate SANs and Host trust for browser authorities such as a Tailscale IP or MagicDNS name. Exact cross-origin browser UIs can be added with `DSH_WEB_TRUSTED_ORIGINS` or `--trusted-origin`; Host trust remains mandatory. Enrollment accepts human-readable Unicode device names; expected input and name-conflict failures reach the browser as actionable responses, while unexpected registry failures remain 500 responses with their cause in the server log. The Web composition mounts the Cordis console exporter for runtime warnings, trust rejections, and accepted/rejected connection and authentication events. Logs include non-secret peer, Host, Origin, path/channel, and Grant identity fields without credentials or request bodies; privacy-minimal admission outcomes remain in `$DSH_HOME/auth/access.jsonl`. Browser trust outside the container remains an external host boundary; `pnpm run web:container:trust` installs the generated CA through the host system trust tool when that host can access the mounted home, while deployments can terminate TLS at a trusted reverse proxy instead.
