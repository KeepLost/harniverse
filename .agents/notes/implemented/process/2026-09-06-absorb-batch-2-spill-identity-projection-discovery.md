...
The spill writer shares one ENOENT retry across directory validation and exclusive open. It recreates the session directory and repeats the full path, ownership, permission, and containment checks, observing cancellation before recreation and before open. A single startup sweep visits each session directory once, so one retry covers its prune; a second disappearance propagates the filesystem failure. Safety failures and other I/O errors remain terminal.
...
**Wait for cleanup before saving, or retry indefinitely.** Waiting couples write latency to the whole root sweep. Unlimited retries can keep a save alive under repeated external deletion. One cancellable retry covers the owned one-shot prune while bounding filesystem attempts.
...
The [spill fault regressions](../../../../packages/spill/spill-local/tests/store-faults.spec.ts) deterministically run the real sweep between directory creation and validation, then require a readable artifact. They also pin retry exhaustion, the shared validation/open retry budget, cancellation before recreation, and rejection of unsafe replacement directories and non-missing validation errors. Native platform CI remains necessary to qualify host-specific filesystem behavior.
