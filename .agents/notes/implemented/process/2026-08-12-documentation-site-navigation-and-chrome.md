...
The navigation-bar title is the plain `Harniverse` product name and preview tag inlined into `siteTitle`, which VitePress renders as HTML. The [Harniverse RC frontend branding note](../feature/2026-09-25-harniverse-rc-frontend-branding.md) partially supersedes the former wordmark and favicon realization; this note remains authoritative for navigation and chrome behavior. The sidebar scrollbar rests invisible and appears while scrolling, marked by a `data-` attribute rather than a class because Vue rewrites `class` wholesale when it patches the element.
...
The site title is a plain Harniverse text lockup, and the documentation site declares no product favicon. The frontend branding note owns the repository artwork and client presentation; this note continues to own the site navigation and chrome implementation.
