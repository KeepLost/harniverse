...
任何被引用的公开值都会成为可读的产物内容，命名错误即泄露信息。构建选择在产物生成时冻结；需要部署后变化的设置必须使用带校验的运行时配置。该记录把环境绑定到字节和源码 revision，而不是绑定到可信构建者：它既不能证明工作树干净，也不能证明构建可复现或已签名，且可以在修改后的输出上重新写入一份新记录。已发布的侧栏使用 Harniverse 鲸鱼标识作为紧凑品牌资源；Web shell 也通过 favicon、认证界面、启动加载页和可安装 manifest 使用同一组资源。
