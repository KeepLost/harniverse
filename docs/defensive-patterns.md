...
`agent.followup()` has no per-message result; a background job's completion races turn boundaries; `reader.close()` fires for both EOF and disposal. The Web control API can fold one admitted `MessageId` into queued, claimed, discarded, or settled lifecycle state, but settlement means only that its shared turn ended. Never treat `agent/status`, `whenIdle()`, a matching `turn/end`, or the last assistant message as that follow-up's result: several queued follow-ups, steering, and injected work may share one `running` interval and one turn. An automation caller that truly owns a run must define its interval explicitly and describe selected output as interval-wide rather than causally attributed to one message. The guard cuts both ways: if the awaited transition can never occur, the wait hangs, so handle the "nothing to wait for" branch explicitly.
