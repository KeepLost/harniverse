...
A **profile** is a named composition stored in the Harness home. It lists the bundles it stacks, holds any out-of-tree plugins it installs, and keeps the user's own `cordis.patch.yml`. `web`, `headless`, and `auth` ship as templates.
...
[`dsh-base`](../packages/bundle/base/README.md) is the first layer of agent profiles: model adapters, tools, persistence, sandbox and approval policy, settings, credentials, telemetry. [`dsh-web-app`](../packages/bundle/web-app/README.md) adds the browser application; [`dsh-headless`](../packages/bundle/headless/README.md) adds a one-shot runner with no server; [`dsh-auth-app`](../packages/bundle/auth-app/README.md) is a standalone one-shot authentication management profile with no agent or network service.
...
`turn/*`, `step/*`, `user/message`, `assistant/*`, and `tool/*` are durable session events; the rest are live extension points across three domains. `agent/pre-step`, `agent/request`, `llm/stream`, and the four transformable `tools/*` events are waterfalls, whose listeners must call `next()` to delegate; `agent/turn-stopping` is serial and has no `next()`.
...
The session log is the source of the context the model sees. `deriveMessages()` projects model history from it, and raw `assistant/chunk` events preserve replay and UI fidelity. Fork, resume, transcripts, telemetry, and persistence all derive from this stream. The non-surface `llm/wire-attempt` event adds adapter transport facts without copying conversation content. Windowed restoration preserves absolute event identities and request boundaries; persistence and projection plugins own checkpoint recovery, not the agent loop.
...
| Change capabilities for future sessions | edit the Agent Profile assembly; its recipe plan compiles selection, immutable-member visibility, and owner-declared configuration into the next generation |
