...
| `@deepseek-ai/dsh-tools` | `run_code` | `ctx.tools`, `ctx.ptcRuntime (execution time)`, `ctx.systemPrompt` | `tool/call`, `one tool/code-dispatch-start + tool/code-dispatch pair per bridged sub-call`, `tool/result` | - | Owned by the tool registry as a reserved transport outside filterable capability layers under `mode: code` / `mode: both` (see the PTC Agent Note). Under `code` it is the registry's only wire contribution; the other visible capabilities are declared in a generated SDK section in the loaded runtime's language, and a program calls them through bindings scheduled under the native concurrency contract (submission-ordered starts and policy; concurrency-safe bodies overlap up to `maxParallelSubCalls`) that re-enter the complete guarded tool pipeline and link each nested execution to this outer result. |
...
| `@deepseek-ai/dsh-tool-pwsh-persistent` | `pwsh` | `ctx.tools`, `ctx.terminals`, `an owning Agent at execution time` | `tool/call`, `PTY shell state`, `tool/result` | - | One owner-isolated persistent pwsh tool; explicit deployment composition supplies a PowerShell-dialect PTY backend and may override the model-facing environment description. |
...
Read a PNG/JPEG/WebP/GIF file and return the image itself. A path without a file extension is accepted; the format is detected from the file content, so normalized attachment paths can be passed directly without copying or renaming. Requires the current model to accept image input.
...
Schedule one prompt to run later in this session or in a dedicated job session, optionally on a repeating interval and optionally with a fresh context each run. Exactly one timing parameter (run_at or after_minutes) is required; add every_minutes to repeat.
...
Cancel one scheduled task owned by this session.
...
List scheduled tasks owned by this session, earliest first.
...
Source: [`packages/spill/tool-result-artifacts/src/index.ts`](../packages/spill/tool-result-artifacts/src/index.ts)
