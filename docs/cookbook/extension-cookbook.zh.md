...
`system-prompt/assemble` 是一个专家协作式的整体装配变换：其返回的装配结果具有权威性，因此监听器作者有责任保留活跃的 PTC 和结构化输出协议的贡献。对于需要在展示、查找和执行之间保持对齐的工具过滤，优先使用 `ctx.tools.restrict()`。
...
| 上下文压缩（context compaction）（自动 + 手动） | `ctx.compaction` seam + 随附的 `dsh-compaction-lossless` Provider；自动压力检查运行在串行 `agent/pre-step`，标准溢出恢复运行在 `agent/request-error`，手动调用方使用同一服务，历史 Consumer 提供有界回溯（[summary DAG Agent Note](../../.agents/notes/implemented/feature/2026-08-18-summary-dag-compaction-history.md)） |
