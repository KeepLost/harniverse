...
`agent.followup()` 没有逐消息结果；后台任务的完成与轮次边界存在竞争；`reader.close()` 在 EOF 和 dispose（资源释放）两种情况下都会触发。Web 控制 API 可以把一个已获准 `MessageId` 折叠为 queued、claimed、discarded 或 settled 生命周期状态，但 settled 只表示其共享轮次已经结束。切勿把 `agent/status`、`whenIdle()`、匹配的 `turn/end` 或最后一条 assistant 消息当作该 follow-up 的结果：多条排队 follow-up、steering（中途引导）和注入工作可能共用同一个 `running` 区间及轮次。真正拥有一次运行的自动化调用方必须显式定义其区间，并将选取的输出描述为整个区间的输出，而不是把因果关系归于单条消息。这条守则是双向的：如果等待的转换永远不会发生，等待就会挂起，因此应显式处理「无需等待」的分支。
