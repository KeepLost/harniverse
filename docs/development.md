...
`pnpm run build` embeds the caller's exact `DSH_CLIENT_*` values into the Web shell and every dynamic client bundle, and uses no public client value when the caller sets none. `pnpm run build:harniverse` is the cross-platform local equivalent of the CI and release artifact build. Each successful complete build writes a gitignored record binding those values and the source revision to the generated browser artifacts; `release:pack --family dsh` rejects a missing record, a different profile, or artifacts changed by a later partial build. The [client build profile note](../.agents/notes/implemented/process/2026-08-23-harniverse-client-build-profile.md) owns this decision.
...
Read [Harniverse version and branch policy](release-policy.md) before choosing a version, admitting unfinished work, cutting a stabilization branch, or publishing a candidate. It defines the complete workflow for a maintainer or agent starting without conversation history.
