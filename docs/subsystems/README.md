...
| [authentication.md](authentication.md) | inbound HTTP/WebSocket admission, browser sessions, credential revisions, and targeted revocation |
...
| [supervision.md](supervision.md) | per-session human-supervision modes, durable `supervision/mode`, policy inheritance, and the `ctx.supervision` service |
...
| [notification.md](notification.md) | the outbound notification seam: stable envelopes and event projections, backend ownership, HTTP outbox delivery, and authority/privacy limits |
| [model-policy.md](model-policy.md) | Session-scoped Model Profiles, ordered Model Routes, durable authorization snapshots, target selection, and cross-model fallback |
