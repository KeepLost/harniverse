...
The executable is resolved in the Session's own execution environment before anything launches: `executablePath` when the operator set one, otherwise `browserCandidates` in order — Chrome, Chromium and Edge under their Linux names plus their macOS and Windows install paths. A host with none of them is a supported deployment, not a failure: `environment` reports `available: false` with a reason naming exactly what the probe looked for, and `create` fails `browser-unavailable` carrying the same text, so the panel can name the remedy instead of showing an opaque failure. The client offers the reader their own browser for conversation links in that state.
...
Source: [`packages/api/browser-controller/src/index.ts:126`](../../packages/api/browser-controller/src/index.ts)
