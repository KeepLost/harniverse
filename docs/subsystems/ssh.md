...
Administrative RPC uses the helper's SSH exec streams. Ordinary stdin, stdout, stderr and terminal output use separately authenticated forwarded Unix sockets. Each forwarded stream has its own SSH channel window; paused program output does not share the administrative window. All channels still share connection bandwidth and transport failure.
