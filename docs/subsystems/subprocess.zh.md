...
`spawnTerminal(spec)` 是非管道进程原语。提供方分配控制终端，并负责 UTF-8 文本传输、前台进程组检查与信号发送，以及一项须等待的 TERM→KILL 操作；该操作会使提供方仍可观察到的每个会话成员完全停稳，提供方则会记录执行基底特有的可观察性限制。本地提供方以 pid 加启动身份围栏保护每个已收养进程：Linux 读取 `/proc` 启动 tick，macOS 读取 `ps` 启动文本，Windows 则在枚举 Toolhelp32 进程树时读取 `GetProcessTimes` 创建时间。PTY 后端仍负责提示符检测、就绪推断、scrollback、沙箱策略和持久会话所有权；普通 `spawn()` 无法重建控制终端语义。
