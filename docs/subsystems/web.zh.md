...
提供方 seam 接收一个标量 `query`；面向模型的 `dsh-tool-web` 消费方要求非空的 `queries` 数组，并受 `searchMaxQueries`（默认 `4`）限制。消费方在验证上限后移除完全相同的查询字符串，并发执行不同查询，逐个通过 `ctx.web.search()` 传递。`maxResults` 仍是消费方自有的来源上限（`searchMaxResults`，默认 `8`），通过 seam 传递并在返回时强制执行——如果提供方返回超量，seam 截断 `sources[]` 并设置 `truncated`。

### 批量模型搜索

工具会在执行提供方之前拒绝缺少、空、非字符串、空白、混合旧式 `{ query }` 以及超限输入。一个兄弟查询失败时，中止共享的批处理信号，等待所有已启动的搜索 settle，然后返回第一个失败；调用方取消沿用同一路径。成功来源按完全相同的 URL 去重，并按查询顺序的 round-robin rank 合并至 `searchMaxResults`。非空提供方答案按查询顺序放在 `### <query>` 标题下。单项数组仍直接使用提供方结果，而提供方 API 仍保持单查询。
