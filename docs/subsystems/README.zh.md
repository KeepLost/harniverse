...
| [governor.md](governor.md) | 资源治理器：样本与视图、配额状态与违约记录，基于计量事件 |
| [queue.md](queue.md) | 会话消息队列：storage domain 之上的 topic、消息与订阅关系 |
...
| [authentication.md](authentication.md) | 入站 HTTP/WebSocket 接入、浏览器会话、凭据 revision 与定向撤销 |
...
| [ptc-runtime.md](ptc-runtime.md) | PTC 运行时 seam：`CodeRunRequest`/`Result`、绑定命名空间、捕获日志、`CodeRunFailure` 分类体系 |
