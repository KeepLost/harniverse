...
const NO_MODEL_EXPERIENCE_SECTION: Readonly<Record<string, string>> = {
  ...
  'packages/util/lazy-require': 'The package is a load-timing utility; it changes when a dependency loads, never anything a model sees.',
...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/auth/authentication': { kind: 'none', reason: 'The admission seam runs before external callers can reach model-facing operations and registers no model context.' },
  'packages/auth/authentication-local': { kind: 'none', reason: 'The local admission provider stores and verifies network credentials without registering model context.' },
  ...
  'packages/code-runtime/code-runtime-ptc': { kind: 'indirect', reason: 'The fresh-process backend delegates model rendering to Code Mode in dsh-tools.' },
  ...
  'packages/client/store': { kind: 'none', reason: 'Browser-side state primitives; registers nothing model-facing.' },
  'packages/util/http-proxy': { kind: 'indirect', reason: 'Outbound routing library; it changes which network path a request takes without touching model-visible vocabulary.' },
  'packages/ssh/ssh': { kind: 'none', reason: 'Host aliases, authentication and stream capabilities are private deployment details; consumers own every model-visible operation.' },
  'packages/api/terminal-controller': { kind: 'none', reason: 'The browser-terminal Remote serves web panels and registers no model-facing vocabulary.' },
  'packages/api/browser-controller': { kind: 'none', reason: 'The host-browser Remote serves the browser panel and registers no model-facing vocabulary; the model\'s own fetching lives in the web seam.' },
  'packages/util/deque': { kind: 'none', reason: 'In-process collection primitive; it registers nothing model-facing and never crosses a wire.' },
  'packages/ssh/fs-ssh': { kind: 'indirect', reason: 'The remote filesystem adapter delegates model rendering to the filesystem tools.' },
  'packages/ssh/subprocess-ssh': { kind: 'indirect', reason: 'The remote subprocess adapter delegates model rendering to the bash, terminal and LSP consumers.' },
  'packages/ssh/sandbox-ssh': { kind: 'indirect', reason: 'The remote confinement adapter delegates mode, denial and enforcement rendering to the sandbox consumers.' },
  ...
  'packages/client/authentication': { kind: 'none', reason: 'Browser credential renewal and request admission register no model context or tools.' },
  'packages/client/ui-connection-status': { kind: 'none', reason: 'Read-only browser connection indicator; it neither initiates operations nor registers model context.' },
  'packages/client/ui-queue': { kind: 'none', reason: 'The panel tab consumes the queue Remote only; delivery injection belongs to dsh-queue and its tools.' },
  'packages/client/ui-governor': { kind: 'none', reason: 'Browser-side governor board over the Remote; quota negotiation by the model rides the host package resource-quota tool.' },
  ...
  'packages/client/ui-scheduler': { kind: 'none', reason: 'Browser-side projection of scheduler Remote verbs for a human; dsh-tool-scheduler owns the model-facing behavior.' },
  ...
  'packages/context/file-reference': { kind: 'none', reason: 'The discovery Definition registers no prompt, tool, or model-facing context.' },
  ...
  'packages/bundle/auth-app': { kind: 'none', reason: 'The one-shot management app neither creates an Agent nor contributes model context.' },
  ...
  'packages/session-query/session-delivery': { kind: 'none', reason: 'The delivery Definition registers no prompt, tool, or model-visible content.' },
  'packages/session-query/session-delivery-local': { kind: 'indirect', reason: 'The local Provider admits messages while its Consumer owns model-facing schemas and acknowledgements.' },
  ...
  'packages/capability/capabilities': { kind: 'indirect', reason: 'Native registry adapters own the model-visible definitions that Profile composition can select.' },
  'packages/host/capability-management': { kind: 'indirect', reason: 'The authorized Remote changes Profile composition while native registries own model rendering.' },
  'packages/client/ui-settings-capabilities': { kind: 'none', reason: 'The browser Settings contribution edits Host composition without registering model context.' },
  ...
  'packages/test-support/remote-mock': { kind: 'none', reason: 'The carrier double substitutes the web transport without invoking a real model.' },
  ...
  'packages/boot/hmr-coordination': { kind: 'none', reason: 'Boot-lifecycle reload coordination contributes no model-visible context.' },
