...
function ciBuildGate(id = 'build', options: Partial<Gate> = {}): Gate {
  return pnpmScript(id, 'build', {
    ...options,
    env: { ...options.env, [CLIENT_BUILD_PROFILE_SELECTOR]: 'harniverse' },
  })
}
...
function ciConsumerGates(): Gate[] {
  ...
  return [
    ...
    pnpmScript('node-compat', 'check:node-compat', {
      ...
      env: { [CLIENT_BUILD_PROFILE_SELECTOR]: 'harniverse' },
    }),
...
function docSyncLeafGates(options: {
  ...
  return [
    ...
    pnpmScript('api-catalog', 'verify-api-catalog', { label: 'api catalog' }),
...
function builtBinSmokeGate(needs: string[] = ['build']): Gate {
  return pnpmExec('built-bin-smoke', [
    ...
    'packages/context/session-reference/tests/built-exports.e2e.ts',
    ...
    'packages/code-runtime/code-runtime-ptc/tests/built-lib.e2e.ts',
