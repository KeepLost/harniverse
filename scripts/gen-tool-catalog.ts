...
const TOOL_PACKAGES: ToolPackage[] = [
  ...
  },
  {
    ...
    async mount() {},
    note:
      'Owned by the tool registry as a reserved transport outside filterable capability layers under `mode: code` / `mode: both` (see the PTC Agent Note). Under `code` it is the registry\'s only wire contribution; the other visible capabilities are declared in a generated SDK section in the loaded runtime\'s language, and a program calls them through bindings scheduled under the native concurrency contract (submission-ordered starts and policy; concurrency-safe bodies overlap up to `maxParallelSubCalls`) that re-enter the complete guarded tool pipeline and link each nested execution to this outer result.',
  },
  {
  ...
  },
  {
    ...
    note:
      'One owner-isolated persistent pwsh tool; explicit deployment composition supplies a PowerShell-dialect PTY backend and may override the model-facing environment description.',
  },
  {
