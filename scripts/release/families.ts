...
/** Peer declarations order publication where they do not conflict with installed dependencies. */
...
export abstract class ReleaseFamily {
  ...
  /**
   * Order members so installed dependencies always precede their consumers and
   * peer dependencies precede them where that does not create a cycle.
  ...
  publishOrder(members: readonly ReleaseMember[]): ReleaseMember[] {
    ...
    const checkInstall = (member: ReleaseMember, path: readonly string[]): void => {
      ...
      for (const dependency of edges(member, INSTALL_SECTIONS)) {
        checkInstall(dependency, [...path, member.name])
      }
      installVisiting.delete(member.name)
      installDone.add(member.name)
    }
    for (const member of sorted) checkInstall(member, [])
    ...
    for (const member of sorted) visit(member)
    return ordered
  }

  /**
   * The family members one member declares in the selected dependency sections.
   * @param member - the dependent member.
   * @param byName - every family member by package name.
   * @param sections - manifest sections to inspect.
...
class DshFamily extends ReleaseFamily {
  ...
  readonly tagPrefix = 'harniverse-v'
