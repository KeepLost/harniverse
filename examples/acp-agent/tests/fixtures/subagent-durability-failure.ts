...
export const inject = ['agents', 'sessionPersistence', 'sessionDelivery', 'subagents']
 ...
 *
 *  - `PLACEHOLDER_CHILD_ID` in a scripted `session_message` is remapped to the real
 *    child so both follow-ups queue onto the same live inbox in FIFO order.
 *  - The unknown-id `session_message` (`UNKNOWN_CHILD_ID`) resolves through a
...
export function apply(ctx: Context): void {
  ...
  const originalSubagentFollowup = subagents.followup
  ...
  agents.create = async (options) => {
    ...
    return {
      ...handle,
      async dispose() {
        handle.agent.followup = originalAgentFollowup
  ...
  persistence.load = async (id) => {
    ...
    return originalLoad.call(persistence, id)
  }
  ctx.effect(() => () => {
    agents.create = originalCreate
    persistence.load = originalLoad
    sessionDelivery.deliver = originalSessionDeliver
    subagents.followup = originalSubagentFollowup
  ...
  subagents.followup = (authority, childId, content, options) => {
    const mapped = childId === PLACEHOLDER_CHILD_ID && realChildId !== undefined
      ...
      : childId
    return originalSubagentFollowup.call(subagents, authority, mapped, content, options)
  }

  // Unified session delivery resolves the target before delegating to the
  // subagent runtime, so map the authored placeholder at that seam as well.
  sessionDelivery.deliver = request => originalSessionDeliver.call(sessionDelivery, {
    ...request,
    targetSessionId: request.targetSessionId === PLACEHOLDER_CHILD_ID && realChildId !== undefined
      ? SessionId(realChildId)
