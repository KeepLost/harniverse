...
[`minimal.cordis.yml`](minimal.cordis.yml) is the complete standalone counterpart of the Web `minimal` preset. `DSH_SYSTEM_PROMPT` selects its persona, with `You are a helpful software engineer assistant.` as the fallback. It uses the shared dynamic prompt assembly and mounts no context-compaction plugin. Its model-facing tools are exactly:
