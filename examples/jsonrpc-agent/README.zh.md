...
[`minimal.cordis.yml`](minimal.cordis.yml) 是 Web `minimal` preset 的完整独立版本。`DSH_SYSTEM_PROMPT` 选择它的人设，未设置时使用 `You are a helpful software engineer assistant.`。它使用共享的动态提示词组装，且不挂载上下文压缩插件。面向模型的工具严格只有：
