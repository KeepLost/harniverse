...
Both commands require `DEEPSEEK_API_KEY`. The Web overlay also enables the parent-private `child_profile_define` and `child_profile_list` tools on the delegation row. The [Cordis tool reference](../../packages/extensions/tool-cordis/README.md) defines the tool arguments, lifetime, cleanup, and safety contracts.
