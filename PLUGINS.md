...
| Parent-private Child Profiles | `dsh-subagent` (official, modified; resolved profile contract and durable descriptor snapshot) | `dsh-subagent-in-process-driver` (official, modified; native workspace/tool application) | `dsh-tool-subagent` remains deferred for the authorized define/list surface | Foundation shipped: host-resolved profiles carry only opaque harness/model-route/capability references, reject requests outside the parent grant, enforce descendant workspace paths and profile-bounded Tool filters, and persist an immutable revision/digest snapshot. Parent-bound profile registry, model-facing define/list operations, model-route resolution, MCP/Skill mounting, and SDK handshake remain deferred. |
...
| Parent-private Child Profile foundation | `dsh-subagent`, `dsh-subagent-in-process-driver`, `dsh-tool-subagent` (consumer surface unchanged) |
