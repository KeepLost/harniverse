...
| Agent Profile plugin composition | `dsh-capabilities` (new; recipe catalog, selection store, and planner) | `dsh-agent-presets` (official, modified; static recipe compiler and generation runtime), `dsh-host-capability-management` (new; authorized Remote) | `dsh-mcp-client` (official, modified), `dsh-client-ui-settings-capabilities` (new) | Complete composition seam for Tool, Skill, MCP-server, and Subagent-provider recipes; catalog reads mount no Profile, global/Profile load-unload inheritance drives native Loader patches, hard dependencies auto-load, running Sessions stay pinned, and each live Session exposes its immutable generation result. |
...
| `dsh-base` Agent capability composition | Mounts the generic recipe and composition registry; global Agent load/unload values inherit into Profile values, while source YAML rows supply native defaults. |
| `dsh-web-app` capability management | Mounts the authorized static-recipe Remote, a Profile Assembly Settings tab, and a read-only Session Capabilities view. Catalog reads parse Profile files without starting them. The next standing generation compiles selections into Loader disable/insert patches; running Sessions stay pinned, compatible read-only Cordis Inspect providers hold shared generation leases, Host-shared MCP connections remain live while unloaded server tools stay hidden in that Profile, and shared Subagent providers remain read-only. |
