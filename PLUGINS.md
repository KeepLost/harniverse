...
| Cross-session query and delivery | Separates current-title/creation/raw-activity discovery from content search and exposes exact status, folded messages, complete raw history and windows, and session or event lineage through one `session_inspect` contract; cwd remains an optional filter rather than exact-target authority. The delivery Definition, local Provider, and Consumer expose `session_create`, non-waiting `session_message`, and safe idle-only `session_unload`; ordinary targets use the Agent inbox, while direct subagent targets retain parent authorization and cold recovery through `ctx.subagents.followup()`. The shared base and Web Agent Profiles expose one continuable spawn-backed `subagent` tool and keep fork/spawn providers mounted as runtime capabilities; the fork alias, global control/list/history tools, and child report tool remain optional plugins outside shipped model rosters. Indexed discovery/search opens lazily on first use. |
...
| Native Child Profile foundation | Native child creation accepts a host-resolved `childProfile`, applies its bounded workspace cwd and Tool allowlist, and persists the profile snapshot in descriptor version 3. No shipped default exposes profile definition or listing to the model; the opt-in `examples/web-cordis` overlay enables the management tools for Web verification without changing shipped defaults. |
| Web Child Profile and history surface | Standard, Code, and Cordis Web Agent presets enable Profile management with host-bound grants and default model routes. Delegation results carry child-session metadata into Trajectory, which exposes an authoritative, catalog-backed child-history action; the `subagent_history` tool reads bounded raw descendant events through native SQLite/JSONL pages when available, without cold-resuming the child. |
