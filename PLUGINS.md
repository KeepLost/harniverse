...
| Parent-private Child Profiles | `dsh-subagent` (official, modified; resolved profile contract, parent-private registry, and durable descriptor snapshot) | `dsh-subagent-in-process-driver`, `dsh-subagent-dsh-sdk` (official, modified; native/SDK workspace, route, Tool, Skill/MCP setup) | `dsh-tool-subagent` (official, modified; optional define/list and `profile_id` delegation) | Active seam: Host-resolved profiles carry only opaque harness/model-route/capability references, reject requests outside the parent grant, enforce descendant workspace paths and profile-bounded Tool filters, resolve primary model routes before startup, persist immutable revision/digest snapshots, and cross the SDK initialize handshake. Parent-private Session visibility/trace projection, route fallback/scheduler priority, and complete MCP/Skill registry adapters remain in progress. |
...
| Native Child Profile foundation | Native child creation accepts a host-resolved `childProfile`, applies its bounded workspace cwd and Tool allowlist, and persists the profile snapshot in descriptor version 3. No shipped default exposes profile definition or listing to the model yet. |
