...
| Read-only workspace inspection | `dsh-api-remotes` and `dsh-workspace` (modified) | `dsh-host-apiproxy` (modified) | `dsh-client-connection`, `dsh-client-runtime`, `dsh-client-ui-workspace` (modified) | In progress; bounded workspace file listing/UTF-8 preview and read-only Git status/history/diff RPCs are wired through the existing authenticated API, with browser-memory lazy-loading UI state. |
...
| Compaction composition, proactive trigger, and recall | `dsh-compaction`, `dsh-compaction-basic`, `dsh-base`, `dsh-web-app`, and the standalone headless example; standard and Cordis Agent presets select both model Consumers, while Code keeps automatic compaction and recall without the direct-only trigger. Request pressure now reserves output capacity before provider dispatch, retries reduced output caps before forced compaction, and manual compaction waits for active agent turns to settle. Auxiliary summary calls treat the configured compaction `maxTokens` as an upper bound and also honor the selected model cap plus remaining context capacity. |
...
| Workspace inspection transport and UI | `dsh-host-apiproxy`, `dsh-api-remotes`, `dsh-client-connection`, `dsh-client-runtime`, `dsh-client-ui-conversation`, `dsh-client-ui-workspace`, `dsh-test-support-client-runtime` |
