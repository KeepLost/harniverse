...
All modes treat the invoking directory as the default workspace root, load applicable `AGENTS.md` and configured custom instructions with a 65,536-byte render budget, and use an in-memory SQLite session content index. Every profile boot watches valid edits of both `cordis.patch.yml` layers (profile and home) and reapplies them transactionally; a one-shot surface exits through its bounded shutdown, which disposes the watchers.
...
`DSH_TOOLS_MODE` selects `native`, `code`, or `both` for the process; another value fails at boot. The shipped `minimal` agent preset uses the shared dynamic prompt assembly with persona `You are a helpful software engineer assistant.`, and composes only the platform shell (`bash` or `pwsh`) plus `str_replace_editor`; internal automatic lossless compaction and its summary DAG remain hidden from the model. Select 极简模式 when creating a Web session; every other prompt section and model-facing plugin remains absent from that agent while the shared browser, workspace, persistence, sandbox, and permission host stays in place.

## Shared deployment behavior

The base bundle mounts the native DeepSeek adapter, settings and credential providers, all three official search providers, and disabled session telemetry. Its host `tool-web` row disables both `web_search` and `web_fetch`; Standard, Code, and Cordis replace their Profile rows with explicit enabled Web tools, while Minimal keeps Web absent. Search provider ids and credential references are `deepseek-official` (the selected default) / `DEEPSEEK_API_KEY`, `exa` / `EXA_API_KEY`, and `perplexity` / `PERPLEXITY_API_KEY`. All three mount keyless; a missing credential fails only when an enabled search selects that provider, and selection never switches automatically based on available keys because synchronous `available()` cannot prove that an asynchronous credential store contains one. Provider credentials resolve from the inherited environment, `$DSH_HOME/.credentials.yaml`, the invoking directory's `.env`, then `$DSH_HOME/.env`; the managed document is never materialized into `process.env`, while both `.env` files are ordinary launch environment layers. DeepSeek search also accepts `DEEPSEEK_SEARCH_BASE_URL`; the local HTTP fetch provider is enabled only by the Standard-family Profile rows.

`DSH_WEB_SEARCH_PROVIDER` changes the untouched shipped default. An explicit later `web.config.searchProvider` patch wins because a patch replaces the row's complete config. `WebRuntime` exposes a live `web` settings section containing only `searchProvider`; the browser override applies to the next search, and clearing it restores the composed selection. `fetchProvider` remains composition-only, and each search snapshots its selection at operation entry.

The Host allowlists `web`, `web-search-deepseek`, `web-search-exa`, and `web-search-perplexity`. One browser Web search card binds all four settings scopes and stages the selector plus every provider form, retaining hidden-provider drafts. It exposes each provider's write-only key and provider-specific fields: DeepSeek `baseURL`, `model`, `apiVersion`, `maxTokens`, and `maxUses`; Exa `baseURL`, `searchType`, `numResults`, and `highlightsPerResult`; Perplexity `baseURL`, `model`, `maxTokens`, and `searchRecency`. One save writes the four namespaces sequentially and non-transactionally: successful writes settle, while each failed form preserves its drafts. Every provider reads live settings and credentials per search, snapshots its complete options once at operation entry, and treats the mounted credentials service as authoritative.
