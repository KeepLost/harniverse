...
describe('web e2e: Profile assembly', () => {
  ...
  it('keeps one recipe catalog across targets and applies inherited global composition through preview', async () => {
    ...
    await target.selectOption('profile:standard')
    await expect.poll(() => target.isEnabled()).toBe(true)
    const filesystem = dialog.getByRole('heading', { name: 'fs', exact: true }).locator('xpath=ancestor::li[1]')
    await filesystem.locator('summary').filter({ hasText: '内置工具' }).click()
    await filesystem.getByRole('checkbox', { name: /write/ }).click()
    const persona = dialog.getByRole('heading', { name: 'persona', exact: true }).locator('xpath=ancestor::li[1]')
    await persona.locator('summary').filter({ hasText: 'Profile 配置' }).click()
    await persona.getByRole('textbox', { name: /Persona 提示词/ }).fill('You are a focused reviewer.')
    await dialog.getByText('2 项待处理变更').waitFor()
    await dialog.getByRole('button', { name: '放弃修改' }).click()
    await target.selectOption('global')
    await expect.poll(() => target.isEnabled()).toBe(true)

    const bash = dialog.getByRole('heading', { name: 'bash', exact: true }).locator('xpath=ancestor::li[1]')
    ...
    const refreshedBash = dialog.getByRole('heading', { name: 'bash', exact: true }).locator('xpath=ancestor::li[1]')
    await expect.poll(() => refreshedBash.getByText('未选择加载').count(), { timeout: 10_000 }).toBe(1)

    await target.selectOption('profile:minimal')
    await expect.poll(() => target.isEnabled()).toBe(true)
    const inheritedBash = dialog.getByRole('heading', { name: 'bash', exact: true }).locator('xpath=ancestor::li[1]')
