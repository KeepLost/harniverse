...
Linux x64, Windows x64 and macOS arm64 have passed native installer packaging, packaged runtime probes, authenticated Host/CDP browser qualification and the empty-path clean-install smoke in CI. The [desktop architecture note](../../.agents/notes/implemented/architecture/2026-09-24-desktop-shell-lifecycle.md#consequences) records the exact revision and platform evidence. This qualification covers the packaged runtime and shell lifecycle; interactive installer upgrades and native update interruption/recovery require separate platform exercises. Release signing and notarization remain separate release gates.
